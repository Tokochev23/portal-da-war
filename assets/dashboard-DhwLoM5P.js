import{a as _,f as E,h as y,_ as ie,g as te}from"./firebase-DhoRyF0N.js";/* empty css             */import{E as we}from"./economicCalculations-EmLH2T6y.js";import{C as _e}from"./consumerGoodsCalculator-BzhzLFUc.js";import{R as ae}from"./resourceConsumptionCalculator-ulCp7K3s.js";import{R as se}from"./resourceProductionCalculator-OqGd_1BQ.js";import{S as ne}from"./shipyardSystem-DcDxaQqG.js";import{g as Ee}from"./renderer-BYnRIEt4.js";import"https://www.gstatic.com/firebasejs/10.12.2/firebase-app-compat.js";import"https://www.gstatic.com/firebasejs/10.12.2/firebase-auth-compat.js";import"https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore-compat.js";import"https://www.gstatic.com/firebasejs/10.12.2/firebase-storage-compat.js";class Ce{constructor(){this.collections={offers:"marketplace_offers",transactions:"marketplace_transactions",embargoes:"marketplace_embargoes",orders:"marketplace_orders"}}getOfferSchema(){return{id:"string",type:"string",category:"string",title:"string",description:"string",item_id:"string",item_name:"string",quantity:"number",unit:"string",price_per_unit:"number",total_value:"number",country_id:"string",country_name:"string",country_flag:"string",player_id:"string",status:"string",created_at:"timestamp",updated_at:"timestamp",expires_at:"timestamp",min_quantity:"number",max_quantity:"number",delivery_time_days:"number",views:"number",interested_countries:"array",tech_level_required:"number",diplomatic_status_required:"string"}}getTransactionSchema(){return{id:"string",offer_id:"string",seller_country_id:"string",seller_country_name:"string",seller_player_id:"string",buyer_country_id:"string",buyer_country_name:"string",buyer_player_id:"string",item_id:"string",item_name:"string",quantity:"number",unit:"string",price_per_unit:"number",total_value:"number",status:"string",created_at:"timestamp",confirmed_at:"timestamp",completed_at:"timestamp",delivery_deadline:"timestamp",status_history:"array",delivery_time_days:"number",delivery_status:"string",negotiated_price:"boolean",original_price_per_unit:"number",discount_percent:"number"}}getEmbargoSchema(){return{id:"string",embargo_country_id:"string",embargo_country_name:"string",target_country_id:"string",target_country_name:"string",type:"string",categories_blocked:"array",reason:"string",created_at:"timestamp",expires_at:"timestamp",status:"string",created_by_player_id:"string",notifications_sent:"boolean"}}getOrderSchema(){return{id:"string",country_id:"string",country_name:"string",player_id:"string",type:"string",item_id:"string",item_name:"string",category:"string",quantity:"number",unit:"string",max_price_per_unit:"number",min_price_per_unit:"number",is_recurring:"boolean",recurrence_type:"string",recurrence_interval:"number",max_executions:"number",executions_count:"number",status:"string",created_at:"timestamp",last_execution:"timestamp",next_execution:"timestamp",auto_execute:"boolean",require_confirmation:"boolean",execution_history:"array"}}async createOffer(e){try{const a=_.currentUser;if(!a)throw new Error("Usuário não autenticado");const s=await E(a.uid);if(!s)throw new Error("Jogador não associado a um país");const i=await this.validateOfferData(e,s,a.uid),r={...i,country_id:s,player_id:a.uid,created_at:new Date,updated_at:new Date,expires_at:new Date(Date.now()+i.duration_days*24*60*60*1e3),status:"active",views:0,interested_countries:[]},o=await y.collection(this.collections.offers).add(r);return{success:!0,offerId:o.id,offer:{id:o.id,...r}}}catch(a){return console.error("Erro ao criar oferta:",a),{success:!1,error:a.message}}}async validateOfferData(e,a,s){if(!e.type||!["sell","buy"].includes(e.type))throw new Error("Tipo de oferta inválido");if(!e.category||!["resources","vehicles","naval"].includes(e.category))throw new Error("Categoria inválida");if(!e.title||e.title.trim().length<3)throw new Error("Título deve ter pelo menos 3 caracteres");if(!e.quantity||e.quantity<=0)throw new Error("Quantidade deve ser maior que zero");if(!e.price_per_unit||e.price_per_unit<=0)throw new Error("Preço deve ser maior que zero");const i=await y.collection("paises").doc(a).get();if(!i.exists)throw new Error("País não encontrado");const r=i.data();e.type==="sell"?(await this.validateSellOfferAvailability(e,a),await this.validateSellOffer(e,r)):await this.validateBuyOffer(e,r);const o=e.quantity*e.price_per_unit;return{type:e.type,category:e.category,title:e.title.trim(),description:(e.description||"").trim(),item_id:e.item_id,item_name:e.item_name,quantity:e.quantity,unit:e.unit,price_per_unit:e.price_per_unit,total_value:o,country_name:r.Pais,country_flag:r.Flag||"🏳️",delivery_time_days:e.delivery_time_days||30,min_quantity:e.min_quantity||1,max_quantity:e.max_quantity||e.quantity,tech_level_required:e.tech_level_required||0,duration_days:e.duration_days||30}}async validateSellOffer(e,a){if(e.category!=="resources"){if(e.category==="vehicles"||e.category==="naval"){const s=await y.collection("inventory").doc(a.id||e.country_id).get();s.exists&&s.data()}}}async validateBuyOffer(e,a){const s=this.calculateCountryBudget(a),i=e.quantity*e.price_per_unit;if(s<i)throw new Error(`Orçamento insuficiente. Necessário: $${i.toLocaleString()}, Disponível: $${s.toLocaleString()}`)}calculateCountryBudget(e){const a=parseFloat(e.PIB)||0,s=(parseFloat(e.Burocracia)||0)/100,i=(parseFloat(e.Estabilidade)||0)/100;return a*.25*s*(i*1.5)}async getOffers(e={}){try{const a=y.collection(this.collections.offers);let s=[],i;try{console.info("🔍 Buscando ofertas usando filtragem client-side (sem índices)"),i=await a.get()}catch(l){console.warn("Falha ao buscar todos os documentos, tentando listagem:",l);try{const c=(await a.listDocuments()).slice(0,100).map(b=>b.get()),p=await Promise.all(c);i={docs:p.filter(b=>b.exists),size:p.filter(b=>b.exists).length,empty:p.filter(b=>b.exists).length===0,forEach:function(b){this.docs.forEach(b)}}}catch(n){console.warn("Listagem de documentos falhou, retornando resultados vazios:",n),i={docs:[],size:0,empty:!0,forEach:function(){}}}}i.forEach(l=>{const n=l.data();if(n.status!=="active")return;const c=n.expires_at?.toDate?n.expires_at.toDate():new Date(n.expires_at);c&&c<=new Date||s.push({id:l.id,...n})});let r=s;if(e.category&&e.category!=="all"&&(r=r.filter(l=>l.category===e.category)),e.type&&e.type!=="all"&&(r=r.filter(l=>l.type===e.type)),e.seller_id&&(r=r.filter(l=>l.seller_id===e.seller_id)),e.buyer_id&&(r=r.filter(l=>l.buyer_id===e.buyer_id)),e.searchTerm){const l=e.searchTerm.toLowerCase();r=r.filter(n=>n.title&&n.title.toLowerCase().includes(l)||n.description&&n.description.toLowerCase().includes(l)||n.item_name&&n.item_name.toLowerCase().includes(l)||n.country_name&&n.country_name.toLowerCase().includes(l))}e.priceMin!==null&&e.priceMin!==void 0&&(r=r.filter(l=>l.price_per_unit>=e.priceMin)),e.priceMax!==null&&e.priceMax!==void 0&&(r=r.filter(l=>l.price_per_unit<=e.priceMax)),e.quantityMin!==null&&e.quantityMin!==void 0&&(r=r.filter(l=>l.quantity>=e.quantityMin)),e.quantityMax!==null&&e.quantityMax!==void 0&&(r=r.filter(l=>l.quantity<=e.quantityMax));const o=e.orderBy||"created_at",d=e.orderDirection||"desc";if(r.sort((l,n)=>{let c=l[o],p=n[o];return o.includes("_at")&&c&&p&&(c=c.toDate?c.toDate():new Date(c),p=p.toDate?p.toDate():new Date(p)),typeof c=="string"&&!isNaN(c)&&(c=parseFloat(c)),typeof p=="string"&&!isNaN(p)&&(p=parseFloat(p)),d==="desc"?p>c?1:p<c?-1:0:c>p?1:c<p?-1:0}),e.countryFilter&&(r=r.filter(l=>l.country_id===e.countryFilter)),e.timeFilter!==null&&e.timeFilter!==void 0){const l=new Date,n=new Date(l.getTime()+e.timeFilter*24*60*60*1e3);r=r.filter(c=>(c.expires_at?.toDate?c.expires_at.toDate():new Date(c.expires_at))<=n)}return r=await this.filterEmbargoedOffers(r,e.current_country_id),e.limit&&r.length>e.limit&&(r=r.slice(0,e.limit)),console.info(`✅ Ofertas filtradas: ${r.length} encontradas usando filtragem client-side`),{success:!0,offers:r,total:r.length,totalCount:r.length}}catch(a){return console.error("Erro ao buscar ofertas:",a),{success:!1,error:a.message,offers:[]}}}async filterEmbargoedOffers(e,a){if(!a)return e;try{const s=await y.collection(this.collections.embargoes).where("status","==","active").where("target_country_id","==",a).get(),i=[];return s.forEach(r=>{i.push(r.data())}),i.length===0?e:e.filter(r=>{const o=i.find(d=>d.embargo_country_id===r.country_id);return o?o.type==="full"?!1:o.type==="partial"&&o.categories_blocked?!o.categories_blocked.includes(r.category):!0:!0})}catch(s){return console.error("Erro ao verificar embargos:",s),e}}async incrementOfferViews(e){try{const a=y.collection(this.collections.offers).doc(e),s=await a.get();if(s.exists){const i=s.data().views||0;await a.update({views:i+1,updated_at:new Date})}}catch(a){console.error("Erro ao incrementar visualizações:",a)}}async createTransaction(e,a){try{const s=_.currentUser;if(!s)throw new Error("Usuário não autenticado");const i=await E(s.uid);if(!i)throw new Error("Jogador não associado a um país");const r=await y.collection(this.collections.offers).doc(e).get();if(!r.exists)throw new Error("Oferta não encontrada");const o=r.data();if(o.status!=="active")throw new Error("Oferta não está ativa");const d=a.quantity||o.quantity;if(d>o.quantity)throw new Error("Quantidade solicitada excede disponível");if(d<o.min_quantity)throw new Error(`Quantidade mínima: ${o.min_quantity}`);if(d>o.max_quantity)throw new Error(`Quantidade máxima: ${o.max_quantity}`);const n=(await y.collection("paises").doc(i).get()).data(),c=d*o.price_per_unit;if(this.calculateCountryBudget(n)<c)throw new Error("Orçamento insuficiente");const b={offer_id:e,seller_country_id:o.country_id,seller_country_name:o.country_name,seller_player_id:o.player_id,buyer_country_id:i,buyer_country_name:n.Pais,buyer_player_id:s.uid,item_id:o.item_id,item_name:o.item_name,quantity:d,unit:o.unit,price_per_unit:o.price_per_unit,total_value:c,status:"pending",created_at:new Date,delivery_deadline:new Date(Date.now()+o.delivery_time_days*24*60*60*1e3),delivery_time_days:o.delivery_time_days,delivery_status:"pending",status_history:[{status:"pending",timestamp:new Date,note:"Transação criada"}],negotiated_price:!1,original_price_per_unit:o.price_per_unit,discount_percent:0},v=await y.collection(this.collections.transactions).add(b);return await y.collection(this.collections.offers).doc(e).update({quantity:o.quantity-d,updated_at:new Date,status:o.quantity-d===0?"completed":"active"}),{success:!0,transactionId:v.id,transaction:{id:v.id,...b}}}catch(s){return console.error("Erro ao criar transação:",s),{success:!1,error:s.message}}}async applyEmbargo(e){try{const a=_.currentUser;if(!a)throw new Error("Usuário não autenticado");const s=await E(a.uid);if(!s)throw new Error("Jogador não associado a um país");const r=(await y.collection("paises").doc(s).get()).data(),o=await y.collection("paises").doc(e.target_country_id).get();if(!o.exists)throw new Error("País alvo não encontrado");const d=o.data();if(!(await y.collection(this.collections.embargoes).where("embargo_country_id","==",s).where("target_country_id","==",e.target_country_id).where("status","==","active").get()).empty)throw new Error("Já existe um embargo ativo contra este país");const n={embargo_country_id:s,embargo_country_name:r.Pais,target_country_id:e.target_country_id,target_country_name:d.Pais,type:e.type||"full",categories_blocked:e.categories_blocked||[],reason:e.reason||"Motivos diplomáticos",created_at:new Date,expires_at:e.expires_at||null,status:"active",created_by_player_id:a.uid,notifications_sent:!1},c=await y.collection(this.collections.embargoes).add(n);return await this.sendEmbargoNotification(n,c.id),{success:!0,embargoId:c.id,embargo:{id:c.id,...n}}}catch(a){return console.error("Erro ao aplicar embargo:",a),{success:!1,error:a.message}}}async sendEmbargoNotification(e,a){try{const s={type:"embargo_applied",embargo_id:a,target_country_id:e.target_country_id,target_country_name:e.target_country_name,embargo_country_id:e.embargo_country_id,embargo_country_name:e.embargo_country_name,embargo_type:e.type,categories_blocked:e.categories_blocked||[],reason:e.reason,created_at:new Date,read:!1,expires_at:e.expires_at,title:`🚫 Embargo Aplicado por ${e.embargo_country_name}`,message:this.getEmbargoNotificationMessage(e),priority:"high"};await y.collection("notifications").add(s),await y.collection(this.collections.embargoes).doc(a).update({notifications_sent:!0}),console.log(`Notificação de embargo enviada para ${e.target_country_name}`)}catch(s){console.error("Erro ao enviar notificação de embargo:",s)}}getEmbargoNotificationMessage(e){const a=e.type==="full"?"total":"parcial";let s=`${e.embargo_country_name} aplicou um embargo ${a} contra seu país.`;if(e.type==="partial"&&e.categories_blocked&&e.categories_blocked.length>0){const i={resources:"Recursos",vehicles:"Veículos",naval:"Naval"},r=e.categories_blocked.map(o=>i[o]||o);s+=` Categorias bloqueadas: ${r.join(", ")}.`}else e.type==="full"&&(s+=" Todas as trocas comerciais estão bloqueadas.");if(e.reason&&e.reason!=="Motivos diplomáticos"&&(s+=` Motivo: ${e.reason}`),e.expires_at){const i=new Date(e.expires_at),r=Math.ceil((i-new Date)/(1440*60*1e3));s+=` O embargo expira em ${r} dias.`}else s+=" O embargo é por tempo indefinido.";return s}async getEmbargoNotifications(e,a=10){try{const s=await y.collection("notifications").where("target_country_id","==",e).where("type","==","embargo_applied").orderBy("created_at","desc").limit(a).get(),i=[];return s.forEach(r=>{i.push({id:r.id,...r.data()})}),{success:!0,notifications:i}}catch(s){return console.error("Erro ao buscar notificações:",s),{success:!1,error:s.message,notifications:[]}}}async markNotificationAsRead(e){try{return await y.collection("notifications").doc(e).update({read:!0,read_at:new Date}),{success:!0}}catch(a){return console.error("Erro ao marcar notificação como lida:",a),{success:!1,error:a.message}}}async getCountryInventory(e){try{const a=await y.collection("inventory").doc(e).get();return a.exists?a.data():{}}catch(a){return console.error("Erro ao buscar inventário:",a),{}}}async getCountryData(e){try{const a=await y.collection("paises").doc(e).get();return a.exists?{id:e,...a.data()}:null}catch(a){return console.error("Erro ao buscar dados do país:",a),null}}calculateAvailableResources(e){console.log("🔍 Debug: Dados do país recebidos:",e),console.log("📊 Usando recursos REAIS do dashboard (não calculados)"),console.log("🔍 Recursos reais no país:"),console.log("  - Carvao:",e.Carvao),console.log("  - Combustivel:",e.Combustivel),console.log("  - Metais:",e.Metais),console.log("  - Graos:",e.Graos);const a={Carvao:Math.max(0,Math.floor((parseFloat(e.Carvao)||0)*.5)),Combustivel:Math.max(0,Math.floor((parseFloat(e.Combustivel)||0)*.5)),Metais:Math.max(0,Math.floor((parseFloat(e.Metais)||0)*.5)),Graos:Math.max(0,Math.floor((parseFloat(e.Graos)||0)*.5))};return console.log("📦 Recursos REAIS disponíveis para venda (50% do estoque):",a),a}getAvailableEquipment(e){const a=[];return Object.keys(e).forEach(s=>{typeof e[s]=="object"&&e[s]!==null&&Object.keys(e[s]).forEach(i=>{const r=e[s][i];if(r&&typeof r=="object"&&r.quantity>0){const o=Math.floor(r.quantity*.5);o>0&&a.push({id:`${s}_${i}`.toLowerCase().replace(/\s+/g,"_"),name:i,category:s,available_quantity:o,total_quantity:r.quantity,unit_cost:r.cost||0,maintenance_cost:(r.cost||0)*.05||0,type:this.getEquipmentType(s)})}})}),a}getEquipmentType(e){return["Couraçados","Cruzadores","Destróieres","Fragatas","Corvetas","Submarinos","Porta-aviões","Patrulhas","Auxiliares","Naval - Outros"].includes(e)?"naval":"vehicles"}async validateSellOfferAvailability(e,a){const s=await this.getCountryData(a);if(!s)throw new Error("Dados do país não encontrados");return e.category==="resources"?this.validateResourceAvailability(e,s):this.validateEquipmentAvailability(e,a)}validateResourceAvailability(e,a){const s=this.calculateAvailableResources(a),r={steel_high_grade:"Metais",steel_standard:"Metais",oil_crude:"Combustivel",oil_aviation:"Combustivel",aluminum:"Metais",copper:"Metais",rare_metals:"Metais",coal:"Carvao",food:"Graos"}[e.item_id];if(!r)throw new Error("Tipo de recurso não reconhecido");const o=s[r]||0;if(e.quantity>o)throw new Error(`Quantidade insuficiente. Disponível para venda: ${o.toLocaleString()} unidades de ${r}`);return{valid:!0,available:o,resourceType:r}}async validateEquipmentAvailability(e,a){const s=await this.getCountryInventory(a),r=this.getAvailableEquipment(s).find(o=>o.id===e.item_id);if(!r)throw new Error(`Equipamento "${e.item_name}" não encontrado no inventário ou indisponível para venda`);if(e.quantity>r.available_quantity)throw new Error(`Quantidade insuficiente. Disponível para venda: ${r.available_quantity} de ${r.total_quantity} unidades totais`);return{valid:!0,equipment:r,availableQuantity:r.available_quantity,totalQuantity:r.total_quantity}}async createTestOffers(){try{console.log("🧪 Criando ofertas de teste...");const e=[{type:"sell",category:"resources",title:"Aço de Alta Qualidade",description:"Aço especializado para construção naval e industrial",item_id:"steel_high_grade",item_name:"Aço de Alta Qualidade",quantity:5e3,unit:"toneladas",price_per_unit:850,total_value:5e3*850,country_id:"test_country_1",country_name:"Estados Unidos",country_flag:"🇺🇸",player_id:"test_player_1",status:"active",created_at:new Date,updated_at:new Date,expires_at:new Date(Date.now()+10080*60*1e3),delivery_time_days:30,min_quantity:100,max_quantity:5e3,views:23,interested_countries:[]},{type:"buy",category:"vehicles",title:"Tanques MBT Modernos",description:"Procurando tanques de batalha principais para modernização das forças armadas",item_id:"mbt_modern",item_name:"Tanque MBT Moderno",quantity:50,unit:"unidades",price_per_unit:25e5,total_value:50*25e5,country_id:"test_country_2",country_name:"Brasil",country_flag:"🇧🇷",player_id:"test_player_2",status:"active",created_at:new Date(Date.now()-1440*60*1e3),updated_at:new Date(Date.now()-1440*60*1e3),expires_at:new Date(Date.now()+336*60*60*1e3),delivery_time_days:45,min_quantity:5,max_quantity:50,views:45,interested_countries:[]},{type:"sell",category:"naval",title:"Destroyers Classe Fletcher",description:"Destroyers modernizados, prontos para serviço imediato",item_id:"destroyer_fletcher",item_name:"Destroyer Classe Fletcher",quantity:3,unit:"navios",price_per_unit:18e7,total_value:3*18e7,country_id:"test_country_3",country_name:"Reino Unido",country_flag:"🇬🇧",player_id:"test_player_3",status:"active",created_at:new Date(Date.now()-4320*60*1e3),updated_at:new Date(Date.now()-4320*60*1e3),expires_at:new Date(Date.now()+504*60*60*1e3),delivery_time_days:60,min_quantity:1,max_quantity:3,views:67,interested_countries:[]},{type:"sell",category:"resources",title:"Petróleo Refinado",description:"Combustível de alta octanagem para aviação militar",item_id:"oil_aviation",item_name:"Petróleo de Aviação",quantity:1e4,unit:"barris",price_per_unit:120,total_value:1e4*120,country_id:"test_country_4",country_name:"Arábia Saudita",country_flag:"🇸🇦",player_id:"test_player_4",status:"active",created_at:new Date(Date.now()-2880*60*1e3),updated_at:new Date(Date.now()-2880*60*1e3),expires_at:new Date(Date.now()+14400*60*1e3),delivery_time_days:15,min_quantity:500,max_quantity:1e4,views:89,interested_countries:[]},{type:"buy",category:"naval",title:"Submarinos Diesel-Elétricos",description:"Necessitamos de submarinos para patrulha costeira",item_id:"submarine_diesel",item_name:"Submarino Diesel-Elétrico",quantity:2,unit:"submarinos",price_per_unit:45e6,total_value:2*45e6,country_id:"test_country_5",country_name:"Argentina",country_flag:"🇦🇷",player_id:"test_player_5",status:"active",created_at:new Date(Date.now()-5760*60*1e3),updated_at:new Date(Date.now()-5760*60*1e3),expires_at:new Date(Date.now()+720*60*60*1e3),delivery_time_days:90,min_quantity:1,max_quantity:2,views:12,interested_countries:[]}];for(const a of e)await y.collection(this.collections.offers).add(a);return console.log(`✅ ${e.length} ofertas de teste criadas com sucesso!`),{success:!0,count:e.length}}catch(e){return console.error("❌ Erro ao criar ofertas de teste:",e),{success:!1,error:e.message}}}async clearTestOffers(){try{console.log("🗑️ Removendo ofertas de teste...");const e=await y.collection(this.collections.offers).where("player_id",">=","test_player_").where("player_id","<","test_player_z").get(),a=y.batch();return e.docs.forEach(s=>{a.delete(s.ref)}),await a.commit(),console.log(`✅ ${e.docs.length} ofertas de teste removidas!`),{success:!0,count:e.docs.length}}catch(e){return console.error("❌ Erro ao remover ofertas de teste:",e),{success:!1,error:e.message}}}}function $e(t){return t<=20?{label:"Anarquia",tone:"bg-rose-500/15 text-rose-300 border-rose-400/30"}:t<=49?{label:"Instável",tone:"bg-amber-500/15 text-amber-300 border-amber-400/30"}:t<=74?{label:"Neutro",tone:"bg-sky-500/15 text-sky-300 border-sky-400/30"}:{label:"Tranquilo",tone:"bg-emerald-500/15 text-emerald-300 border-emerald-400/30"}}function S(t){if(!t||isNaN(t))return"0";const e=parseFloat(t);return e>=1e9?(e/1e9).toFixed(1)+"B":e>=1e6?(e/1e6).toFixed(1)+"M":e>=1e3?(e/1e3).toFixed(0)+"K":Math.round(e).toLocaleString("pt-BR")}function C(t){if(!t)return"US$ 0";const e=parseFloat(t);return e>=1e9?"US$ "+(e/1e9).toFixed(1)+"B":e>=1e6?"US$ "+(e/1e6).toFixed(1)+"M":e>=1e3?"US$ "+(e/1e3).toFixed(1)+"K":"US$ "+e.toLocaleString()}function w(t){const e=parseFloat(t)||0;return e===0?"US$ 0":e>=1e9?`US$ ${(e/1e9).toFixed(1)}bi`:e>=1e6?`US$ ${(e/1e6).toFixed(1)}mi`:e>=1e3?`US$ ${(e/1e3).toFixed(1)}mil`:`US$ ${Math.round(e)}`}function Me(t){const e=parseFloat(t.WarPower)||0;return Math.round(e)}function O(t){const e=parseFloat(t.PIB)||0,a=(parseFloat(t.Burocracia)||0)/100,s=(parseFloat(t.Estabilidade)||0)/100,orcamentoTotal=e*.25*a*(s*1.5),orcamentoGasto=parseFloat(t.OrcamentoGasto||0),agencyBudget=parseFloat(t.AgencyBudgetSpent||0);return Math.max(0,orcamentoTotal-orcamentoGasto-agencyBudget)}function le(t){const e=O(t),a=(parseFloat(t.MilitaryBudgetPercent)||30)/100;return e*a}function de(t){const e=(parseFloat(t.MilitaryDistributionVehicles)||40)/100,a=(parseFloat(t.MilitaryDistributionAircraft)||30)/100,s=(parseFloat(t.MilitaryDistributionNaval)||30)/100;return{vehicles:e,aircraft:a,naval:s,maintenancePercent:.15}}function qe(t){const s=(parseFloat(t.MilitaryBudgetPercent)||30)-30;let i=0,r=0;return s>0&&(i=Math.floor(s/2)*-1,r=s*-.5),{stabilityPenalty:i,economicPenalty:r,isOverBudget:s>0}}function K(t){const e=le(t),a=de(t),s=e*(1-a.maintenancePercent),i=(parseFloat(t.Tecnologia)||0)/100,r=(parseFloat(t.IndustrialEfficiency)||30)/100,o=(parseFloat(t.Marinha)||0)/100,d=(parseFloat(t.Urbanizacao)||0)/100;return s*a.naval*i*r*o*d}function ke(t){we.computeEnergyDemandGW(t);const e=_e.calculateConsumerGoods(t),a=ae.calculateCountryConsumption(t),s=se.calculateCountryProduction(t),i={Carvao:Math.round((s.Carvao||0)-(a.Carvao||0)),Combustivel:Math.round((s.Combustivel||0)-(a.Combustivel||0)),Metais:Math.round((s.Metais||0)-(a.Metais||0)),Graos:Math.round((s.Graos||0)-(a.Graos||0)),Energia:Math.round((s.Energia||0)-(a.Energia||0))},r=$e(parseFloat(t.Estabilidade)||0),o=Me(t),d=(parseFloat(t.PIB)||0)/(parseFloat(t.Populacao)||1),l=O(t),n=le(t),c=de(t),p=n*c.vehicles,b=n*c.aircraft,v=n*c.naval,x=qe(t),h=window.getFlagHTML||function(u,m="h-full w-full"){return'<span class="text-slate-400 text-xs">—</span>'};return`
          ...
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-6">
              <div class="text-right">
                ...
                <div class="text-2xl font-bold text-red-400">${S(o)}</div>
              </div>
              <div class="text-right">
              ...
              <div class="px-3 py-1 rounded-lg border ${r.tone}">
      ...
      <div class="max-w-7xl mx-auto px-6 py-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                  ...
                  ${x.isOverBudget?`
                    <div class="bg-red-500/10 border border-red-500/20 rounded-lg p-3 mb-3">
                      <div class="text-xs text-red-400">
                        ...
                        Penalidade de estabilidade: ${x.stabilityPenalty}%<br>
                        Impacto econômico: ${x.economicPenalty}% no crescimento
                      </div>
                    </div>
                  `:""}
                </div>
              ...
              <div class="bg-slate-900/50 border border-slate-800/50 rounded-xl p-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                  ${Object.entries(i).map(([u,m])=>{const g={Carvao:"🪨",Combustivel:"⛽",Metais:"🔩",Graos:"🌾",Energia:"⚡"},f={Carvao:"Carvão",Combustivel:"Combustível",Metais:"Metais",Graos:"Grãos",Energia:"Energia"},$=u==="Energia"?"MW":"";return`
                      <div class="text-center">
                        <div class="text-lg mb-1">${g[u]}</div>
                        <div class="text-xs text-slate-400 mb-1">${f[u]}</div>
                        <div class="text-sm font-bold ${m>=0?"text-emerald-400":"text-red-400"}">
                          ${m>=0?"+":""}${S(m)}${$}
                        </div>
                      </div>
                    `}).join("")}
              ...
              <div class="bg-slate-900/50 border border-slate-800/50 rounded-xl p-6">
                ...
                <div class="space-y-3">
                  <div class="flex items-center justify-between rounded-lg border border-white/5 bg-slate-800/30 px-4 py-3">
                    ...
                    <span class="text-sm font-semibold text-blue-400">${w(p)}/turno</span>
                  </div>
                  <div class="flex items-center justify-between rounded-lg border border-white/5 bg-slate-800/30 px-4 py-3">
                    <div class="flex items-center gap-3">
                      <span class="text-lg">✈️</span>
                ...
                <div class="space-y-4">
                  <a href="criador-veiculos-refatorado.html" class="block p-4 bg-slate-800/30 hover:bg-slate-700/50 rounded-lg border border-slate-700/30 transition-colors">
                    <div class="flex items-center justify-between">
                      ...
                      <div class="text-right">
                        <div class="text-sm font-semibold text-slate-200">${w(p)}/turno</div>
                      </div>
                    </div>
                  </a>
        ...
        <div id="tab-resources" class="dashboard-tab-content hidden">
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ${["Carvao","Combustivel","Metais","Graos","Energia"].map(u=>{const m={Carvao:"🪨",Combustivel:"⛽",Metais:"🔩",Graos:"🌾",Energia:"⚡"},g={Carvao:"Carvão",Combustivel:"Combustível",Metais:"Metais",Graos:"Grãos",Energia:"Energia"},f=t[`Potencial${u}`]||(u==="Energia"?"N/A":"3"),$=Math.round(s[u]||0),I=Math.round(a[u]||0),M=i[u],k=u==="Energia"?"MW":"unidades";return`
                <div class="bg-slate-900/50 border border-slate-800/50 rounded-xl p-6">
                  <div class="flex items-center justify-between mb-4">
                    <div class="flex items-center gap-3">
                      <span class="text-2xl">${m[u]}</span>
                      <div>
                        <h3 class="text-lg font-semibold text-slate-200">${g[u]}</h3>
                        ${u!=="Energia"?`<p class="text-sm text-slate-400">Potencial: ${f}/10</p>`:""}
                      </div>
                    </div>
                    <div class="text-right">
                      <div class="text-2xl font-bold ${M>=0?"text-emerald-400":"text-red-400"}">
                        ${M>=0?"+":""}${S(M)}
                      </div>
                      <div class="text-xs text-slate-400">${k}/mês</div>
                    </div>
                  </div>
                  ...
                  <div class="grid grid-cols-2 gap-4 mb-4">
                    <div class="bg-slate-800/30 rounded-lg p-3">
                      ...
                      <div class="text-lg font-semibold text-cyan-400">${S($)} ${k}</div>
                    </div>
                    <div class="bg-slate-800/30 rounded-lg p-3">
                      <div class="text-xs text-slate-400 uppercase tracking-wide">Consumo</div>
                      <div class="text-lg font-semibold text-amber-400">${S(I)} ${k}</div>
                    </div>
                  </div>
                  ...
                  <div class="space-y-2">
                    <div class="flex justify-between text-sm">
                      ...
                      <span class="text-slate-200">${$>0?Math.round($/Math.max(I,1)*100):0}%</span>
                    </div>
                    <div class="w-full bg-slate-800 rounded-full h-2">
                      <div class="h-2 rounded-full ${M>=0?"bg-emerald-500":"bg-red-500"}"
                           style="width: ${Math.min(Math.max($/Math.max(I,1)*100,0),100)}%"></div>
                    </div>
                  </div>
                </div>
              `}).join("")}
  ...
  `}function Ie(){document.addEventListener("click",t=>{if(t.target.matches(".dashboard-tab")){const e=t.target.dataset.tab;document.querySelectorAll(".dashboard-tab").forEach(a=>{a.classList.remove("active","border-blue-500","text-blue-400"),a.classList.add("border-transparent","text-slate-400")}),t.target.classList.add("active","border-blue-500","text-blue-400"),t.target.classList.remove("border-transparent","text-slate-400"),document.querySelectorAll(".dashboard-tab-content").forEach(a=>{a.classList.add("hidden")}),document.getElementById(`tab-${e}`)?.classList.remove("hidden"),e==="vehicles"&&Be(),e==="naval"&&ue(),e==="market"&&Fe()}if(t.target.matches(".inventory-category-card")||t.target.closest(".inventory-category-card")){const a=t.target.closest(".inventory-category-card").dataset.category;Te(a)}if(t.target.matches(".equipment-item")||t.target.closest(".equipment-item")){const e=t.target.closest(".equipment-item"),a=e.dataset.equipment,s=e.dataset.category;ce(s,a)}})}async function Be(){try{const t=document.getElementById("vehicles-inventory-container");if(!t)return;t.innerHTML=`
      <div class="flex items-center justify-center py-8">
        <div class="text-slate-400">🔄 Carregando inventário...</div>
      </div>
    `;const e=_.currentUser;if(!e)return;const a=await E(e.uid);if(!a)return;const s=await y.collection("inventory").doc(a).get();if(!s.exists){t.innerHTML=`
        <div class="bg-slate-900/50 border border-slate-800/50 rounded-xl p-6 text-center">
      ...
      `;return}const i=s.data();t.innerHTML=Le(i)}catch(t){console.error("Erro ao carregar inventário:",t);const e=document.getElementById("vehicles-inventory-container");e&&(e.innerHTML=`
      ...
      `)}}function Le(t){const e=Object.keys(t);if(e.length===0)return`
      <div class="bg-slate-900/50 border border-slate-800/50 rounded-xl p-6 text-center">
        <div class="text-6xl mb-4">📦</div>
        <h3 class="text-xl font-semibold text-slate-200 mb-2">Inventário Vazio</h3>
        <p class="text-slate-400">Nenhum equipamento aprovado encontrado</p>
    ...
    <div class="space-y-6">
      <div class="bg-slate-900/50 border border-slate-800/50 rounded-xl p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          ${e.map(s=>{const i=t[s],r=Object.keys(i).length,o=Object.values(i).reduce((l,n)=>l+(n.quantity||0),0),d=Object.values(i).reduce((l,n)=>{const c=n.quantity||0,b=(n.cost||0)*.05;return l+b*c},0);return`
              <div class="inventory-category-card bg-slate-800/30 hover:bg-slate-700/50 border border-slate-700/30 rounded-lg p-4 cursor-pointer transition-colors" data-category="${s}">
                <div class="text-center">
                  ...
                  <div class="text-xs text-slate-400 space-y-1">
                    <div>${r} tipo${r!==1?"s":""}</div>
                    <div>${o} unidade${o!==1?"s":""}</div>
  ...
  `}async function Te(t){try{const e=_.currentUser;if(!e)return;const a=await E(e.uid);if(!a)return;const s=await y.collection("inventory").doc(a).get();if(!s.exists)return;const r=s.data()[t];if(!r)return;const o=document.getElementById("inventory-details-modal");o&&o.remove();const d=document.createElement("div");d.id="inventory-details-modal",d.className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4";const l=document.createElement("div");l.className="bg-bg border border-bg-ring/70 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col";const n=Object.entries(r),c=n.reduce((b,[v,x])=>b+(x.quantity||0),0),p=n.reduce((b,[v,x])=>{const h=x.quantity||0,m=(x.cost||0)*.05;return b+m*h},0);l.innerHTML=`
      <div class="flex items-center justify-between p-6 border-b border-bg-ring/50">
        <div>
          ...
          <p class="text-sm text-slate-400">${n.length} equipamentos • ${c} unidades • ${w(p)}/mês manutenção</p>
        </div>
      ...
      <div class="flex-1 overflow-auto p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ${n.map(([b,v])=>{const x=v.quantity||0,h=v.cost||0,u=h*.05*x,m=h*x;return`
              <div class="equipment-item bg-slate-800/30 border border-slate-700/30 rounded-lg p-4 hover:bg-slate-700/50 cursor-pointer transition-colors"
                ...
                <div class="flex items-start justify-between mb-3">
                  <div class="flex-1">
                    ...
                    <div class="text-xs text-slate-400 space-y-1">
                      <div>📦 <strong>Quantidade:</strong> ${x} unidades</div>
                      <div>💰 <strong>Custo unitário:</strong> ${w(h)}</div>
                      <div>💵 <strong>Valor total:</strong> ${w(m)}</div>
    ...
    `,d.appendChild(l),d.addEventListener("click",b=>{b.target===d&&d.remove()}),l.querySelector("#close-inventory-modal").addEventListener("click",()=>{d.remove()}),document.addEventListener("keydown",function b(v){v.key==="Escape"&&(d.remove(),document.removeEventListener("keydown",b))}),document.body.appendChild(d)}catch(e){console.error("Erro ao carregar detalhes do inventário:",e)}}async function ce(t,e){try{const a=_.currentUser;if(!a)return;const s=await E(a.uid);if(!s)return;const i=await y.collection("inventory").doc(s).get();if(!i.exists)return;const o=i.data()[t]?.[e];if(!o)return;const d=document.getElementById("equipment-details-modal");d&&d.remove();const l=o.quantity||0,n=o.cost||0,c=n*.05*l,p=n*l,b=o.specs||{},v=document.createElement("div");v.id="equipment-details-modal",v.className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4";const x=document.createElement("div");x.className="bg-bg border border-bg-ring/70 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col",x.innerHTML=`
      <div class="flex items-center justify-between p-6 border-b border-bg-ring/50">
        <div>
      ...
      <div class="flex-1 overflow-auto p-6">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div class="bg-slate-800/30 border border-slate-700/30 rounded-lg p-4">
            ...
            <div class="space-y-2 text-sm">
              ...
              <div class="flex justify-between border-t border-slate-600 pt-2">
                ...
                <span class="text-green-400 font-semibold">${w(p)}</span>
              </div>
              <div class="flex justify-between">
          ...
          <div class="bg-slate-800/30 border border-slate-700/30 rounded-lg p-4">
            ...
            <div class="space-y-2 text-sm">
              ${Object.entries(b).map(([u,m])=>{if(typeof m=="object"||u==="components"||u==="total_cost")return"";const g=u.replace(/_/g," ").replace(/\b\w/g,$=>$.toUpperCase());let f=m;return u.includes("cost")||u.includes("price")?f=w(m):u.includes("weight")?f=`${m} tons`:u.includes("speed")?f=`${m} km/h`:u.includes("armor")||u.includes("thickness")?f=`${m}mm`:(u.includes("caliber")||u.includes("gun"))&&(f=`${m}mm`),`
                  <div class="flex justify-between">
              ...
              ${o.approvedDate?`
                <div class="flex justify-between border-t border-slate-600 pt-2">
                  ...
                  <span class="text-slate-200">${new Date(o.approvedDate).toLocaleDateString("pt-BR")}</span>
                </div>
              `:""}
              ${o.approvedBy?`
                <div class="flex justify-between">
                  <span class="text-slate-400">Aprovado por:</span>
                  <span class="text-slate-200">${o.approvedBy}</span>
                </div>
              `:""}
            </div>
          </div>
    ...
    `,v.appendChild(x),v.addEventListener("click",u=>{u.target===v&&v.remove()}),x.querySelector("#close-equipment-modal").addEventListener("click",()=>{v.remove()});const h=x.querySelector("#view-equipment-sheet");h&&o.sheetImageUrl&&h.addEventListener("click",()=>{De(e,o.sheetImageUrl)}),document.addEventListener("keydown",function u(m){m.key==="Escape"&&(v.remove(),document.removeEventListener("keydown",u))}),document.body.appendChild(v)}catch(a){console.error("Erro ao carregar detalhes do equipamento:",a)}}function De(t,e){const a=document.getElementById("equipment-sheet-modal");a&&a.remove();const s=document.createElement("div");s.id="equipment-sheet-modal",s.className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4";const i=document.createElement("div");i.className="bg-bg border border-bg-ring/70 rounded-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col",i.innerHTML=`
  ...
  `,s.appendChild(i),s.addEventListener("click",r=>{r.target===s&&s.remove()}),i.querySelector("#close-sheet-modal").addEventListener("click",()=>{s.remove()}),i.querySelector("#open-sheet-new-tab").addEventListener("click",()=>{window.open(e,"_blank")}),document.addEventListener("keydown",function r(o){o.key==="Escape"&&(s.remove(),document.removeEventListener("keydown",r))}),document.body.appendChild(s)}async function ue(){try{const t=document.getElementById("naval-content-container");if(!t)return;t.innerHTML='<div class="flex items-center justify-center py-8"><div class="text-slate-400">🔄 Carregando sistema naval...</div></div>';const e=_.currentUser;if(!e)return;const a=await E(e.uid);if(!a)return;const s=new ne,i=window.currentCountry;if(!i)throw new Error("Dados do país não encontrados. Recarregue a página.");const[r,o]=await Promise.all([s.getCurrentShipyardLevel(a),y.collection("paises").get().then(d=>{if(d.empty)return 0;let l=0,n=0;return d.forEach(c=>{const p=parseFloat(c.data().PIB);isNaN(p)||(l+=p,n++)}),n>0?l/n:0})]);t.innerHTML=Se(s,r,i,a,o)}catch(t){console.error("Erro ao carregar sistema naval:",t);const e=document.getElementById("naval-content-container");e&&(e.innerHTML=`<div class="bg-red-900/50 border border-red-800/50 rounded-xl p-6 text-center"><div class="text-6xl mb-4">❌</div><h3 class="text-xl font-semibold text-red-200 mb-2">Erro</h3><p class="text-red-400">Erro ao carregar sistema naval: ${t.message}</p></div>`)}}function Se(t,e,a,s,i){const r=O(a),o=t.getLevelInfo(e,a,i),d=t.calculateMaintenanceCost(e,r),l=t.canUpgrade(e,a,i,r),n=[];for(let c=1;c<=3;c++)e+c<=t.maxLevel&&n.push(t.getLevelInfo(e+c,a,i));return`
    <div class="space-y-6">
            ...
            <div class="bg-slate-800/30 border border-slate-700/30 rounded-lg p-4">
              ...
              <div class="space-y-2 text-sm">
                ...
                <div class="flex justify-between">
                  ...
                  <span class="text-slate-200 text-xs">${o.description}</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-slate-400">Bônus paralelo:</span>
                  <span class="text-green-400">+${o.parallelBonus}%</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-slate-400">Redução tempo:</span>
                  <span class="text-blue-400">-${o.timeReduction}%</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-slate-400">Projetos simultâneos:</span>
                  <span class="text-purple-400">${o.maxProjects}</span>
                </div>
                <div class="flex justify-between border-t border-slate-600 pt-2">
                ...
                <div class="flex justify-between">
                  ...
                  <span class="text-red-400">${(o.maintenancePercent*100).toFixed(2)}%</span>
                </div>
              </div>
            </div>
              ...
              <div class="bg-emerald-900/20 border border-emerald-500/30 rounded-lg p-4">
                ...
                <div class="space-y-3">
                  <div class="flex justify-between text-sm">
                    ...
                    <span class="text-emerald-300 font-semibold">${w(o.upgradeCost)}</span>
                  </div>
          ...
          <div class="p-4 bg-slate-800/30 rounded-lg border border-slate-700/30">
            ...
            <div class="space-y-2 text-sm">
              ...
              <div class="flex justify-between border-t border-slate-600 pt-2">
                ...
                <span class="text-emerald-400 font-semibold">${w(K(a)*(1+o.parallelBonus/100))}</span>
              </div>
            </div>
          </div>
        </div>
  ...
  `}function je(t,e){return[{name:"Corveta",baseTime:8,baseParallel:12},{name:"Destroyer",baseTime:18,baseParallel:4},{name:"Cruzador",baseTime:30,baseParallel:2}].map(s=>{const i=t.calculateProductionBonus(e),r=Math.ceil(s.baseTime*(1-i.timeReduction)),o=Math.ceil(s.baseParallel*i.parallelMultiplier),d=Math.ceil(s.baseTime/3),l=Math.ceil(r/3);return`
      <div class="bg-slate-800/30 border border-slate-700/30 rounded-lg p-4">
        ...
        <div class="space-y-2 text-sm">
          ...
          <div class="flex justify-between">
            ...
            <span class="text-blue-400">${r} meses (${l} turnos)</span>
          </div>
          <div class="flex justify-between">
...
📈 Novos bônus: +${o.levelInfo.parallelBonus}% paralelo, -${o.levelInfo.timeReduction}% tempo`),setTimeout(()=>{ue()},1500)):(s.textContent="❌ Erro",s.classList.add("bg-red-600"),alert("Erro ao fazer upgrade: "+o.error),setTimeout(()=>{s.textContent=i,s.classList.remove("bg-red-600"),s.disabled=!1},3e3))}catch(e){console.error("Erro ao fazer upgrade do estaleiro:",e),alert("Erro ao fazer upgrade: "+e.message),event.target&&(event.target.textContent="❌ Erro",event.target.classList.add("bg-red-600"),setTimeout(()=>{event.target.textContent="⬆️ Fazer Upgrade",event.target.classList.remove("bg-red-600"),event.target.disabled=!1},3e3))}};window.showEquipmentDetails=ce;async function Pe(){try{await new Promise(i=>{_.onAuthStateChanged(i)});const t=_.currentUser;if(!t){document.getElementById("dashboard-content").innerHTML=`
      ...
      `;return}window.currentCountry=s,document.getElementById("dashboard-content").innerHTML=ke(s),Ie()}catch(t){console.error("Erro ao carregar dashboard:",t),document.getElementById("dashboard-content").innerHTML=`
      <div class="min-h-screen flex items-center justify-center bg-slate-950">
        <div class="text-center">
    ...
    `}}window.updateBudgetDisplay=function(t){document.getElementById("budget-display").textContent=t};window.updateDistributionDisplay=function(t){const e=document.getElementById("vehicles-slider"),a=document.getElementById("aircraft-slider"),s=document.getElementById("naval-slider");let i=parseInt(e.value),r=parseInt(a.value),o=parseInt(s.value);if(t&&i+r+o>100){if(t==="vehicles"){const h=r+o;h>0&&(r=Math.max(10,Math.floor(r*(100-i)/h)),o=Math.max(10,100-i-r))}else if(t==="aircraft"){const h=i+o;h>0&&(i=Math.max(10,Math.floor(i*(100-r)/h)),o=Math.max(10,100-r-i))}else if(t==="naval"){const h=i+r;h>0&&(i=Math.max(10,Math.floor(i*(100-o)/h)),r=Math.max(10,100-o-i))}e.value=i,a.value=r,s.value=o}document.getElementById("vehicles-display").textContent=i,document.getElementById("aircraft-display").textContent=r,document.getElementById("naval-display").textContent=o;const d=i+r+o,l=document.getElementById("total-distribution-display");l.textContent=d+"%",d===100?l.className="text-lg font-bold text-emerald-400":d>100?l.className="text-lg font-bold text-red-400":l.className="text-lg font-bold text-yellow-400";const n=document.getElementById("military-budget-slider"),c=parseFloat(n.value)/100,v=O(window.currentCountry)*c*.85;document.getElementById("vehicles-amount").textContent=w(v*i/100),document.getElementById("aircraft-amount").textContent=w(v*r/100),document.getElementById("naval-amount").textContent=w(v*o/100)};window.saveMilitaryBudget=async function(t){try{const e=parseFloat(document.getElementById("military-budget-slider").value),a=_.currentUser;if(!a)return;const s=await E(a.uid);if(!s)return;const{db:i}=await ie(async()=>{const{db:d}=await import("./firebase-DhoRyF0N.js").then(l=>l.o);return{db:d}},[]);await i.collection("paises").doc(s).update({MilitaryBudgetPercent:e});const r=t.target,o=r.textContent;r.textContent="✓ Salvo!",r.classList.add("bg-green-600"),setTimeout(()=>{r.textContent=o,r.classList.remove("bg-green-600")},2e3),setTimeout(()=>window.location.reload(),1e3)}catch(e){console.error("Erro ao salvar orçamento militar:",e),alert("Erro ao salvar orçamento militar. Tente novamente.")}};window.saveMilitaryDistribution=async function(t){try{const e=parseInt(document.getElementById("vehicles-slider").value),a=parseInt(document.getElementById("aircraft-slider").value),s=parseInt(document.getElementById("naval-slider").value),i=e+a+s;if(i!==100){alert(`A soma das distribuições deve ser exatamente 100%! Atual: ${i}%`);return}const r=_.currentUser;if(!r)return;const o=await E(r.uid);if(!o)return;const{db:d}=await ie(async()=>{const{db:c}=await import("./firebase-DhoRyF0N.js").then(p=>p.o);return{db:c}},[]);await d.collection("paises").doc(o).update({MilitaryDistributionVehicles:e,MilitaryDistributionAircraft:a,MilitaryDistributionNaval:s});const l=t.target,n=l.textContent;l.textContent="✓ Salvo!",l.classList.add("bg-green-600"),setTimeout(()=>{l.textContent=n,l.classList.remove("bg-green-600")},2e3),setTimeout(()=>window.location.reload(),1e3)}catch(e){console.error("Erro ao salvar distribuição militar:",e),alert("Erro ao salvar distribuição militar. Tente novamente.")}};let q=null;async function Fe(){const t=document.getElementById("marketplace-container");if(t)try{const e=_.currentUser;if(!e){t.innerHTML='<div class="text-center py-8 text-slate-400">Faça login para acessar o mercado internacional</div>';return}const a=await E(e.uid);if(!a){t.innerHTML='<div class="text-center py-8 text-slate-400">Você precisa estar associado a um país</div>';return}q||(q=new Ce),t.innerHTML=`
      <div class="space-y-6">
        <!-- Header -->
    ...
    `;const s=document.getElementById("marketplace-search")?.value||"",i=document.getElementById("marketplace-sort")?.value||"date",r=document.getElementById("marketplace-type")?.value||"all",o=parseFloat(document.getElementById("price-min")?.value)||null,d=parseFloat(document.getElementById("price-max")?.value)||null,l=parseInt(document.getElementById("quantity-min")?.value)||null,n=parseInt(document.getElementById("quantity-max")?.value)||null,c=document.getElementById("country-filter")?.value||null,p=parseInt(document.getElementById("time-filter")?.value)||null,b={category:t,type:r,searchTerm:s,current_country_id:e,orderBy:Oe(i),orderDirection:Ne(i),limit:50,priceMin:o,priceMax:d,quantityMin:l,quantityMax:n,countryFilter:c,timeFilter:p};let v=[],x={success:!0,offers:[]};if(t==="favorites"){const m=he();if(m.length===0)v=[],x={success:!0,offers:[],totalCount:0};else{const g={...b,category:"all",limit:1e3};x=await q.getOffers(g),x.success&&(v=x.offers.filter(f=>m.includes(f.id)))}}else x=await q.getOffers(b),v=x.offers||[];if(!x.success)throw new Error(x.error);if(v.length===0){const m=await Y(e);a.innerHTML=`
          ...
          ${m.hasEmbargoes?`
            <div class="bg-red-500/10 border border-red-500/30 rounded-lg p-4 mb-6 mx-auto max-w-md">
              ...
              <p class="text-sm text-red-300">
                ${m.totalEmbargoes} país(es) aplicaram embargos contra você,
                limitando ${m.blockedCategories.length>0?"algumas categorias":"todas as trocas"}.
              </p>
      ...
      `;return}const h=await Y(e);let u="";h.hasEmbargoes&&(u+=`
        <div class="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4 mb-6">
          ...
          <p class="text-sm text-yellow-300">
            ...
            ${h.totalEmbargoes} país(es) aplicaram restrições comerciais.
          </p>
      ...
      <div class="mt-8 border-t border-bg-ring/50 pt-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div class="text-sm text-slate-400">
            Mostrando ${v.length} de ${x.totalCount||v.length} ofertas
          </div>
    ...
    `}}function Oe(t){switch(t){case"price-low":case"price-high":return"price_per_unit";case"quantity":return"quantity";case"popularity":return"views";case"expires-soon":return"expires_at";case"date":default:return"created_at"}}function Ne(t){switch(t){case"price-low":case"expires-soon":return"asc";case"price-high":case"quantity":case"popularity":case"date":default:return"desc"}}async function Y(t){try{if(!t)return{hasEmbargoes:!1,totalEmbargoes:0,blockedCategories:[]};const e=await y.collection("marketplace_embargoes").where("target_country_id","==",t).where("status","==","active").get(),a=[];if(e.forEach(r=>{a.push(r.data())}),a.length===0)return{hasEmbargoes:!1,totalEmbargoes:0,blockedCategories:[]};const s=new Set;let i=!1;return a.forEach(r=>{r.type==="full"?(i=!0,s.add("resources"),s.add("vehicles"),s.add("naval")):r.type==="partial"&&r.categories_blocked&&r.categories_blocked.forEach(o=>{s.add(o)})}),{hasEmbargoes:!0,totalEmbargoes:a.length,blockedCategories:Array.from(s),hasFullEmbargo:i,embargoes:a}}catch(e){return console.error("Erro ao verificar embargos:",e),{hasEmbargoes:!1,totalEmbargoes:0,blockedCategories:[]}}}function me(t){const e=t.expires_at?.toDate?t.expires_at.toDate():new Date(t.expires_at),a=t.quantity*t.price_per_unit,s=Math.max(0,Math.floor((e-new Date)/(1440*60*1e3))),i=t.type==="sell"?{label:"Venda",color:"text-green-400 bg-green-400/20",icon:"💰"}:{label:"Compra",color:"text-blue-400 bg-blue-400/20",icon:"🛒"},r={resources:"🏭",vehicles:"🚗",naval:"🚢"};return`
    <div class="bg-bg-soft border border-bg-ring/70 rounded-xl p-4 hover:border-brand-400/30 transition-colors cursor-pointer" onclick="openOfferDetails('${t.id}')">
      ...
      <div class="flex items-start justify-between mb-3">
        <div class="flex items-center gap-2">
          <span class="text-lg">${r[t.category]}</span>
  ...
  `}async function Re(t){try{q&&q.incrementOfferViews(t);const e=_.currentUser;if(!e){alert("Você precisa estar logado para visualizar detalhes");return}const a=await E(e.uid);if(!a){alert("Você precisa estar associado a um país");return}const i=(await te()).find(g=>g.id===a);if(!i){alert("Dados do país não encontrados");return}const r=document.querySelectorAll('[onclick*="openOfferDetails"]');let o=null;try{const g=await q.getOffers({limit:1e3});g.success&&g.offers&&(o=g.offers.find(f=>f.id===t))}catch(g){console.error("Error finding offer:",g)}if(!o){alert("Oferta não encontrada");return}const d=document.getElementById("offer-details-modal");d&&d.remove();const l=document.createElement("div");l.id="offer-details-modal",l.className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4";const n=o.player_id===e.uid||o.country_id===a,c=!n&&o.type==="sell"&&o.status==="active",p=!n&&o.type==="buy"&&o.status==="active",b=c||p,v=O(i),x=o.quantity*o.price_per_unit,h=v>=x,u=o.expires_at?.toDate?o.expires_at.toDate():new Date(o.expires_at),m=Math.max(0,Math.ceil((u-new Date)/(1440*60*1e3)));l.innerHTML=`
        ...
        <div class="p-6 border-b border-bg-ring/50">
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
              <div class="text-2xl">
                ${o.type==="sell"?"🔥":"💰"}
                ${o.category==="resources"?"🏭":o.category==="vehicles"?"🚗":"🚢"}
              </div>
              <div>
                <h2 class="text-xl font-bold text-white">${o.title}</h2>
                <div class="flex items-center space-x-4 text-sm text-slate-400">
                  <span>${o.country_flag} ${o.country_name}</span>
                  <span>${o.type==="sell"?"Vendendo":"Comprando"}</span>
                  <span>${m} dias restantes</span>
                </div>
              </div>
            </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="space-y-6">
              <div class="bg-bg/30 rounded-lg p-4">
                ...
                <div class="space-y-2 text-sm">
                  <div class="flex justify-between">
                    ...
                    <span class="text-white">${o.item_name}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-slate-400">Categoria:</span>
                    <span class="text-white">${o.category==="resources"?"Recursos":o.category==="vehicles"?"Veículos":"Naval"}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-slate-400">Quantidade:</span>
                    <span class="text-white font-medium">${o.quantity.toLocaleString()} ${o.unit}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-slate-400">Preço por ${o.unit.slice(0,-1)}:</span>
                    <span class="text-white font-medium">${C(o.price_per_unit)}</span>
                  </div>
                  <div class="flex justify-between border-t border-bg-ring pt-2 mt-3">
                    <span class="text-slate-400">Valor Total:</span>
                    <span class="text-brand-300 font-bold text-lg">${C(o.total_value)}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="bg-bg/30 rounded-lg p-4">
                ...
                <p class="text-slate-300 text-sm">${o.description}</p>
              </div>
              `:""}
              ...
              <div class="bg-bg/30 rounded-lg p-4">
                ...
                <div class="space-y-2 text-sm">
                  <div class="flex justify-between">
                    ...
                    <span class="text-white">${o.min_quantity||1} ${o.unit}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-slate-400">Quantidade Máxima:</span>
                    <span class="text-white">${o.max_quantity||o.quantity} ${o.unit}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-slate-400">Tempo de Entrega:</span>
                    <span class="text-white">${o.delivery_time_days||30} dias</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-slate-400">Criado em:</span>
                    <span class="text-white">${new Date(o.created_at?.seconds*1e3||o.created_at).toLocaleDateString("pt-BR")}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="bg-bg/30 rounded-lg p-4">
                ...
                <div class="space-y-2 text-sm">
                  <div class="flex justify-between">
                    ...
                    <span class="text-white">${o.views||0}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-slate-400">Países Interessados:</span>
                    <span class="text-white">${o.interested_countries?.length||0}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="space-y-6">
              ...
              `:b?`
                <div class="bg-bg/30 rounded-lg p-4">
                  <h3 class="text-white font-medium mb-4">
                    ${o.type==="sell"?"💰 Comprar Item":"🔥 Vender Item"}
                  </h3>
                  ...
                  <div class="space-y-4">
                    <div>
                      ...
                      <div class="flex space-x-2">
                        <input type="number" id="transaction-quantity" min="${o.min_quantity||1}" max="${o.max_quantity||o.quantity}" value="${o.min_quantity||1}" class="flex-1 px-3 py-2 bg-bg border border-bg-ring rounded-lg text-white focus:border-brand-400 focus:outline-none">
                        <span class="px-3 py-2 text-slate-400 bg-bg/50 border border-bg-ring rounded-lg">${o.unit}</span>
                      </div>
                      <div class="text-xs text-slate-400 mt-1">
                        Mín: ${o.min_quantity||1} | Máx: ${o.max_quantity||o.quantity}
                      </div>
                    </div>
                    ...
                    <div id="transaction-summary" class="bg-brand-500/10 border border-brand-400/30 rounded-lg p-3">
                      <div class="text-sm space-y-1">
                        <div class="flex justify-between">
                          <span class="text-slate-400">Quantidade:</span>
                          <span class="text-white"><span id="summary-quantity">${o.min_quantity||1}</span> ${o.unit}</span>
                        </div>
                        <div class="flex justify-between">
                          <span class="text-slate-400">Preço unitário:</span>
                          <span class="text-white">${C(o.price_per_unit)}</span>
                        </div>
                        <div class="flex justify-between font-medium border-t border-brand-400/30 pt-1 mt-2">
                          ...
                          <span class="text-brand-300" id="summary-total">${C((o.min_quantity||1)*o.price_per_unit)}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="flex space-x-2">
                      ...
                      <button onclick="processTransaction('${o.id}')" id="process-transaction-btn" class="flex-1 px-4 py-2 bg-brand-500 hover:bg-brand-600 text-black font-medium rounded-lg transition-colors ${o.type==="sell"&&!h?"opacity-50 cursor-not-allowed":""}" ${o.type==="sell"&&!h?"disabled":""}>
                        ${o.type==="sell"?"💰 Comprar":"🔥 Vender"}
                      </button>
                    </div>
                  </div>
                </div>
                ...
                <div class="bg-amber-500/10 border border-amber-400/30 rounded-lg p-4">
                  <div class="flex items-start space-x-2">
                    ...
                    <div>
                      ...
                      <div class="text-sm text-slate-300 mt-1">
                        ${o.status!=="active"?"Esta oferta não está mais ativa.":"Você não pode interagir com esta oferta."}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              ${o.category==="vehicles"||o.category==="naval"?`
              ...
              <div class="bg-purple-500/10 border border-purple-400/30 rounded-lg p-4">
                <div class="flex items-center justify-between mb-3">
                  ...
                  <button onclick="openEquipmentDetails('${o.item_id}', '${o.category}', '${o.country_id}')" class="px-3 py-1 bg-purple-600/20 text-purple-300 text-xs rounded-lg hover:bg-purple-600/30 transition-colors">
                    📋 Ver Ficha Completa
                  </button>
                </div>
                <div id="equipment-specs-${o.id}" class="text-sm text-slate-300 space-y-2">
                  <div class="flex justify-between">
                    <span class="text-slate-400">Tipo:</span>
                    <span class="text-white">${o.category==="vehicles"?"🚗 Veículo Terrestre":"🚢 Embarcação Naval"}</span>
                  </div>
                  <div class="flex justify-between">
              ...
              <div class="bg-bg/30 rounded-lg p-4">
                ...
                <div class="flex items-center space-x-3 mb-3">
                  <div class="text-2xl">${o.country_flag}</div>
                  <div>
                    <div class="text-white font-medium">${o.country_name}</div>
                    <div class="text-sm text-slate-400">${o.type==="sell"?"Vendedor":"Comprador"}</div>
                  </div>
                </div>
                <div class="text-sm text-slate-400">
                  Este país ${o.type==="sell"?"está oferecendo":"está procurando"} ${o.item_name.toLowerCase()}
                  ${o.type==="sell"?"para venda":"para compra"} no mercado internacional.
                </div>
              </div>
              ...
              <div class="bg-bg/30 rounded-lg p-4">
                ...
                <div class="space-y-2 text-sm">
                  <div class="flex justify-between">
                    ...
                    <span class="text-white">${C(o.price_per_unit*(.9+Math.random()*.2))}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-slate-400">Esta Oferta:</span>
                    <span class="${o.price_per_unit>o.price_per_unit*1.1?"text-red-300":o.price_per_unit<o.price_per_unit*.9?"text-green-300":"text-yellow-300"}">
                      ${o.price_per_unit>o.price_per_unit*1.1?"📈 Acima":o.price_per_unit<o.price_per_unit*.9?"📉 Abaixo":"📊 Na Média"}
                    </span>
                  </div>
                </div>
              </div>
    ...
    `,document.body.appendChild(l),Ve(o,i,a)}catch(e){console.error("Erro ao abrir detalhes da oferta:",e),alert("Erro ao carregar detalhes da oferta")}}function U(){const t=document.getElementById("offer-details-modal");t&&t.remove()}function Ve(t,e,a){const s=document.getElementById("offer-details-modal");if(!s)return;const i=s.querySelector("#transaction-quantity");i&&i.addEventListener("input",()=>{He(t,i.value)}),s.addEventListener("click",r=>{r.target===s&&U()}),document.addEventListener("keydown",function r(o){o.key==="Escape"&&(U(),document.removeEventListener("keydown",r))})}function He(t,e){const a=document.getElementById("offer-details-modal");if(!a)return;const s=a.querySelector("#summary-quantity"),i=a.querySelector("#summary-total");if(s&&i){const r=parseInt(e)||1,o=r*t.price_per_unit;s.textContent=r.toLocaleString(),i.textContent=C(o)}}async function Ue(t){try{const e=document.getElementById("offer-details-modal"),a=e.querySelector("#transaction-quantity"),s=e.querySelector("#process-transaction-btn"),i=s.textContent;if(!a){alert("Erro: quantidade não especificada");return}const r=parseInt(a.value);if(!r||r<=0){alert("Por favor, especifique uma quantidade válida"),a.focus();return}const o=_.currentUser;if(!o){alert("Você precisa estar logado");return}if(!await E(o.uid)){alert("Você precisa estar associado a um país");return}const n=(await q.getOffers({limit:1e3})).offers?.find(x=>x.id===t);if(!n){alert("Oferta não encontrada");return}if(r<(n.min_quantity||1)){alert(`Quantidade mínima: ${n.min_quantity||1} ${n.unit}`);return}if(r>(n.max_quantity||n.quantity)){alert(`Quantidade máxima: ${n.max_quantity||n.quantity} ${n.unit}`);return}if(r>n.quantity){alert(`Quantidade disponível: ${n.quantity} ${n.unit}`);return}const c=r*n.price_per_unit,b=`
      ...
      Deseja continuar?
    `;if(!confirm(b))return;s.disabled=!0,s.textContent="⏳ Processando...";const v=await q.createTransaction(t,{quantity:r});if(v.success)s.textContent="✅ Sucesso!",s.classList.remove("bg-brand-500","hover:bg-brand-600"),s.classList.add("bg-green-600"),setTimeout(()=>{alert("Transação criada com sucesso! A negociação foi iniciada."),U();const x=document.querySelector(".marketplace-category-btn.active")?.dataset.category||"all",h=_.currentUser;h&&E(h.uid).then(u=>{u&&B(x,u)})},1500);else throw new Error(v.error||"Erro desconhecido ao processar transação")}catch(e){console.error("Erro ao processar transação:",e);const a=document.querySelector("#process-transaction-btn");a&&(a.textContent="❌ Erro",a.classList.remove("bg-brand-500","hover:bg-brand-600"),a.classList.add("bg-red-600"),setTimeout(()=>{a.textContent=offer.type==="sell"?"💰 Comprar":"🔥 Vender",a.classList.remove("bg-red-600"),a.classList.add("bg-brand-500","hover:bg-brand-600"),a.disabled=!1},3e3)),alert("Erro ao processar transação: "+e.message)}}async function ze(t,e,a){try{console.log("Abrindo detalhes do equipamento:",{itemId:t,category:e,countryId:a});const s=await q.getCountryInventory(a);let i=null,r=null;if(Object.keys(s).forEach(l=>{s[l]&&typeof s[l]=="object"&&Object.keys(s[l]).forEach(n=>{const c=s[l][n];c&&typeof c=="object"&&(`${l}_${n}`.toLowerCase().replace(/\s+/g,"_")===t||n.toLowerCase().includes(t.toLowerCase()))&&(i=c,r=l,i.name=n,i.category=l)})}),!i){alert("Equipamento não encontrado no inventário do país vendedor.");return}const o=document.getElementById("equipment-details-modal");o&&o.remove();const d=document.createElement("div");d.id="equipment-details-modal",d.className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[60] flex items-center justify-center p-4",d.innerHTML=`
    ...
    `,document.body.appendChild(r),Ge(s,e),setTimeout(()=>{const o=r.querySelector('input[type="radio"]');o&&o.focus()},100)}catch(t){console.error("Erro ao abrir modal de criação:",t),alert("Erro ao abrir formulário de criação de ofertas")}}function H(){const t=document.getElementById("create-offer-modal");t&&t.remove()}function Ge(t,e){const a=document.getElementById("create-offer-modal");if(!a)return;let s={resources:[],vehicles:[],naval:[]};async function i(){try{const u=a.querySelectorAll('input[name="offer-type"]');let m=null;u.forEach(f=>{f.checked&&(m=f.value)}),console.log("🔍 Tipo de oferta selecionado:",m),m==="sell"?(console.log("📦 Carregando itens do inventário real para VENDA"),await r()):m==="buy"?(console.log("🛒 Carregando todos os tipos de itens para COMPRA"),o()):(console.log("⚠️ Nenhum tipo selecionado, usando VENDA como padrão"),await r());const g=l.value;g&&d(g)}catch(u){console.error("Erro ao carregar itens disponíveis:",u)}}async function r(){if(q)try{console.log("🔍 Carregando itens do inventário para país:",e);const u=window.currentCountry;if(!u){console.error("❌ Dados do país não encontrados no window.currentCountry");return}const m=ae.calculateCountryConsumption(u),g=se.calculateCountryProduction(u),f={Carvao:Math.round((g.Carvao||0)-(m.Carvao||0)),Combustivel:Math.round((g.Combustivel||0)-(m.Combustivel||0)),Metais:Math.round((g.Metais||0)-(m.Metais||0)),Graos:Math.round((g.Graos||0)-(m.Graos||0))};s.resources=[],Object.entries(f).forEach(([M,k])=>{k>0&&s.resources.push({id:M.toLowerCase(),name:`${M} (Excedente: ${k.toLocaleString()})`,unit:M==="Combustivel"?"barris":"toneladas",available:k})}),console.log("✅ Recursos excedentes carregados para venda:",s.resources);const $=await q.getCountryInventory(e),I=q.getAvailableEquipment($);s.vehicles=I.filter(M=>M.type==="vehicles"),s.naval=I.filter(M=>M.type==="naval")}catch(u){console.error("Erro ao carregar itens do inventário:",u)}}function o(){s={resources:[{id:"steel_high_grade",name:"Aço de Alta Qualidade",unit:"toneladas"},{id:"steel_standard",name:"Aço Padrão",unit:"toneladas"},{id:"oil_crude",name:"Petróleo Bruto",unit:"barris"},{id:"oil_aviation",name:"Petróleo de Aviação",unit:"barris"},{id:"aluminum",name:"Alumínio",unit:"toneladas"},{id:"copper",name:"Cobre",unit:"toneladas"},{id:"rare_metals",name:"Metais Raros",unit:"toneladas"},{id:"coal",name:"Carvão",unit:"toneladas"},{id:"food",name:"Alimentos",unit:"toneladas"}],vehicles:[{id:"mbt_modern",name:"Tanque MBT Moderno",unit:"unidades"},{id:"mbt_standard",name:"Tanque MBT Padrão",unit:"unidades"},{id:"light_tank",name:"Tanque Leve",unit:"unidades"},{id:"heavy_tank",name:"Tanque Pesado",unit:"unidades"},{id:"artillery_howitzer",name:"Artilharia Howitzer",unit:"unidades"},{id:"artillery_rocket",name:"Artilharia de Foguetes",unit:"unidades"},{id:"apc_standard",name:"Transporte Blindado",unit:"unidades"},{id:"ifv_modern",name:"Veículo de Combate",unit:"unidades"}],naval:[{id:"destroyer_standard",name:"Destroyer Padrão",unit:"navios"},{id:"destroyer_fletcher",name:"Destroyer Classe Fletcher",unit:"navios"},{id:"cruiser_heavy",name:"Cruzador Pesado",unit:"navios"},{id:"cruiser_light",name:"Cruzador Leve",unit:"navios"},{id:"submarine_diesel",name:"Submarino Diesel-Elétrico",unit:"submarinos"},{id:"submarine_nuclear",name:"Submarino Nuclear",unit:"submarinos"},{id:"corvette_patrol",name:"Corveta de Patrulha",unit:"navios"},{id:"frigate_escort",name:"Fragata de Escolta",unit:"navios"}]}}function d(u){if(console.log(`🎯 Populando itens para categoria: ${u}`),console.log("📋 availableItems atual:",s),n.innerHTML='<option value="">Selecione um item</option>',u&&s[u]){n.disabled=!1;const m=s[u];if(console.log(`📦 Itens encontrados para ${u}:`,m),m.length===0){const g=document.createElement("option");g.value="",g.textContent="Nenhum item disponível para venda",g.disabled=!0,n.appendChild(g),n.disabled=!0}else m.forEach(g=>{console.log(`➕ Adicionando item: ${g.name} (${g.id})`);const f=document.createElement("option");f.value=g.id,f.textContent=g.name,f.dataset.unit=g.unit,f.dataset.available=g.available_quantity||g.available||"",f.dataset.cost=g.unit_cost||"",f.dataset.maintenance=g.maintenance_cost||"",n.appendChild(f)})}else console.log(`❌ Categoria ${u} não encontrada ou sem itens`),n.disabled=!0;x()}i();const l=a.querySelector("#offer-category"),n=a.querySelector("#offer-item"),c=a.querySelector("#offer-unit");l.addEventListener("change",()=>{const u=l.value;d(u)}),a.querySelectorAll('input[name="offer-type"]').forEach(u=>{u.addEventListener("change",()=>{i()})}),n.addEventListener("change",()=>{const u=n.querySelector("option:checked");u&&u.dataset.unit&&(c.value=u.dataset.unit),x()}),a.querySelectorAll("input, select, textarea").forEach(u=>{u.addEventListener("input",x),u.addEventListener("change",x)});const v=a.querySelector("#create-offer-form");v.addEventListener("submit",async u=>{u.preventDefault();const m=a.querySelector("#submit-offer-btn"),g=m.textContent;try{m.disabled=!0,m.textContent="⏳ Criando...";const f=new FormData(v),$={type:f.get("offer-type"),category:f.get("category"),item_id:f.get("item_id"),item_name:n.querySelector("option:checked")?.textContent||"",title:f.get("title"),description:f.get("description"),quantity:parseInt(f.get("quantity")),unit:f.get("unit"),price_per_unit:parseFloat(f.get("price_per_unit")),min_quantity:parseInt(f.get("min_quantity"))||1,delivery_time_days:parseInt(f.get("delivery_time_days"))||30,duration_days:parseInt(f.get("duration_days"))};if(!$.type||!$.category||!$.item_id||!$.title||!$.quantity||!$.price_per_unit)throw new Error("Preencha todos os campos obrigatórios");const I=await q.createOffer($);if(I.success)m.textContent="✅ Criado!",m.classList.remove("bg-brand-500","hover:bg-brand-600"),m.classList.add("bg-green-600"),setTimeout(()=>{alert("Oferta criada com sucesso!"),H();const M=document.querySelector(".marketplace-category-btn.active")?.dataset.category||"all",k=_.currentUser;k&&E(k.uid).then(j=>{j&&B(M,j)})},1e3);else throw new Error(I.error||"Erro desconhecido ao criar oferta")}catch(f){console.error("Erro ao criar oferta:",f),m.textContent="❌ Erro",m.classList.remove("bg-brand-500","hover:bg-brand-600"),m.classList.add("bg-red-600"),alert("Erro ao criar oferta: "+f.message),setTimeout(()=>{m.textContent=g,m.classList.remove("bg-red-600"),m.classList.add("bg-brand-500","hover:bg-brand-600"),m.disabled=!1},3e3)}});function x(){const u=a.querySelector("#offer-summary"),m=a.querySelector("#offer-summary-content"),g=a.querySelector("#budget-warning"),f=a.querySelector("#item-info"),$=a.querySelector("#item-info-content"),I=new FormData(v),M=I.get("offer-type"),k=parseInt(I.get("quantity"))||0,j=parseFloat(I.get("price_per_unit"))||0,J=k*j,P=n.querySelector("option:checked");if(P&&P.value){const W=P.textContent,F=P.dataset.available,A=P.dataset.cost,D=P.dataset.maintenance;let T=`<div><strong>Item selecionado:</strong> ${W}</div>`;if(M==="sell"&&F&&(T+=`<div class="text-green-400"><strong>Disponível para venda:</strong> ${parseInt(F).toLocaleString()} unidades</div>`),A&&parseFloat(A)>0&&(T+=`<div><strong>Custo de produção:</strong> ${C(parseFloat(A))} por unidade</div>`),D&&parseFloat(D)>0&&(T+=`<div><strong>Custo de manutenção:</strong> ${C(parseFloat(D))} por unidade/mês</div>`),M==="sell"&&F&&k>0){const V=parseInt(F);k>V&&(T+=`<div class="text-red-400 mt-2"><strong>⚠️ Quantidade excede o disponível!</strong><br>Máximo vendível: ${V.toLocaleString()}</div>`)}$.innerHTML=T,f.classList.remove("hidden")}else f.classList.add("hidden");if(k>0&&j>0){u.classList.remove("hidden");const W=P?.textContent||"Item",F=I.get("unit")||"unidades";let A=`
        <div><strong>Tipo:</strong> ${M==="sell"?"🔥 Venda":"💰 Compra"}</div>
        <div><strong>Item:</strong> ${W}</div>
        <div><strong>Quantidade:</strong> ${k.toLocaleString()} ${F}</div>
        <div><strong>Preço por ${F.slice(0,-1)}:</strong> ${C(j)}</div>
        <div class="font-medium text-brand-300"><strong>Valor Total:</strong> ${C(J)}</div>
      `;if(M==="sell"&&cost&&parseFloat(cost)>0){const D=parseFloat(cost),T=j-D,V=T*k,oe=T>0?"text-green-400":"text-red-400";A+=`<div class="${oe}"><strong>Lucro por unidade:</strong> ${C(T)}</div>`,A+=`<div class="${oe}"><strong>Lucro total:</strong> ${C(V)}</div>`}if(m.innerHTML=A,M==="buy"){const D=O(t);J>D?(g.classList.remove("hidden"),g.querySelector(".text-sm").innerHTML=`
          ...
          `):g.classList.add("hidden")}else g.classList.add("hidden")}else u.classList.add("hidden"),g.classList.add("hidden")}a.addEventListener("click",u=>{u.target===a&&H()}),document.addEventListener("keydown",function u(m){m.key==="Escape"&&(H(),document.removeEventListener("keydown",u))}),document.body.appendChild(a);const h=a.querySelector('input[name="offer-type"][value="sell"]');h&&(h.checked=!0,console.log('✅ Radio "Vender" selecionado por padrão')),console.log("🚀 Carregando itens iniciais para modo VENDA"),i()}window.closeCreateOfferModal=H;function Je(t,e){let a;return function(...i){const r=()=>{clearTimeout(a),t(...i)};clearTimeout(a),a=setTimeout(r,e)}}async function We(){try{const t=_.currentUser;if(!t)return;const e=await E(t.uid);if(!e)return;const a=document.createElement("div");a.className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4",a.id="notifications-modal",a.innerHTML=`
  ...
  `}function Q(t){let e=L;t==="unread"?e=L.filter(a=>!a.read):t==="embargo"?e=L.filter(a=>a.type&&a.type.includes("embargo")):t==="transaction"&&(e=L.filter(a=>a.type&&a.type.includes("transaction"))),pe(e)}async function Ze(t){try{await y.collection("notifications").doc(t).update({read:!0,read_at:new Date});const e=L.find(s=>s.id===t);e&&(e.read=!0);const a=document.querySelector(".notification-filter-btn.active")?.dataset.filter||"all";Q(a),await G()}catch(e){console.error("Erro ao marcar notificação como lida:",e)}}async function et(){try{const t=_.currentUser;if(!t||!await E(t.uid))return;const a=y.batch();L.forEach(i=>{if(!i.read){const r=y.collection("notifications").doc(i.id);a.update(r,{read:!0,read_at:new Date}),i.read=!0}}),await a.commit();const s=document.querySelector(".notification-filter-btn.active")?.dataset.filter||"all";Q(s),await G()}catch(t){console.error("Erro ao marcar todas as notificações como lidas:",t)}}async function tt(t){try{if(!confirm("Tem certeza que deseja excluir esta notificação?"))return;await y.collection("notifications").doc(t).delete(),L=L.filter(a=>a.id!==t);const e=document.querySelector(".notification-filter-btn.active")?.dataset.filter||"all";Q(e),await G()}catch(e){console.error("Erro ao excluir notificação:",e)}}async function G(){try{const t=_.currentUser;if(!t)return;const e=await E(t.uid);if(!e)return;const s=(await y.collection("notifications").where("target_country_id","==",e).where("read","==",!1).get()).size,i=document.getElementById("notifications-count-badge");i&&(s>0?(i.textContent=s>99?"99+":s.toString(),i.classList.remove("hidden")):i.classList.add("hidden"))}catch(t){console.error("Erro ao atualizar contador de notificações:",t)}}function at(t){const a=new Date-t,s=Math.floor(a/(1e3*60)),i=Math.floor(a/(1e3*60*60)),r=Math.floor(a/(1e3*60*60*24));return s<1?"Agora há pouco":s<60?`${s} min atrás`:i<24?`${i}h atrás`:r<7?`${r} dias atrás`:t.toLocaleDateString("pt-BR")}window.closeNotificationsModal=Z;window.markNotificationAsRead=Ze;window.markAllNotificationsAsRead=et;window.deleteNotification=tt;async function st(){try{const t=_.currentUser;if(!t)return;const e=await E(t.uid);if(!e)return;const a=document.createElement("div");a.className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4",a.id="embargoes-modal",a.innerHTML=`
      <div class="bg-bg-soft border border-bg-ring rounded-xl max-w-4xl w-full max-h-[80vh] overflow-y-auto">
        <div class="p-6 border-b border-bg-ring/50">
          <div class="flex items-center justify-between">
    ...
    `,document.body.appendChild(a),await ot(e)}catch(t){console.error("Erro ao abrir modal de embargos:",t)}}async function ot(t){try{const e=await y.collection("marketplace_embargoes").where("embargo_country_id","==",t).where("status","==","active").orderBy("created_at","desc").get(),a=await y.collection("marketplace_embargoes").where("target_country_id","==",t).where("status","==","active").orderBy("created_at","desc").get(),s=[],i=[];e.forEach(d=>{s.push({id:d.id,...d.data()})}),a.forEach(d=>{i.push({id:d.id,...d.data()})});const r=document.getElementById("applied-embargoes-list");r&&(s.length===0?r.innerHTML=`
          <div class="text-center py-8 text-slate-400">
            <div class="text-4xl mb-2">🕊️</div>
            <p>Nenhum embargo aplicado</p>
          </div>
        `:r.innerHTML=s.map(d=>re(d,"applied")).join(""));const o=document.getElementById("received-embargoes-list");o&&(i.length===0?o.innerHTML=`
          <div class="text-center py-8 text-slate-400">
            <div class="text-4xl mb-2">✅</div>
            <p>Nenhum embargo recebido</p>
          </div>
        `:o.innerHTML=i.map(d=>re(d,"received")).join(""))}catch(e){console.error("Erro ao carregar dados de embargos:",e)}}function re(t,e){const a=t.expires_at&&new Date(t.expires_at.toDate?t.expires_at.toDate():t.expires_at)<new Date(Date.now()+6048e5),s=e==="applied"?t.target_country_name:t.embargo_country_name,i=e==="applied"?"🎯":"⚠️",r=t.type==="full"?{label:"Total",color:"text-red-400 bg-red-400/20"}:{label:"Parcial",color:"text-yellow-400 bg-yellow-400/20"},o=t.created_at?.toDate?t.created_at.toDate():new Date(t.created_at),d=Math.floor((new Date-o)/(1440*60*1e3));return`
      ...
      <div class="flex items-start justify-between mb-3">
        <div class="flex items-center gap-2">
          ...
          <div>
            ...
            <span class="px-2 py-1 rounded text-xs font-medium ${r.color}">
              ${r.label}
            </span>
          </div>
        </div>
  ...
  `}async function rt(){try{const t=_.currentUser;if(!t)return;const e=await E(t.uid);if(!e)return;const a=await y.collection("paises").get(),s=[];a.forEach(r=>{const o=r.data();r.id!==e&&s.push({id:r.id,name:o.Pais,flag:o.Flag||"🏳️"})});const i=document.createElement("div");i.className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4",i.id="create-embargo-modal",i.innerHTML=`
