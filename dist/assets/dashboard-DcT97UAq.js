...
import{_ as oe}from"./preload-helper-f85Crcwt.js";/* empty css             */import{c as _,j as E,k as y,h as te}from"./firebase-DSEPx490.js";import{E as _e}from"./economicCalculations-CnCK_Mqi.js";import{C as Ee}from"./consumerGoodsCalculator-BJu5SIvV.js";import{R as ie}from"./resourceConsumptionCalculator-XVfvtOaE.js";import{R as ne}from"./resourceProductionCalculator-DyW2EJ2N.js";import{S as le}from"./shipyardSystem-CYVZPvns.js";import"https://www.gstatic.com/firebasejs/10.12.2/firebase-app-compat.js";import"https://www.gstatic.com/firebasejs/10.12.2/firebase-auth-compat.js";import"https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore-compat.js";import"https://www.gstatic.com/firebasejs/10.12.2/firebase-storage-compat.js";class Ce{constructor(){this.collections={offers:"marketplace_offers",transactions:"marketplace_transactions",embargoes:"marketplace_embargoes",orders:"marketplace_orders"}}getOfferSchema(){return{id:"string",type:"string",category:"string",title:"string",description:"string",item_id:"string",item_name:"string",quantity:"number",unit:"string",price_per_unit:"number",total_value:"number",country_id:"string",country_name:"string",country_flag:"string",player_id:"string",status:"string",created_at:"timestamp",updated_at:"timestamp",expires_at:"timestamp",min_quantity:"number",max_quantity:"number",delivery_time_days:"number",views:"number",interested_countries:"array",tech_level_required:"number",diplomatic_status_required:"string"}}getTransactionSchema(){return{id:"string",offer_id:"string",seller_country_id:"string",seller_country_name:"string",seller_player_id:"string",buyer_country_id:"string",buyer_country_name:"string",buyer_player_id:"string",item_id:"string",item_name:"string",quantity:"number",unit:"string",price_per_unit:"number",total_value:"number",status:"string",created_at:"timestamp",confirmed_at:"timestamp",completed_at:"timestamp",delivery_deadline:"timestamp",status_history:"array",delivery_time_days:"number",delivery_status:"string",negotiated_price:"boolean",original_price_per_unit:"number",discount_percent:"number"}}getEmbargoSchema(){return{id:"string",embargo_country_id:"string",embargo_country_name:"string",target_country_id:"string",target_country_name:"string",type:"string",categories_blocked:"array",reason:"string",created_at:"timestamp",expires_at:"timestamp",status:"string",created_by_player_id:"string",notifications_sent:"boolean"}}getOrderSchema(){return{id:"string",country_id:"string",country_name:"string",player_id:"string",type:"string",item_id:"string",item_name:"string",category:"string",quantity:"number",unit:"string",max_price_per_unit:"number",min_price_per_unit:"number",is_recurring:"boolean",recurrence_type:"string",recurrence_interval:"number",max_executions:"number",executions_count:"number",status:"string",created_at:"timestamp",last_execution:"timestamp",next_execution:"timestamp",auto_execute:"boolean",require_confirmation:"boolean",execution_history:"array"}}async createOffer(e){try{const a=_.currentUser;if(!a)throw new Error("Usuário não autenticado");const s=await E(a.uid);if(!s)throw new Error("Jogador não associado a um país");const i=await this.validateOfferData(e,s,a.uid),o={...i,country_id:s,player_id:a.uid,created_at:new Date,updated_at:new Date,expires_at:new Date(Date.now()+i.duration_days*24*60*60*1e3),status:"active",views:0,interested_countries:[]},r=await y.collection(this.collections.offers).add(o);return{success:!0,offerId:r.id,offer:{id:r.id,...o}}}catch(a){return console.error("Erro ao criar oferta:",a),{success:!1,error:a.message}}}async validateOfferData(e,a,s){if(!e.type||!["sell","buy"].includes(e.type))throw new Error("Tipo de oferta inválido");if(!e.category||!["resources","vehicles","naval"].includes(e.category))throw new Error("Categoria inválida");if(!e.title||e.title.trim().length<3)throw new Error("Título deve ter pelo menos 3 caracteres");if(!e.quantity||e.quantity<=0)throw new Error("Quantidade deve ser maior que zero");if(!e.price_per_unit||e.price_per_unit<=0)throw new Error("Preço deve ser maior que zero");const i=await y.collection("paises").doc(a).get();if(!i.exists)throw new Error("País não encontrado");const o=i.data();e.type==="sell"?(await this.validateSellOfferAvailability(e,a),await this.validateSellOffer(e,o)):await this.validateBuyOffer(e,o);const r=e.quantity*e.price_per_unit;return{type:e.type,category:e.category,title:e.title.trim(),description:(e.description||"").trim(),item_id:e.item_id,item_name:e.item_name,quantity:e.quantity,unit:e.unit,price_per_unit:e.price_per_unit,total_value:r,country_name:o.Pais,country_flag:o.Flag||"🏳️",delivery_time_days:e.delivery_time_days||30,min_quantity:e.min_quantity||1,max_quantity:e.max_quantity||e.quantity,tech_level_required:e.tech_level_required||0,duration_days:e.duration_days||30}}async validateSellOffer(e,a){if(e.category!=="resources"){if(e.category==="vehicles"||e.category==="naval"){const s=await y.collection("inventory").doc(a.id||e.country_id).get();s.exists&&s.data()}}}async validateBuyOffer(e,a){const s=this.calculateCountryBudget(a),i=e.quantity*e.price_per_unit;if(s<i)throw new Error(`Orçamento insuficiente. Necessário: $${i.toLocaleString()}, Disponível: $${s.toLocaleString()}`)}calculateCountryBudget(e){const a=parseFloat(e.PIB)||0,s=(parseFloat(e.Burocracia)||0)/100,i=(parseFloat(e.Estabilidade)||0)/100;return a*.25*s*(i*1.5)}async getOffers(e={}){try{const a=y.collection(this.collections.offers);let s=[],i;try{console.info("🔍 Buscando ofertas usando filtragem client-side (sem índices)"),i=await a.get()}catch(l){console.warn("Falha ao buscar todos os documentos, tentando listagem:",l);try{const c=(await a.listDocuments()).slice(0,100).map(b=>b.get()),p=await Promise.all(c);i={docs:p.filter(b=>b.exists),size:p.filter(b=>b.exists).length,empty:p.filter(b=>b.exists).length===0,forEach:function(b){this.docs.forEach(b)}}}catch(n){console.warn("Listagem de documentos falhou, retornando resultados vazios:",n),i={docs:[],size:0,empty:!0,forEach:function(){}}}}i.forEach(l=>{const n=l.data();if(n.status!=="active")return;const c=n.expires_at?.toDate?n.expires_at.toDate():new Date(n.expires_at);c&&c<=new Date||s.push({id:l.id,...n})});let o=s;if(e.category&&e.category!=="all"&&(o=o.filter(l=>l.category===e.category)),e.type&&e.type!=="all"&&(o=o.filter(l=>l.type===e.type)),e.seller_id&&(o=o.filter(l=>l.seller_id===e.seller_id)),e.buyer_id&&(o=o.filter(l=>l.buyer_id===e.buyer_id)),e.searchTerm){const l=e.searchTerm.toLowerCase();o=o.filter(n=>n.title&&n.title.toLowerCase().includes(l)||n.description&&n.description.toLowerCase().includes(l)||n.item_name&&n.item_name.toLowerCase().includes(l)||n.country_name&&n.country_name.toLowerCase().includes(l))}e.priceMin!==null&&e.priceMin!==void 0&&(o=o.filter(l=>l.price_per_unit>=e.priceMin)),e.priceMax!==null&&e.priceMax!==void 0&&(o=o.filter(l=>l.price_per_unit<=e.priceMax)),e.quantityMin!==null&&e.quantityMin!==void 0&&(o=o.filter(l=>l.quantity>=e.quantityMin)),e.quantityMax!==null&&e.quantityMax!==void 0&&(o=o.filter(l=>l.quantity<=e.quantityMax));const r=e.orderBy||"created_at",d=e.orderDirection||"desc";if(o.sort((l,n)=>{let c=l[r],p=n[r];return r.includes("_at")&&c&&p&&(c=c.toDate?c.toDate():new Date(c),p=p.toDate?p.toDate():new Date(p)),typeof c=="string"&&!isNaN(c)&&(c=parseFloat(c)),typeof p=="string"&&!isNaN(p)&&(p=parseFloat(p)),d==="desc"?p>c?1:p<c?-1:0:c>p?1:c<p?-1:0}),e.countryFilter&&(o=o.filter(l=>l.country_id===e.countryFilter)),e.timeFilter!==null&&e.timeFilter!==void 0){const l=new Date,n=new Date(l.getTime()+e.timeFilter*24*60*60*1e3);o=o.filter(c=>(c.expires_at?.toDate?c.expires_at.toDate():new Date(c.expires_at))<=n)}return o=await this.filterEmbargoedOffers(o,e.current_country_id),e.limit&&o.length>e.limit&&(o=o.slice(0,e.limit)),console.info(`✅ Ofertas filtradas: ${o.length} encontradas usando filtragem client-side`),{success:!0,offers:o,total:o.length,totalCount:o.length}}catch(a){return console.error("Erro ao buscar ofertas:",a),{success:!1,error:a.message,offers:[]}}}async filterEmbargoedOffers(e,a){if(!a)return e;try{const s=await y.collection(this.collections.embargoes).where("status","==","active").where("target_country_id","==",a).get(),i=[];return s.forEach(o=>{i.push(o.data())}),i.length===0?e:e.filter(o=>{const r=i.find(d=>d.embargo_country_id===o.country_id);return r?r.type==="full"?!1:r.type==="partial"&&r.categories_blocked?!r.categories_blocked.includes(o.category):!0:!0})}catch(s){return console.error("Erro ao verificar embargos:",s),e}}async incrementOfferViews(e){try{const a=y.collection(this.collections.offers).doc(e),s=await a.get();if(s.exists){const i=s.data().views||0;await a.update({views:i+1,updated_at:new Date})}}catch(a){console.error("Erro ao incrementar visualizações:",a)}}async createTransaction(e,a){try{const s=_.currentUser;if(!s)throw new Error("Usuário não autenticado");const i=await E(s.uid);if(!i)throw new Error("Jogador não associado a um país");const o=await y.collection(this.collections.offers).doc(e).get();if(!o.exists)throw new Error("Oferta não encontrada");const r=o.data();if(r.status!=="active")throw new Error("Oferta não está ativa");const d=a.quantity||r.quantity;if(d>r.quantity)throw new Error("Quantidade solicitada excede disponível");if(d<r.min_quantity)throw new Error(`Quantidade mínima: ${r.min_quantity}`);if(d>r.max_quantity)throw new Error(`Quantidade máxima: ${r.max_quantity}`);const n=(await y.collection("paises").doc(i).get()).data(),c=d*r.price_per_unit;if(this.calculateCountryBudget(n)<c)throw new Error("Orçamento insuficiente");const b={offer_id:e,seller_country_id:r.country_id,seller_country_name:r.country_name,seller_player_id:r.player_id,buyer_country_id:i,buyer_country_name:n.Pais,buyer_player_id:s.uid,item_id:r.item_id,item_name:r.item_name,quantity:d,unit:r.unit,price_per_unit:r.price_per_unit,total_value:c,status:"pending",created_at:new Date,delivery_deadline:new Date(Date.now()+r.delivery_time_days*24*60*60*1e3),delivery_time_days:r.delivery_time_days,delivery_status:"pending",status_history:[{status:"pending",timestamp:new Date,note:"Transação criada"}],negotiated_price:!1,original_price_per_unit:r.price_per_unit,discount_percent:0},v=await y.collection(this.collections.transactions).add(b);return await y.collection(this.collections.offers).doc(e).update({quantity:r.quantity-d,updated_at:new Date,status:r.quantity-d===0?"completed":"active"}),{success:!0,transactionId:v.id,transaction:{id:v.id,...b}}}catch(s){return console.error("Erro ao criar transação:",s),{success:!1,error:s.message}}}async applyEmbargo(e){try{const a=_.currentUser;if(!a)throw new Error("Usuário não autenticado");const s=await E(a.uid);if(!s)throw new Error("Jogador não associado a um país");const o=(await y.collection("paises").doc(s).get()).data(),r=await y.collection("paises").doc(e.target_country_id).get();if(!r.exists)throw new Error("País alvo não encontrado");const d=r.data();if(!(await y.collection(this.collections.embargoes).where("embargo_country_id","==",s).where("target_country_id","==",e.target_country_id).where("status","==","active").get()).empty)throw new Error("Já existe um embargo ativo contra este país");const n={embargo_country_id:s,embargo_country_name:o.Pais,target_country_id:e.target_country_id,target_country_name:d.Pais,type:e.type||"full",categories_blocked:e.categories_blocked||[],reason:e.reason||"Motivos diplomáticos",created_at:new Date,expires_at:e.expires_at||null,status:"active",created_by_player_id:a.uid,notifications_sent:!1},c=await y.collection(this.collections.embargoes).add(n);return await this.sendEmbargoNotification(n,c.id),{success:!0,embargoId:c.id,embargo:{id:c.id,...n}}}catch(a){return console.error("Erro ao aplicar embargo:",a),{success:!1,error:a.message}}}async sendEmbargoNotification(e,a){try{const s={type:"embargo_applied",embargo_id:a,target_country_id:e.target_country_id,target_country_name:e.target_country_name,embargo_country_id:e.embargo_country_id,embargo_country_name:e.embargo_country_name,embargo_type:e.type,categories_blocked:e.categories_blocked||[],reason:e.reason,created_at:new Date,read:!1,expires_at:e.expires_at,title:`🚫 Embargo Aplicado por ${e.embargo_country_name}`,message:this.getEmbargoNotificationMessage(e),priority:"high"};await y.collection("notifications").add(s),await y.collection(this.collections.embargoes).doc(a).update({notifications_sent:!0}),console.log(`Notificação de embargo enviada para ${e.target_country_name}`)}catch(s){console.error("Erro ao enviar notificação de embargo:",s)}}getEmbargoNotificationMessage(e){const a=e.type==="full"?"total":"parcial";let s=`${e.embargo_country_name} aplicou um embargo ${a} contra seu país.`;if(e.type==="partial"&&e.categories_blocked&&e.categories_blocked.length>0){const i={resources:"Recursos",vehicles:"Veículos",naval:"Naval"},o=e.categories_blocked.map(r=>i[r]||r);s+=` Categorias bloqueadas: ${o.join(", ")}.`}else e.type==="full"&&(s+=" Todas as trocas comerciais estão bloqueadas.");if(e.reason&&e.reason!=="Motivos diplomáticos"&&(s+=` Motivo: ${e.reason}`),e.expires_at){const i=new Date(e.expires_at),o=Math.ceil((i-new Date)/(1440*60*1e3));s+=` O embargo expira em ${o} dias.`}else s+=" O embargo é por tempo indefinido.";return s}async getEmbargoNotifications(e,a=10){try{const s=await y.collection("notifications").where("target_country_id","==",e).where("type","==","embargo_applied").orderBy("created_at","desc").limit(a).get(),i=[];return s.forEach(o=>{i.push({id:o.id,...o.data()})}),{success:!0,notifications:i}}catch(s){return console.error("Erro ao buscar notificações:",s),{success:!1,error:s.message,notifications:[]}}}async markNotificationAsRead(e){try{return await y.collection("notifications").doc(e).update({read:!0,read_at:new Date}),{success:!0}}catch(a){return console.error("Erro ao marcar notificação como lida:",a),{success:!1,error:a.message}}}async getCountryInventory(e){try{const a=await y.collection("inventory").doc(e).get();return a.exists?a.data():{}}catch(a){return console.error("Erro ao buscar inventário:",a),{}}}async getCountryData(e){try{const a=await y.collection("paises").doc(e).get();return a.exists?{id:e,...a.data()}:null}catch(a){return console.error("Erro ao buscar dados do país:",a),null}}calculateAvailableResources(e){console.log("🔍 Debug: Dados do país recebidos:",e),console.log("📊 Usando recursos REAIS do dashboard (não calculados)"),console.log("🔍 Recursos reais no país:"),console.log("  - Carvao:",e.Carvao),console.log("  - Combustivel:",e.Combustivel),console.log("  - Metais:",e.Metais),console.log("  - Graos:",e.Graos);const a={Carvao:Math.max(0,Math.floor((parseFloat(e.Carvao)||0)*.5)),Combustivel:Math.max(0,Math.floor((parseFloat(e.Combustivel)||0)*.5)),Metais:Math.max(0,Math.floor((parseFloat(e.Metais)||0)*.5)),Graos:Math.max(0,Math.floor((parseFloat(e.Graos)||0)*.5))};return console.log("📦 Recursos REAIS disponíveis para venda (50% do estoque):",a),a}getAvailableEquipment(e){const a=[];return Object.keys(e).forEach(s=>{typeof e[s]=="object"&&e[s]!==null&&Object.keys(e[s]).forEach(i=>{const o=e[s][i];if(o&&typeof o=="object"&&o.quantity>0){const r=Math.floor(o.quantity*.5);r>0&&a.push({id:`${s}_${i}`.toLowerCase().replace(/\s+/g,"_"),name:i,category:s,available_quantity:r,total_quantity:o.quantity,unit_cost:o.cost||0,maintenance_cost:(o.cost||0)*.05||0,type:this.getEquipmentType(s)})}})}),a}getEquipmentType(e){return["Couraçados","Cruzadores","Destróieres","Fragatas","Corvetas","Submarinos","Porta-aviões","Patrulhas","Auxiliares","Naval - Outros"].includes(e)?"naval":"vehicles"}async validateSellOfferAvailability(e,a){const s=await this.getCountryData(a);if(!s)throw new Error("Dados do país não encontrados");return e.category==="resources"?this.validateResourceAvailability(e,s):this.validateEquipmentAvailability(e,a)}validateResourceAvailability(e,a){const s=this.calculateAvailableResources(a),o={steel_high_grade:"Metais",steel_standard:"Metais",oil_crude:"Combustivel",oil_aviation:"Combustivel",aluminum:"Metais",copper:"Metais",rare_metals:"Metais",coal:"Carvao",food:"Graos"}[e.item_id];if(!o)throw new Error("Tipo de recurso não reconhecido");const r=s[o]||0;if(e.quantity>r)throw new Error(`Quantidade insuficiente. Disponível para venda: ${r.toLocaleString()} unidades de ${o}`);return{valid:!0,available:r,resourceType:o}}async validateEquipmentAvailability(e,a){const s=await this.getCountryInventory(a),o=this.getAvailableEquipment(s).find(r=>r.id===e.item_id);if(!o)throw new Error(`Equipamento "${e.item_name}" não encontrado no inventário ou indisponível para venda`);if(e.quantity>o.available_quantity)throw new Error(`Quantidade insuficiente. Disponível para venda: ${o.available_quantity} de ${o.total_quantity} unidades totais`);return{valid:!0,equipment:o,availableQuantity:o.available_quantity,totalQuantity:o.total_quantity}}async createTestOffers(){try{console.log("🧪 Criando ofertas de teste...");const e=[{type:"sell",category:"resources",title:"Aço de Alta Qualidade",description:"Aço especializado para construção naval e industrial",item_id:"steel_high_grade",item_name:"Aço de Alta Qualidade",quantity:5e3,unit:"toneladas",price_per_unit:850,total_value:5e3*850,country_id:"test_country_1",country_name:"Estados Unidos",country_flag:"🇺🇸",player_id:"test_player_1",status:"active",created_at:new Date,updated_at:new Date,expires_at:new Date(Date.now()+10080*60*1e3),delivery_time_days:30,min_quantity:100,max_quantity:5e3,views:23,interested_countries:[]},{type:"buy",category:"vehicles",title:"Tanques MBT Modernos",description:"Procurando tanques de batalha principais para modernização das forças armadas",item_id:"mbt_modern",item_name:"Tanque MBT Moderno",quantity:50,unit:"unidades",price_per_unit:25e5,total_value:50*25e5,country_id:"test_country_2",country_name:"Brasil",country_flag:"🇧🇷",player_id:"test_player_2",status:"active",created_at:new Date(Date.now()-1440*60*1e3),updated_at:new Date(Date.now()-1440*60*1e3),expires_at:new Date(Date.now()+336*60*60*1e3),delivery_time_days:45,min_quantity:5,max_quantity:50,views:45,interested_countries:[]},{type:"sell",category:"naval",title:"Destroyers Classe Fletcher",description:"Destroyers modernizados, prontos para serviço imediato",item_id:"destroyer_fletcher",item_name:"Destroyer Classe Fletcher",quantity:3,unit:"navios",price_per_unit:18e7,total_value:3*18e7,country_id:"test_country_3",country_name:"Reino Unido",country_flag:"🇬🇧",player_id:"test_player_3",status:"active",created_at:new Date(Date.now()-4320*60*1e3),updated_at:new Date(Date.now()-4320*60*1e3),expires_at:new Date(Date.now()+504*60*60*1e3),delivery_time_days:60,min_quantity:1,max_quantity:3,views:67,interested_countries:[]},{type:"sell",category:"resources",title:"Petróleo Refinado",description:"Combustível de alta octanagem para aviação militar",item_id:"oil_aviation",item_name:"Petróleo de Aviação",quantity:1e4,unit:"barris",price_per_unit:120,total_value:1e4*120,country_id:"test_country_4",country_name:"Arábia Saudita",country_flag:"🇸🇦",player_id:"test_player_4",status:"active",created_at:new Date(Date.now()-2880*60*1e3),updated_at:new Date(Date.now()-2880*60*1e3),expires_at:new Date(Date.now()+14400*60*1e3),delivery_time_days:15,min_quantity:500,max_quantity:1e4,views:89,interested_countries:[]},{type:"buy",category:"naval",title:"Submarinos Diesel-Elétricos",description:"Necessitamos de submarinos para patrulha costeira",item_id:"submarine_diesel",item_name:"Submarino Diesel-Elétrico",quantity:2,unit:"submarinos",price_per_unit:45e6,total_value:2*45e6,country_id:"test_country_5",country_name:"Argentina",country_flag:"🇦🇷",player_id:"test_player_5",status:"active",created_at:new Date(Date.now()-5760*60*1e3),updated_at:new Date(Date.now()-5760*60*1e3),expires_at:new Date(Date.now()+720*60*60*1e3),delivery_time_days:90,min_quantity:1,max_quantity:2,views:12,interested_countries:[]}];for(const a of e)await y.collection(this.collections.offers).add(a);return console.log(`✅ ${e.length} ofertas de teste criadas com sucesso!`),{success:!0,count:e.length}}catch(e){return console.error("❌ Erro ao criar ofertas de teste:",e),{success:!1,error:e.message}}}async clearTestOffers(){try{console.log("🗑️ Removendo ofertas de teste...");const e=await y.collection(this.collections.offers).where("player_id",">=","test_player_").where("player_id","<","test_player_z").get(),a=y.batch();return e.docs.forEach(s=>{a.delete(s.ref)}),await a.commit(),console.log(`✅ ${e.docs.length} ofertas de teste removidas!`),{success:!0,count:e.docs.length}}catch(e){return console.error("❌ Erro ao remover ofertas de teste:",e),{success:!1,error:e.message}}}}function $e(t){return t<=20?{label:"Anarquia",tone:"bg-rose-500/15 text-rose-300 border-rose-400/30"}:t<=49?{label:"Instável",tone:"bg-amber-500/15 text-amber-300 border-amber-400/30"}:t<=74?{label:"Neutro",tone:"bg-sky-500/15 text-sky-300 border-sky-400/30"}:{label:"Tranquilo",tone:"bg-emerald-500/15 text-emerald-300 border-emerald-400/30"}}function S(t){if(!t||isNaN(t))return"0";const e=parseFloat(t);return e>=1e9?(e/1e9).toFixed(1)+"B":e>=1e6?(e/1e6).toFixed(1)+"M":e>=1e3?(e/1e3).toFixed(0)+"K":Math.round(e).toLocaleString("pt-BR")}function C(t){if(!t)return"US$ 0";const e=parseFloat(t);return e>=1e9?"US$ "+(e/1e9).toFixed(1)+"B":e>=1e6?"US$ "+(e/1e6).toFixed(1)+"M":e>=1e3?"US$ "+(e/1e3).toFixed(1)+"K":"US$ "+e.toLocaleString()}function w(t){const e=parseFloat(t)||0;return e===0?"US$ 0":e>=1e9?`US$ ${(e/1e9).toFixed(1)}bi`:e>=1e6?`US$ ${(e/1e6).toFixed(1)}mi`:e>=1e3?`US$ ${(e/1e3).toFixed(1)}mil`:`US$ ${Math.round(e)}`}function Me(t){const e=parseFloat(t.WarPower)||0;return Math.round(e)}function O(t){const e=parseFloat(t.PIB)||0,a=(parseFloat(t.Burocracia)||0)/100,s=(parseFloat(t.Estabilidade)||0)/100;return e*.25*a*(s*1.5)}function de(t){const e=O(t),a=(parseFloat(t.MilitaryBudgetPercent)||30)/100;return e*a}function ce(t){const e=(parseFloat(t.MilitaryDistributionVehicles)||40)/100,a=(parseFloat(t.MilitaryDistributionAircraft)||30)/100,s=(parseFloat(t.MilitaryDistributionNaval)||30)/100;return{vehicles:e,aircraft:a,naval:s,maintenancePercent:.15}}function qe(t){const s=(parseFloat(t.MilitaryBudgetPercent)||30)-30;let i=0,o=0;return s>0&&(i=Math.floor(s/2)*-1,o=s*-.5),{stabilityPenalty:i,economicPenalty:o,isOverBudget:s>0}}function K(t){const e=de(t),a=ce(t),s=e*(1-a.maintenancePercent),i=(parseFloat(t.Tecnologia)||0)/100,o=(parseFloat(t.IndustrialEfficiency)||30)/100,r=(parseFloat(t.Marinha)||0)/100,d=(parseFloat(t.Urbanizacao)||0)/100;return s*a.naval*i*o*r*d}function ke(t){_e.computeEnergyDemandGW(t);const e=Ee.calculateConsumerGoods(t),a=ie.calculateCountryConsumption(t),s=ne.calculateCountryProduction(t),i={Carvao:Math.round((s.Carvao||0)-(a.Carvao||0)),Combustivel:Math.round((s.Combustivel||0)-(a.Combustivel||0)),Metais:Math.round((s.Metais||0)-(a.Metais||0)),Graos:Math.round((s.Graos||0)-(a.Graos||0)),Energia:Math.round((s.Energia||0)-(a.Energia||0))},o=$e(parseFloat(t.Estabilidade)||0),r=Me(t),d=(parseFloat(t.PIB)||0)/(parseFloat(t.Populacao)||1),l=O(t),n=de(t),c=ce(t),p=n*c.vehicles,b=n*c.aircraft,v=n*c.naval,x=qe(t),h=window.getFlagHTML||function(u,m="h-full w-full"){return'<span class="text-slate-400 text-xs">—</span>'};return`
  ...
  `}async function Te(t){try{const e=_.currentUser;if(!e)return;const a=await E(e.uid);if(!a)return;const s=await y.collection("inventory").doc(a).get();if(!s.exists)return;const o=s.data()[t];if(!o)return;const r=document.getElementById("inventory-details-modal");r&&r.remove();const d=document.createElement("div");d.id="inventory-details-modal",d.className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4";const l=document.createElement("div");l.className="bg-bg border border-bg-ring/70 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col";const n=Object.entries(o),c=n.reduce((b,[v,x])=>b+(x.quantity||0),0),p=n.reduce((b,[v,x])=>{const h=x.quantity||0,m=(x.cost||0)*.05;return b+m*h},0);l.innerHTML=`
    ...
    `,d.appendChild(l),d.addEventListener("click",b=>{b.target===d&&d.remove()}),l.querySelector("#close-inventory-modal").addEventListener("click",()=>{d.remove()}),document.addEventListener("keydown",function b(v){v.key==="Escape"&&(d.remove(),document.removeEventListener("keydown",b))}),document.body.appendChild(d)}catch(e){console.error("Erro ao carregar detalhes do inventário:",e)}}async function ue(t,e){try{const a=_.currentUser;if(!a)return;const s=await E(a.uid);if(!s)return;const i=await y.collection("inventory").doc(s).get();if(!i.exists)return;const r=i.data()[t]?.[e];if(!r)return;const d=document.getElementById("equipment-details-modal");d&&d.remove();const l=r.quantity||0,n=r.cost||0,c=n*.05*l,p=n*l,b=r.specs||{},v=document.createElement("div");v.id="equipment-details-modal",v.className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4";const x=document.createElement("div");x.className="bg-bg border border-bg-ring/70 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col",x.innerHTML=`
      <div class="flex items-center justify-between p-6 border-b border-bg-ring/50">
        <div>
  ...
  `,s.appendChild(i),s.addEventListener("click",o=>{o.target===s&&s.remove()}),i.querySelector("#close-sheet-modal").addEventListener("click",()=>{s.remove()}),i.querySelector("#open-sheet-new-tab").addEventListener("click",()=>{window.open(e,"_blank")}),document.addEventListener("keydown",function o(r){r.key==="Escape"&&(s.remove(),document.removeEventListener("keydown",o))}),document.body.appendChild(s)}async function me(){try{const t=document.getElementById("naval-content-container");if(!t)return;t.innerHTML='<div class="flex items-center justify-center py-8"><div class="text-slate-400">🔄 Carregando sistema naval...</div></div>';const e=_.currentUser;if(!e)return;const a=await E(e.uid);if(!a)return;const s=new le,i=window.currentCountry;if(!i)throw new Error("Dados do país não encontrados. Recarregue a página.");const[o,r]=await Promise.all([s.getCurrentShipyardLevel(a),y.collection("paises").get().then(d=>{if(d.empty)return 0;let l=0,n=0;return d.forEach(c=>{const p=parseFloat(c.data().PIB);isNaN(p)||(l+=p,n++)}),n>0?l/n:0})]);t.innerHTML=Se(s,o,i,a,r)}catch(t){console.error("Erro ao carregar sistema naval:",t);const e=document.getElementById("naval-content-container");e&&(e.innerHTML=`<div class="bg-red-900/50 border border-red-800/50 rounded-xl p-6 text-center"><div class="text-6xl mb-4">❌</div><h3 class="text-xl font-semibold text-red-200 mb-2">Erro</h3><p class="text-red-400">Erro ao carregar sistema naval: ${t.message}</p></div>`)}}function Se(t,e,a,s,i){const o=O(a),r=t.getLevelInfo(e,a,i),d=t.calculateMaintenanceCost(e,o),l=t.canUpgrade(e,a,i,o),n=[];for(let c=1;c<=3;c++)e+c<=t.maxLevel&&n.push(t.getLevelInfo(e+c,a,i));return`
    <div class="space-y-6">
...
📈 Novos bônus: +${r.levelInfo.parallelBonus}% paralelo, -${r.levelInfo.timeReduction}% tempo`),setTimeout(()=>{me()},1500)):(s.textContent="❌ Erro",s.classList.add("bg-red-600"),alert("Erro ao fazer upgrade: "+r.error),setTimeout(()=>{s.textContent=i,s.classList.remove("bg-red-600"),s.disabled=!1},3e3))}catch(e){console.error("Erro ao fazer upgrade do estaleiro:",e),alert("Erro ao fazer upgrade: "+e.message),event.target&&(event.target.textContent="❌ Erro",event.target.classList.add("bg-red-600"),setTimeout(()=>{event.target.textContent="⬆️ Fazer Upgrade",event.target.classList.remove("bg-red-600"),event.target.disabled=!1},3e3))}};window.showEquipmentDetails=ue;async function se(){try{await new Promise(i=>{_.onAuthStateChanged(i)});const t=_.currentUser;if(!t){document.getElementById("dashboard-content").innerHTML=`
    ...
    `,a.innerHTML=u,gt(v),pt(e,t,b)}catch(s){console.error("Erro ao carregar ofertas:",s),a.innerHTML=`
  ...
  `}async function Ne(t){try{q&&q.incrementOfferViews(t);const e=_.currentUser;if(!e){alert("Você precisa estar logado para visualizar detalhes");return}const a=await E(e.uid);if(!a){alert("Você precisa estar associado a um país");return}const i=(await te()).find(g=>g.id===a);if(!i){alert("Dados do país não encontrados");return}const o=document.querySelectorAll('[onclick*="openOfferDetails"]');let r=null;try{const g=await q.getOffers({limit:1e3});g.success&&g.offers&&(r=g.offers.find(f=>f.id===t))}catch(g){console.error("Error finding offer:",g)}if(!r){alert("Oferta não encontrada");return}const d=document.getElementById("offer-details-modal");d&&d.remove();const l=document.createElement("div");l.id="offer-details-modal",l.className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4";const n=r.player_id===e.uid||r.country_id===a,c=!n&&r.type==="sell"&&r.status==="active",p=!n&&r.type==="buy"&&r.status==="active",b=c||p,v=O(i),x=r.quantity*r.price_per_unit,h=v>=x,u=r.expires_at?.toDate?r.expires_at.toDate():new Date(r.expires_at),m=Math.max(0,Math.ceil((u-new Date)/(1440*60*1e3)));l.innerHTML=`
                ...
                <div class="bg-bg/30 rounded-lg p-4">
                  ...
                  <div class="space-y-4">
                    ...
                    <div class="flex space-x-2">
                      ...
                      <button onclick="processTransaction('${r.id}')" id="process-transaction-btn" class="flex-1 px-4 py-2 bg-brand-500 hover:bg-brand-600 text-black font-medium rounded-lg transition-colors ${r.type==="sell"&&!h?"opacity-50 cursor-not-allowed":""}" ${r.type==="sell"&&!h?"disabled":""}>
    ...
    `,document.body.appendChild(o),Qe(s,e),setTimeout(()=>{const r=o.querySelector('input[type="radio"]');r&&r.focus()},100)}catch(t){console.error("Erro ao abrir modal de criação:",t),alert("Erro ao abrir formulário de criação de ofertas")}}function H(){const t=document.getElementById("create-offer-modal");t&&t.remove()}function Qe(t,e){const a=document.getElementById("create-offer-modal");if(!a)return;let s={resources:[],vehicles:[],naval:[]};async function i(){try{const u=a.querySelectorAll('input[name="offer-type"]');let m=null;u.forEach(f=>{f.checked&&(m=f.value)}),console.log("🔍 Tipo de oferta selecionado:",m),m==="sell"?(console.log("📦 Carregando itens do inventário real para VENDA"),await o()):m==="buy"?(console.log("🛒 Carregando todos os tipos de itens para COMPRA"),r()):(console.log("⚠️ Nenhum tipo selecionado, usando VENDA como padrão"),await o());const g=l.value;g&&d(g)}catch(u){console.error("Erro ao carregar itens disponíveis:",u)}}async function o(){if(q)try{console.log("🔍 Carregando itens do inventário para país:",e);const u=window.currentCountry;if(!u){console.error("❌ Dados do país não encontrados no window.currentCountry");return}const m=ie.calculateCountryConsumption(u),g=ne.calculateCountryProduction(u),f={Carvao:Math.round((g.Carvao||0)-(m.Carvao||0)),Combustivel:Math.round((g.Combustivel||0)-(m.Combustivel||0)),Metais:Math.round((g.Metais||0)-(m.Metais||0)),Graos:Math.round((g.Graos||0)-(m.Graos||0))};s.resources=[],Object.entries(f).forEach(([M,k])=>{k>0&&s.resources.push({id:M.toLowerCase(),name:`${M} (Excedente: ${k.toLocaleString()})`,unit:M==="Combustivel"?"barris":"toneladas",available:k})}),console.log("✅ Recursos excedentes carregados para venda:",s.resources);const $=await q.getCountryInventory(e),I=q.getAvailableEquipment($);s.vehicles=I.filter(M=>M.type==="vehicles"),s.naval=I.filter(M=>M.type==="naval")}catch(u){console.error("Erro ao carregar itens do inventário:",u)}}function r(){s={resources:[{id:"steel_high_grade",name:"Aço de Alta Qualidade",unit:"toneladas"},{id:"steel_standard",name:"Aço Padrão",unit:"toneladas"},{id:"oil_crude",name:"Petróleo Bruto",unit:"barris"},{id:"oil_aviation",name:"Petróleo de Aviação",unit:"barris"},{id:"aluminum",name:"Alumínio",unit:"toneladas"},{id:"copper",name:"Cobre",unit:"toneladas"},{id:"rare_metals",name:"Metais Raros",unit:"toneladas"},{id:"coal",name:"Carvão",unit:"toneladas"},{id:"food",name:"Alimentos",unit:"toneladas"}],vehicles:[{id:"mbt_modern",name:"Tanque MBT Moderno",unit:"unidades"},{id:"mbt_standard",name:"Tanque MBT Padrão",unit:"unidades"},{id:"light_tank",name:"Tanque Leve",unit:"unidades"},{id:"heavy_tank",name:"Tanque Pesado",unit:"unidades"},{id:"artillery_howitzer",name:"Artilharia Howitzer",unit:"unidades"},{id:"artillery_rocket",name:"Artilharia de Foguetes",unit:"unidades"},{id:"apc_standard",name:"Transporte Blindado",unit:"unidades"},{id:"ifv_modern",name:"Veículo de Combate",unit:"unidades"}],naval:[{id:"destroyer_standard",name:"Destroyer Padrão",unit:"navios"},{id:"destroyer_fletcher",name:"Destroyer Classe Fletcher",unit:"navios"},{id:"cruiser_heavy",name:"Cruzador Pesado",unit:"navios"},{id:"cruiser_light",name:"Cruzador Leve",unit:"navios"},{id:"submarine_diesel",name:"Submarino Diesel-Elétrico",unit:"submarinos"},{id:"submarine_nuclear",name:"Submarino Nuclear",unit:"submarinos"},{id:"corvette_patrol",name:"Corveta de Patrulha",unit:"navios"},{id:"frigate_escort",name:"Fragata de Escolta",unit:"navios"}]}}function d(u){if(console.log(`🎯 Populando itens para categoria: ${u}`),console.log("📋 availableItems atual:",s),n.innerHTML='<option value="">Selecione um item</option>',u&&s[u]){n.disabled=!1;const m=s[u];if(console.log(`📦 Itens encontrados para ${u}:`,m),m.length===0){const g=document.createElement("option");g.value="",g.textContent="Nenhum item disponível para venda",g.disabled=!0,n.appendChild(g),n.disabled=!0}else m.forEach(g=>{console.log(`➕ Adicionando item: ${g.name} (${g.id})`);const f=document.createElement("option");f.value=g.id,f.textContent=g.name,f.dataset.unit=g.unit,f.dataset.available=g.available_quantity||g.available||"",f.dataset.cost=g.unit_cost||"",f.dataset.maintenance=g.maintenance_cost||"",n.appendChild(f)})}else console.log(`❌ Categoria ${u} não encontrada ou sem itens`),n.disabled=!0;x()}i();const l=a.querySelector("#offer-category"),n=a.querySelector("#offer-item"),c=a.querySelector("#offer-unit");l.addEventListener("change",()=>{const u=l.value;d(u)}),a.querySelectorAll('input[name="offer-type"]').forEach(u=>{u.addEventListener("change",()=>{i()})}),n.addEventListener("change",()=>{const u=n.querySelector("option:checked");u&&u.dataset.unit&&(c.value=u.dataset.unit),x()}),a.querySelectorAll("input, select, textarea").forEach(u=>{u.addEventListener("input",x),u.addEventListener("change",x)});const v=a.querySelector("#create-offer-form");v.addEventListener("submit",async u=>{u.preventDefault();const m=a.querySelector("#submit-offer-btn"),g=m.textContent;try{m.disabled=!0,m.textContent="⏳ Criando...";const f=new FormData(v),$={type:f.get("offer-type"),category:f.get("category"),item_id:f.get("item_id"),item_name:n.querySelector("option:checked")?.textContent||"",title:f.get("title"),description:f.get("description"),quantity:parseInt(f.get("quantity")),unit:f.get("unit"),price_per_unit:parseFloat(f.get("price_per_unit")),min_quantity:parseInt(f.get("min_quantity"))||1,delivery_time_days:parseInt(f.get("delivery_time_days"))||30,duration_days:parseInt(f.get("duration_days"))};if(!$.type||!$.category||!$.item_id||!$.title||!$.quantity||!$.price_per_unit)throw new Error("Preencha todos os campos obrigatórios");const I=await q.createOffer($);if(I.success)m.textContent="✅ Criado!",m.classList.remove("bg-brand-500","hover:bg-brand-600"),m.classList.add("bg-green-600"),setTimeout(()=>{alert("Oferta criada com sucesso!"),H();const M=document.querySelector(".marketplace-category-btn.active")?.dataset.category||"all",k=_.currentUser;k&&E(k.uid).then(j=>{j&&B(M,j)})},1e3);else throw new Error(I.error||"Erro desconhecido ao criar oferta")}catch(f){console.error("Erro ao criar oferta:",f),m.textContent="❌ Erro",m.classList.remove("bg-brand-500","hover:bg-brand-600"),m.classList.add("bg-red-600"),alert("Erro ao criar oferta: "+f.message),setTimeout(()=>{m.textContent=g,m.classList.remove("bg-red-600"),m.classList.add("bg-brand-500","hover:bg-brand-600"),m.disabled=!1},3e3)}});function x(){const u=a.querySelector("#offer-summary"),m=a.querySelector("#offer-summary-content"),g=a.querySelector("#budget-warning"),f=a.querySelector("#item-info"),$=a.querySelector("#item-info-content"),I=new FormData(v),M=I.get("offer-type"),k=parseInt(I.get("quantity"))||0,j=parseFloat(I.get("price_per_unit"))||0,J=k*j,P=n.querySelector("option:checked");if(P&&P.value){const W=P.textContent,F=P.dataset.available,A=P.dataset.cost,D=P.dataset.maintenance;let T=`<div><strong>Item selecionado:</strong> ${W}</div>`;if(M==="sell"&&F&&(T+=`<div class="text-green-400"><strong>Disponível para venda:</strong> ${parseInt(F).toLocaleString()} unidades</div>`),A&&parseFloat(A)>0&&(T+=`<div><strong>Custo de produção:</strong> ${C(parseFloat(A))} por unidade</div>`),D&&parseFloat(D)>0&&(T+=`<div><strong>Custo de manutenção:</strong> ${C(parseFloat(D))} por unidade/mês</div>`),M==="sell"&&F&&k>0){const V=parseInt(F);k>V&&(T+=`<div class="text-red-400 mt-2"><strong>⚠️ Quantidade excede o disponível!</strong><br>Máximo vendível: ${V.toLocaleString()}</div>`)}$.innerHTML=T,f.classList.remove("hidden")}else f.classList.add("hidden");if(k>0&&j>0){u.classList.remove("hidden");const W=P?.textContent||"Item",F=I.get("unit")||"unidades";let A=`
      ...
      `;if(M==="sell"&&cost&&parseFloat(cost)>0){const D=parseFloat(cost),T=j-D,V=T*k,ae=T>0?"text-green-400":"text-red-400";A+=`<div class="${ae}"><strong>Lucro por unidade:</strong> ${C(T)}</div>`,A+=`<div class="${ae}"><strong>Lucro total:</strong> ${C(V)}</div>`}if(m.innerHTML=A,M==="buy"){const D=O(t);J>D?(g.classList.remove("hidden"),g.querySelector(".text-sm").innerHTML=`
    ...
    `,document.body.appendChild(a),We(e),await Ke(e)}catch(t){console.error("Erro ao abrir modal de notificações:",t)}}function Z(){const t=document.getElementById("notifications-modal");t&&t.remove()}function We(t){const e=document.getElementById("notifications-modal");if(!e)return;const a=e.querySelectorAll(".notification-filter-btn");a.forEach(s=>{s.addEventListener("click",()=>{a.forEach(o=>{o.classList.remove("active","bg-brand-500/20","text-brand-400","border-brand-400/30"),o.classList.add("bg-bg/50","text-slate-300","border-bg-ring")}),s.classList.add("active","bg-brand-500/20","text-brand-400","border-brand-400/30"),s.classList.remove("bg-bg/50","text-slate-300","border-bg-ring");const i=s.dataset.filter;Q(i)})}),e.addEventListener("click",s=>{s.target===e&&Z()}),document.addEventListener("keydown",function s(i){i.key==="Escape"&&(Z(),document.removeEventListener("keydown",s))})}let L=[];async function Ke(t){try{const e=await y.collection("notifications").where("target_country_id","==",t).orderBy("created_at","desc").limit(50).get();L=[],e.forEach(a=>{L.push({id:a.id,...a.data()})}),be(L)}catch(e){console.error("Erro ao carregar notificações:",e);const a=document.getElementById("notifications-list");a&&(a.innerHTML=`
        <div class="text-center py-8 text-red-400">
          <div class="text-4xl mb-2">❌</div>
          <p>Erro ao carregar notificações</p>
        </div>
      `)}}function be(t){const e=document.getElementById("notifications-list");if(e){if(t.length===0){e.innerHTML=`
      <div class="text-center py-8 text-slate-400">
        <div class="text-4xl mb-2">📪</div>
        <p>Nenhuma notificação encontrada</p>
      </div>
    `;return}e.innerHTML=t.map(a=>Ye(a)).join("")}}function Ye(t){const e=!t.read,a=t.created_at?.toDate?t.created_at.toDate():new Date(t.created_at),s=tt(a),i={embargo_applied:"🚫",embargo_lifted:"✅",transaction_created:"💰",transaction_completed:"✅",trade_offer:"📦",diplomatic:"🏛️"},o={embargo_applied:"border-red-400/30 bg-red-400/10",embargo_lifted:"border-green-400/30 bg-green-400/10",transaction_created:"border-blue-400/30 bg-blue-400/10",transaction_completed:"border-green-400/30 bg-green-400/10",trade_offer:"border-yellow-400/30 bg-yellow-400/10",diplomatic:"border-purple-400/30 bg-purple-400/10"},r=i[t.type]||"📬";return`
  ...
  `}function Q(t){let e=L;t==="unread"?e=L.filter(a=>!a.read):t==="embargo"?e=L.filter(a=>a.type&&a.type.includes("embargo")):t==="transaction"&&(e=L.filter(a=>a.type&&a.type.includes("transaction"))),be(e)}async function Xe(t){try{await y.collection("notifications").doc(t).update({read:!0,read_at:new Date});const e=L.find(s=>s.id===t);e&&(e.read=!0);const a=document.querySelector(".notification-filter-btn.active")?.dataset.filter||"all";Q(a),await G()}catch(e){console.error("Erro ao marcar notificação como lida:",e)}}async function Ze(){try{const t=_.currentUser;if(!t||!await E(t.uid))return;const a=y.batch();L.forEach(i=>{if(!i.read){const o=y.collection("notifications").doc(i.id);a.update(o,{read:!0,read_at:new Date}),i.read=!0}}),await a.commit();const s=document.querySelector(".notification-filter-btn.active")?.dataset.filter||"all";Q(s),await G()}catch(t){console.error("Erro ao marcar todas as notificações como lidas:",t)}}async function et(t){try{if(!confirm("Tem certeza que deseja excluir esta notificação?"))return;await y.collection("notifications").doc(t).delete(),L=L.filter(a=>a.id!==t);const e=document.querySelector(".notification-filter-btn.active")?.dataset.filter||"all";Q(e),await G()}catch(e){console.error("Erro ao excluir notificação:",e)}}async function G(){try{const t=_.currentUser;if(!t)return;const e=await E(t.uid);if(!e)return;const s=(await y.collection("notifications").where("target_country_id","==",e).where("read","==",!1).get()).size,i=document.getElementById("notifications-count-badge");i&&(s>0?(i.textContent=s>99?"99+":s.toString(),i.classList.remove("hidden")):i.classList.add("hidden"))}catch(t){console.error("Erro ao atualizar contador de notificações:",t)}}function tt(t){const a=new Date-t,s=Math.floor(a/(1e3*60)),i=Math.floor(a/(1e3*60*60)),o=Math.floor(a/(1e3*60*60*24));return s<1?"Agora há pouco":s<60?`${s} min atrás`:i<24?`${i}h atrás`:o<7?`${o} dias atrás`:t.toLocaleDateString("pt-BR")}window.closeNotificationsModal=Z;window.markNotificationAsRead=Xe;window.markAllNotificationsAsRead=Ze;window.deleteNotification=et;async function at(){try{const t=_.currentUser;if(!t)return;const e=await E(t.uid);if(!e)return;const a=document.createElement("div");a.className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4",a.id="embargoes-modal",a.innerHTML=`
      <div class="bg-bg-soft border border-bg-ring rounded-xl max-w-4xl w-full max-h-[80vh] overflow-y-auto">
        <div class="p-6 border-b border-bg-ring/50">
          <div class="flex items-center justify-between">
    ...
    `,document.body.appendChild(a),await st(e)}catch(t){console.error("Erro ao abrir modal de embargos:",t)}}async function st(t){try{const e=await y.collection("marketplace_embargoes").where("embargo_country_id","==",t).where("status","==","active").orderBy("created_at","desc").get(),a=await y.collection("marketplace_embargoes").where("target_country_id","==",t).where("status","==","active").orderBy("created_at","desc").get(),s=[],i=[];e.forEach(d=>{s.push({id:d.id,...d.data()})}),a.forEach(d=>{i.push({id:d.id,...d.data()})});const o=document.getElementById("applied-embargoes-list");o&&(s.length===0?o.innerHTML=`
          <div class="text-center py-8 text-slate-400">
            <div class="text-4xl mb-2">🕊️</div>
            <p>Nenhum embargo aplicado</p>
          </div>
        `:o.innerHTML=s.map(d=>re(d,"applied")).join(""));const r=document.getElementById("received-embargoes-list");r&&(i.length===0?r.innerHTML=`
          <div class="text-center py-8 text-slate-400">
            <div class="text-4xl mb-2">✅</div>
  ...
  `}async function rt(){try{const t=_.currentUser;if(!t)return;const e=await E(t.uid);if(!e)return;const a=await y.collection("paises").get(),s=[];a.forEach(o=>{const r=o.data();o.id!==e&&s.push({id:o.id,name:r.Pais,flag:r.Flag||"🏳️"})});const i=document.createElement("div");i.className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4",i.id="create-embargo-modal",i.innerHTML=`
    ...
    `,document.body.appendChild(i),ot()}catch(t){console.error("Erro ao abrir modal de criar embargo:",t)}}function ot(){const t=document.querySelectorAll('input[name="embargo-type"]'),e=document.getElementById("categories-section");t.forEach(s=>{s.addEventListener("change",()=>{s.value==="partial"?e.classList.remove("hidden"):e.classList.add("hidden")})});const a=document.getElementById("create-embargo-form");a&&a.addEventListener("submit",it)}async function it(t){t.preventDefault();try{const e=new FormData(t.target),a=document.getElementById("target-country").value,s=e.get("embargo-type"),i=document.getElementById("embargo-reason").value,o=document.getElementById("embargo-duration").value;if(!a){alert("Selecione um país alvo");return}let r=[];if(s==="partial"){const b=document.querySelectorAll('input[name="blocked-categories"]:checked');if(r=Array.from(b).map(v=>v.value),r.length===0){alert("Selecione pelo menos uma categoria para embargo parcial");return}}let d=null;o&&(d=new Date(Date.now()+parseInt(o)*24*60*60*1e3));const l={target_country_id:a,type:s,categories_blocked:r,reason:i||"Motivos diplomáticos",expires_at:d},n=t.target.querySelector('button[type="submit"]'),c=n.textContent;n.textContent="Aplicando...",n.disabled=!0;const p=await q.applyEmbargo(l);if(p.success){alert("Embargo aplicado com sucesso!"),nt();const b=_.currentUser;if(b){const v=await E(b.uid);if(v){const x=document.querySelector(".marketplace-category-btn.active")?.dataset.category||"all";B(x,v),ve(v)}}}else throw new Error(p.error)}catch(e){console.error("Erro ao aplicar embargo:",e),alert(`Erro ao aplicar embargo: ${e.message}`);const a=t.target.querySelector('button[type="submit"]');a.textContent="Aplicar Embargo",a.disabled=!1}}function nt(){const t=document.getElementById("create-embargo-modal");t&&t.remove()}async function ve(t){try{const e=await Y(t),a=document.getElementById("embargo-status-indicator"),s=document.getElementById("embargo-count-badge");if(!a||!s)return;if(e.hasEmbargoes){const i=e.hasFullEmbargo?"todas as categorias":`${e.blockedCategories.length} categoria(s)`;a.innerHTML=`
      ...
      `,s.classList.add("hidden")}catch(e){console.error("Erro ao atualizar indicador de embargo:",e)}}function lt(){const t=document.getElementById("toggle-advanced-filters"),e=document.getElementById("advanced-filters-panel"),a=document.getElementById("advanced-filters-icon");t&&e&&a&&t.addEventListener("click",()=>{e.classList.toggle("hidden"),a.classList.toggle("rotate-180")});const s=document.getElementById("apply-filters-btn"),i=document.getElementById("clear-filters-btn"),o=document.getElementById("save-filters-btn");s&&s.addEventListener("click",ge),i&&i.addEventListener("click",dt),o&&o.addEventListener("click",ct),ut()}async function ge(){const t=_.currentUser;if(!t)return;const e=await E(t.uid);if(!e)return;he();const a=document.querySelector(".marketplace-category-btn.active")?.dataset.category||"all";await B(a,e)}function dt(){document.getElementById("marketplace-search").value="",document.getElementById("marketplace-sort").value="date",document.getElementById("marketplace-type").value="all",document.getElementById("price-min").value="",document.getElementById("price-max").value="",document.getElementById("quantity-min").value="",document.getElementById("quantity-max").value="",document.getElementById("country-filter").value="",document.getElementById("time-filter").value="",ge()}function ct(){const t={search:document.getElementById("marketplace-search").value,sort:document.getElementById("marketplace-sort").value,type:document.getElementById("marketplace-type").value,priceMin:document.getElementById("price-min").value,priceMax:document.getElementById("price-max").value,quantityMin:document.getElementById("quantity-min").value,quantityMax:document.getElementById("quantity-max").value,country:document.getElementById("country-filter").value,timeFilter:document.getElementById("time-filter").value};localStorage.setItem("marketplace-filters",JSON.stringify(t));const e=document.getElementById("save-filters-btn"),a=e.textContent;e.textContent="✅ Salvo!",e.disabled=!0,setTimeout(()=>{e.textContent=a,e.disabled=!1},2e3)}function ut(){try{const t=localStorage.getItem("marketplace-filters");if(!t)return;const e=JSON.parse(t);e.search&&(document.getElementById("marketplace-search").value=e.search),e.sort&&(document.getElementById("marketplace-sort").value=e.sort),e.type&&(document.getElementById("marketplace-type").value=e.type),e.priceMin&&(document.getElementById("price-min").value=e.priceMin),e.priceMax&&(document.getElementById("price-max").value=e.priceMax),e.quantityMin&&(document.getElementById("quantity-min").value=e.quantityMin),e.quantityMax&&(document.getElementById("quantity-max").value=e.quantityMax),e.country&&(document.getElementById("country-filter").value=e.country),e.timeFilter&&(document.getElementById("time-filter").value=e.timeFilter)}catch(t){console.error("Erro ao carregar filtros salvos:",t)}}async function mt(){try{const t=await y.collection("paises").get(),e=document.getElementById("country-filter");if(!e)return;for(;e.children.length>1;)e.removeChild(e.lastChild);const a=[];t.forEach(s=>{const i=s.data();a.push({id:s.id,name:i.Pais,flag:i.Flag||"🏳️"})}),a.sort((s,i)=>s.name.localeCompare(i.name)),a.forEach(s=>{const i=document.createElement("option");i.value=s.id,i.textContent=`${s.flag} ${s.name}`,e.appendChild(i)})}catch(t){console.error("Erro ao carregar países para filtro:",t)}}let ee=1,N=!1,R=!1,z=null;function pt(t,e,a){const s=document.getElementById("load-more-btn"),i=document.getElementById("infinite-scroll-toggle"),o=document.getElementById("infinite-scroll-icon");z={...a,countryId:t,category:e},s&&s.addEventListener("click",()=>{fe()}),i&&o&&(i.addEventListener("click",()=>{N=!N,N?(o.textContent="♾️",i.title="Carregamento automático ativado",bt()):(o.textContent="🔄",i.title="Carregamento automático desativado",xe()),localStorage.setItem("marketplace-infinite-scroll",N)}),localStorage.getItem("marketplace-infinite-scroll")==="true"&&i.click())}async function fe(){if(R||!z)return;R=!0;const t=document.getElementById("load-more-btn"),e=document.getElementById("load-more-state"),a=document.getElementById("offers-grid");t&&(t.disabled=!0),e&&e.classList.remove("hidden");try{ee++;const s={...z,limit:20,offset:(ee-1)*20},i=await q.getOffers(s);if(i.success&&i.offers.length>0){const o=i.offers.map(l=>pe(l)).join("");a&&(a.innerHTML+=o);const r=a?.children.length||0,d=document.querySelector(".text-sm.text-slate-400");d&&(d.textContent=`Mostrando ${r} ofertas`),i.offers.length<20&&t&&(t.textContent="✅ Todas as ofertas carregadas",t.disabled=!0)}else t&&(t.textContent="✅ Todas as ofertas carregadas",t.disabled=!0)}catch(s){console.error("Erro ao carregar mais ofertas:",s),t&&(t.disabled=!1)}finally{R=!1,e&&e.classList.add("hidden")}}function bt(){xe(),window.addEventListener("scroll",ye)}function xe(){window.removeEventListener("scroll",ye)}function ye(){if(!N||R)return;const t=window.innerHeight+window.scrollY,e=document.documentElement.offsetHeight;if(t>=e-200){const s=document.getElementById("load-more-btn");s&&!s.disabled&&fe()}}function he(){ee=1,R=!1,z=null}function we(){try{const t=localStorage.getItem("marketplace-favorites");return t?JSON.parse(t):[]}catch(t){return console.error("Erro ao carregar favoritos:",t),[]}}function vt(t){return we().includes(t)}function gt(t){t.forEach(e=>{const a=document.getElementById(`favorite-btn-${e.id}`),s=document.getElementById(`favorite-icon-${e.id}`);a&&s&&(vt(e.id)?(a.classList.remove("bg-slate-600/20","text-slate-400"),a.classList.add("bg-yellow-500/20","text-yellow-400"),a.title="Remover dos favoritos",s.textContent="🌟"):(a.classList.remove("bg-yellow-500/20","text-yellow-400"),a.classList.add("bg-slate-600/20","text-slate-400"),a.title="Adicionar aos favoritos",s.textContent="⭐"))})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",se):se();
