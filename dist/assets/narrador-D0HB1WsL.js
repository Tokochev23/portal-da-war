                ...
                <div class="flex-1 overflow-auto p-6">
                    ...
                    `:`
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ${Object.entries(a).map(([l,d])=>this.renderVehicleCard(l,d,e)).join("")}
                        </div>
                    `}
                </div>
            </div>
        `,s.addEventListener("click",l=>{l.target===s&&s.remove()}),document.addEventListener("keydown",function l(d){d.key==="Escape"&&(s.remove(),document.removeEventListener("keydown",l))}),document.body.appendChild(s)}renderVehicleCard(e,t,a){const o=(t.cost||0)*(t.quantity||0),r=t.sheetImageUrl||t.sheetHtmlUrl||t.specs,s=t.specs||{},n=t.maintenanceCost||t.costs?.maintenance||0,i=n*(t.quantity||0);return`
...
Todos os arquivos associados serão removidos para economizar espaço.`)){console.log(`🗑️ Iniciando rejeição em lote de ${e.length} veículos...`);for(const t of e)await this.rejectVehicle(t);console.log(`✅ Rejeição em lote concluída. ${e.length} veículos e arquivos removidos.`)}}getSelectedVehicles(){const e=document.querySelectorAll(".vehicle-select:checked");return Array.from(e).map(t=>t.dataset.vehicleId)}updateStats(){const e=document.getElementById("pending-count");e&&(e.textContent=this.pendingVehicles.length)}async debugSystem(){console.log("🔍 === DEBUG DO SISTEMA DE APROVAÇÃO ===");try{console.log("🔥 Firebase auth:",window.firebase?.auth()),console.log("👤 Current user:",window.firebase?.auth()?.currentUser),console.log("🗃️ Firestore db:",p);const e=p.collection("vehicles_pending");console.log("📁 Pending collection ref:",e);const t=await e.get();if(console.log("📊 Snapshot size:",t.size),console.log("📊 Snapshot empty:",t.empty),!t.empty){t.docs.forEach((o,r)=>{console.log(`📄 Doc ${r+1}:`,o.id,o.data())}),console.log("🔧 FORÇANDO PROCESSAMENTO DOS DOCUMENTOS:");const a=[];for(const o of t.docs)try{const r=o.data();console.log("🔍 Processando no debug:",o.id,Object.keys(r));let s=new Date;r.submittedAt&&r.submittedAt.toDate&&(s=r.submittedAt.toDate());const n={id:o.id,...r,submissionDate:s};a.push(n),console.log("✅ Processado no debug:",n.id,n.vehicleData?.name)}catch(r){console.error("❌ Erro no debug:",r)}console.log("🚀 Total processado no debug:",a.length)}console.log("🧠 Current pending vehicles:",this.pendingVehicles),console.log("🎯 Current filter:",this.currentFilter)}catch(e){console.error("💥 Debug error:",e)}console.log("🔍 === FIM DO DEBUG ===")}async forceReload(){console.log("🔧 === FORCE RELOAD INICIADO ===");try{this.pendingVehicles=[];const e=await p.collection("vehicles_pending").get();console.log("📊 Force reload - documents found:",e.size);for(const t of e.docs){const a=t.data();console.log("🔍 Processing in force reload:",t.id);let o=new Date;a.submittedAt&&a.submittedAt.toDate?o=a.submittedAt.toDate():a.submissionDate&&a.submissionDate.toDate&&(o=a.submissionDate.toDate());const r={id:t.id,...a,submissionDate:o};this.pendingVehicles.push(r),console.log("✅ Added vehicle:",r.id,r.vehicleData?.name)}console.log("🚀 Force reload completed:",this.pendingVehicles.length,"vehicles"),this.render()}catch(e){console.error("❌ Force reload failed:",e)}console.log("🔧 === FORCE RELOAD FIM ===")}}const M={dependency:{light:.3,moderate:.5,heavy:.7,critical:.85},historyTurns:5,effects:{growth_bonus:{light:.05,moderate:.1,heavy:.15,critical:.2},crisis_impact:{light:.1,moderate:.2,heavy:.35,critical:.5}}};class Pe{constructor(){this.dependencyCache=new Map,this.lastCacheUpdate=0,this.cacheTimeout=3e5}async analyzeDependency(e,t,a=!1){try{const o=`${e}-${t}`,r=Date.now();if(!a&&this.dependencyCache.has(o)){const i=this.dependencyCache.get(o);if(r-i.timestamp<this.cacheTimeout)return i.data}const s=await this.getEconomicHistory(e),n=this.calculateDependency(s,t);return this.dependencyCache.set(o,{data:n,timestamp:r}),n}catch(o){throw u.error("Erro ao analisar dependência econômica:",o),o}}async getEconomicHistory(e){try{return(await p.collection("economic_history").where("countryId","==",e).orderBy("turn","desc").limit(M.historyTurns).get()).docs.map(a=>({id:a.id,...a.data()}))}catch(t){return u.error("Erro ao buscar histórico econômico:",t),[]}}calculateDependency(e,t){if(!e||e.length<2)return{level:"none",percentage:0,totalExternal:0,fromInvestor:0,turnsAnalyzed:e.length,riskLevel:"low"};let a=0,o=0,r=0;e.forEach(d=>{d.externalInvestments&&Object.entries(d.externalInvestments).forEach(([v,y])=>{const x=parseFloat(y)||0;a+=x,v===t&&(o+=x,r++)})});const s=a>0?o/a:0,n=r/e.length,i=s*(.5+.5*n);let c="none",l="low";return i>=M.dependency.critical?(c="critical",l="critical"):i>=M.dependency.heavy?(c="heavy",l="high"):i>=M.dependency.moderate?(c="moderate",l="medium"):i>=M.dependency.light&&(c="light",l="low"),{level:c,percentage:i,rawPercentage:s,totalExternal:a,fromInvestor:o,turnsAnalyzed:e.length,turnsWithInvestment:r,consistencyFactor:n,riskLevel:l,growthBonus:M.effects.growth_bonus[c]||0,crisisImpact:M.effects.crisis_impact[c]||0}}async analyzeAllDependencies(e){try{const t=await this.getEconomicHistory(e),a=new Map,o=new Set;t.forEach(s=>{s.externalInvestments&&Object.keys(s.externalInvestments).forEach(n=>{o.add(n)})});for(const s of o){const n=this.calculateDependency(t,s);n.level!=="none"&&a.set(s,n)}const r=Array.from(a.entries()).sort((s,n)=>n[1].percentage-s[1].percentage);return{countryId:e,dependencies:r,totalDependencies:a.size,highestDependency:r[0]||null,riskLevel:this.calculateOverallRisk(r)}}catch(t){throw u.error("Erro ao analisar todas as dependências:",t),t}}calculateOverallRisk(e){if(e.length===0)return"none";const t=e.filter(([,r])=>r.level==="critical").length,a=e.filter(([,r])=>r.level==="heavy").length,o=e.filter(([,r])=>r.level==="moderate").length;return t>0?"critical":a>1||a===1&&o>0?"high":a===1||o>1?"medium":"low"}async checkEconomicCrisis(e){try{const t=await p.collection("paises").doc(e).get();if(!t.exists)return!1;const a=t.data(),o=parseFloat(a.PIB)||0,r=parseFloat(a.Estabilidade)||0,s=await this.getEconomicHistory(e);if(s.length<2)return!1;const n=parseFloat(s[1].results?.newPIB||a.PIB),i=(o-n)/n;return{isCrisis:i<-.15||r<25||i<-.05&&r<40,pibChange:i,stability:r,severity:this.calculateCrisisSeverity(i,r)}}catch(t){return u.error("Erro ao verificar crise econômica:",t),!1}}calculateCrisisSeverity(e,t){let a=0;return e<-.3?a+=3:e<-.2?a+=2:e<-.1&&(a+=1),t<20?a+=3:t<35?a+=2:t<50&&(a+=1),a>=5?"severe":a>=3?"moderate":a>=1?"mild":"none"}async applyDependencyCrisisEffects(e){try{const t=await this.checkEconomicCrisis(e);if(!t.isCrisis)return[];const a=[],o=await p.collection("paises").get();for(const r of o.docs){const s=r.id;if(s===e)continue;const n=await this.analyzeDependency(s,e);if(n.level!=="none"){const i=r.data(),c=parseFloat(i.PIB)||0,l=n.crisisImpact*t.severity==="severe"?1.5:1,d=c*l,v=c-d;await p.collection("paises").doc(s).update({PIB:v,TurnoUltimaAtualizacao:parseInt(document.getElementById("turno-atual-admin")?.textContent?.replace("#",""))||1}),a.push({countryId:s,countryName:i.Pais,dependencyLevel:n.level,pibLoss:d,newPIB:v,impact:l*100})}}return a}catch(t){throw u.error("Erro ao aplicar efeitos de crise de dependência:",t),t}}generateDependencyReport(e){const{countryId:t,dependencies:a,riskLevel:o}=e;return{summary:this.generateSummaryText(a,o),recommendations:this.generateRecommendations(a,o),riskMatrix:a.map(([s,n])=>({investor:s,level:n.level,percentage:n.percentage,risk:n.riskLevel,growthBonus:n.growthBonus,crisisImpact:n.crisisImpact}))}}generateSummaryText(e,t){if(e.length===0)return"País mantém independência econômica total. Sem dependências externas significativas.";const a=e.length,o=e.filter(([,n])=>n.level==="critical").length,r=e.filter(([,n])=>n.level==="heavy").length;let s=`País possui ${a} dependência${a>1?"s":""} econômica${a>1?"s":""}.`;return o>0&&(s+=` ${o} crítica${o>1?"s":""}.`),r>0&&(s+=` ${r} pesada${r>1?"s":""}.`),s+=` Risco geral: ${t}.`,s}generateRecommendations(e,t){const a=[];(t==="critical"||t==="high")&&(a.push("Diversificar fontes de investimento externo urgentemente."),a.push("Aumentar investimentos internos para reduzir dependência.")),e.length>3&&a.push("Consolidar parcerias econômicas para reduzir complexidade.");const o=e.filter(([,r])=>r.level==="critical");return o.length>0&&a.push(`Negociar maior autonomia com ${o[0][0]} devido à dependência crítica.`),a.length===0&&a.push("Manter diversificação atual de investimentos externos."),a}clearCache(){this.dependencyCache.clear(),this.lastCacheUpdate=0}}const Ie=new Pe,F={growth:{excellent:["✨ **Crescimento Excepcional!** A economia nacional floresceu sob suas políticas visionárias.","🚀 **Boom Econômico!** Seus investimentos estratégicos criaram um círculo virtuoso de prosperidade.","⭐ **Era Dourada!** O país vivencia seu melhor período econômico em décadas."],good:["✅ **Crescimento Sólido** A diversificação econômica está dando frutos positivos.","📈 **Progresso Sustentável** Suas reformas econômicas mostram resultados consistentes.","💪 **Economia Resiliente** O país demonstra capacidade de crescimento estável."],moderate:["📊 **Crescimento Moderado** A economia mantém trajetória de expansão cautelosa.","⚖️ **Desenvolvimento Equilibrado** O país avança de forma sustentada, sem riscos.","🎯 **Metas Atingidas** Os objetivos econômicos estão sendo cumpridos gradualmente."],poor:["⚠️ **Crescimento Limitado** A economia enfrenta desafios que impedem maior expansão.","🔄 **Ajustes Necessários** É preciso revisar as estratégias de investimento atuais.","📉 **Potencial Não Realizado** O país possui capacidade para crescimento maior."],negative:["🚨 **Recessão Econômica** A economia nacional enfrenta sérias dificuldades.","⛈️ **Crise Econômica** Políticas de emergência são necessárias para estabilização.","🆘 **Situação Crítica** Reformas estruturais urgentes são essenciais para recuperação."]},inflation:{low:["💡 **Gestão Eficiente** Seus investimentos foram bem planejados, com baixa inflação.","🎯 **Precisão Econômica** A estratégia de diversificação evitou pressões inflacionárias.","⚡ **Investimento Inteligente** A alocação equilibrada de recursos maximizou a eficiência."],moderate:["⚠️ **Inflação Controlável** Há sinais de aquecimento econômico que requerem atenção.","🌡️ **Economia Aquecida** O volume de investimentos está criando pressões de preços.","⚖️ **Equilíbrio Delicado** É preciso balancear crescimento com estabilidade de preços."],high:["🔥 **Alta Inflação** O excesso de investimentos está criando desequilíbrios econômicos.","⛔ **Superaquecimento** A economia precisa de políticas de resfriamento urgentes.","📈 **Pressão de Preços** A concentração de gastos está gerando inflação preocupante."],severe:["🚨 **Hiperinflação Ameaça** Os investimentos excessivos criaram uma crise inflacionária.","💥 **Colapso de Preços** A estratégia econômica resultou em instabilidade monetária severa.","🌪️ **Descontrole Inflacionário** Medidas de emergência são necessárias imediatamente."]},chains:["🔗 **Sinergia Perfeita!** A combinação de infraestrutura e indústria potencializou o crescimento.","⚙️ **Engrenagem Eficiente** Pesquisa & Desenvolvimento impulsionou a modernização industrial.","🧬 **DNA de Inovação** A integração entre ciência e políticas sociais criou resultados excepcionais.","🏗️ **Base Sólida** Investimentos em infraestrutura criaram fundações para expansão industrial.","🔬 **Revolução Científica** P&D transformou o panorama tecnológico e social do país."],dependency:{created:["🤝 **Nova Parceria** Sua cooperação com {investor} fortaleceu os laços econômicos.","🌍 **Integração Internacional** Os investimentos externos expandiram horizontes econômicos.","💼 **Diplomacia Econômica** A parceria internacional traz benefícios mútuos."],increased:["📈 **Dependência Crescente** Sua economia está cada vez mais integrada com {investor}.","⚠️ **Atenção Necessária** A dependência de {investor} requer monitoramento cuidadoso.","🔄 **Diversificação Recomendada** Considere expandir parcerias para reduzir riscos."],critical:["🚨 **Dependência Crítica** Sua economia tornou-se vulnerável às crises de {investor}.","⛔ **Risco Elevado** A dependência excessiva de {investor} compromete a autonomia nacional.","🆘 **Alerta Máximo** É urgente diversificar fontes de investimento externo."]},external_actions:["🌐 **Influência Internacional** Seus investimentos em {target} fortalecem sua posição geopolítica.","🤝 **Soft Power** A ajuda econômica a {target} amplia sua influência regional.","💰 **Diplomacia do Dólar** Os investimentos externos são uma ferramenta de política externa eficaz.","🌟 **Liderança Global** Sua capacidade de investir no exterior demonstra força econômica.","⚖️ **Responsabilidade Internacional** Os investimentos externos equilibram desenvolvimento e cooperação."],stability:["🏥 **Bem-Estar Social** Investimentos em saúde e educação fortalecem a coesão nacional.","👥 **Harmonia Social** Políticas sociais reduzem tensões e aumentam a estabilidade.","🛡️ **Resiliência Nacional** A estabilidade política é a base para crescimento sustentado.","🕊️ **Paz Social** Investimentos sociais criam um ambiente favorável ao desenvolvimento."],rejection:["😠 **Resistência Popular** A população de {target} vê seus investimentos como ingerência externa.","🗳️ **Tensão Política** Os investimentos em {target} geraram protestos e instabilidade.","🚫 **Rejeição Nacional** {target} demonstra resistência crescente à sua influência econômica.","⚡ **Conflito Diplomático** Os investimentos externos criaram atritos internacionais."]};class Se{async generatePlayerFeedback(e,t,a){try{const o={countryId:e,turn:this.getCurrentTurn(),timestamp:new Date,mainMessage:"",details:[],warnings:[],achievements:[],recommendations:[]},r=this.generateGrowthFeedback(t);o.mainMessage=r.message,r.achievement&&o.achievements.push(r.achievement);const s=this.generateInflationFeedback(t);if(s&&o.details.push(s),t.productiveChains.length>0){const l=this.generateChainFeedback(t.productiveChains);o.details.push(l)}const n=a.filter(l=>l.isExternal);if(n.length>0){const l=await this.generateExternalFeedback(n,e);o.details.push(...l)}const i=await this.generateDependencyFeedback(e);i&&o.warnings.push(...i);const c=this.generateStrategicRecommendations(t,a);return o.recommendations.push(...c),await this.saveFeedback(o),o}catch(o){throw u.error("Erro ao gerar feedback do player:",o),o}}generateGrowthFeedback(e){const t=e.finalGrowth/e.newPIB*100;let a,o=null;t>=15?(a="excellent",o="🏆 **Milagre Econômico** - Crescimento excepcional de mais de 15%"):t>=8?(a="good",o="🥇 **Crescimento Exemplar** - Expansão econômica robusta"):t>=3?a="moderate":t>=0?a="poor":a="negative";const r=F.growth[a];return{message:this.randomChoice(r),achievement:o}}generateInflationFeedback(e){const t=e.totalInflation*100;let a;if(t>=60?a="severe":t>=35?a="high":t>=15?a="moderate":a="low",a==="low")return null;const o=F.inflation[a];return this.randomChoice(o)}generateChainFeedback(e){e.map(a=>a.name).join(", ");let t=this.randomChoice(F.chains);return e.some(a=>a.name.includes("Infraestrutura"))?t="🔗 **Sinergia Infraestrutural** A base sólida potencializou outros setores da economia.":e.some(a=>a.name.includes("P&D"))&&(t="🧬 **Inovação Integrada** Pesquisa & Desenvolvimento revolucionou múltiplos setores."),t}async generateExternalFeedback(e,t){const a=[];for(const o of e){if(!o.targetCountry)continue;const r=await this.getCountryData(o.targetCountry);if(!r)continue;if(this.checkRejectionRisk(o,r).hasRisk){const n=this.randomChoice(F.rejection).replace("{target}",r.Pais||o.targetCountry);a.push(n)}else{const n=this.randomChoice(F.external_actions).replace("{target}",r.Pais||o.targetCountry);a.push(n)}}return a}async generateDependencyFeedback(e){try{const t=await Ie.analyzeAllDependencies(e),a=[];if(t.dependencies.length===0)return null;const o=t.dependencies.filter(([,s])=>s.level==="critical"),r=t.dependencies.filter(([,s])=>s.level==="heavy");for(const[s,n]of o){const i=await this.getCountryData(s),c=this.randomChoice(F.dependency.critical).replace("{investor}",i?.Pais||s);a.push(c)}for(const[s,n]of r.slice(0,2)){const i=await this.getCountryData(s),c=this.randomChoice(F.dependency.increased).replace("{investor}",i?.Pais||s);a.push(c)}return a}catch(t){return u.error("Erro ao gerar feedback de dependência:",t),null}}generateStrategicRecommendations(e,t){const a=[],o=e.totalInflation*100,r=t.some(n=>n.isExternal),s=[...new Set(t.map(n=>n.type))];return o>40?a.push("💡 **Sugestão:** Reduza o volume de investimentos no próximo turno para controlar a inflação."):o<5&&a.push("💡 **Oportunidade:** Sua economia pode absorver mais investimentos sem riscos inflacionários."),s.length<=2&&a.push("🎯 **Estratégia:** Diversifique os tipos de investimento para ativar cadeias produtivas."),!r&&e.finalGrowth>0?a.push("🌍 **Diplomacia:** Considere investimentos externos para expandir sua influência internacional."):r&&t.filter(n=>n.isExternal).length>=3&&a.push("🏠 **Foco Interno:** Balance investimentos externos com desenvolvimento interno."),e.productiveChains.length===0&&a.push("🔗 **Sinergia:** Combine diferentes tipos de investimento para ativar bônus de cadeias produtivas."),a.slice(0,3)}checkRejectionRisk(e,t){const a=parseFloat(t.Estabilidade)||0,o=parseFloat(t.PIB)||1,s=(parseFloat(e.value)||0)*1e6/o;return{hasRisk:a<40&&s>.1,riskLevel:s>.2?"high":"medium"}}async saveFeedback(e){try{await p.collection("player_feedback").doc().set({...e,createdAt:new Date}),u.info(`Feedback salvo para país ${e.countryId}`)}catch(t){throw u.error("Erro ao salvar feedback:",t),t}}async getPlayerFeedback(e,t=5){try{return(await p.collection("player_feedback").where("countryId","==",e).orderBy("turn","desc").limit(t).get()).docs.map(o=>({id:o.id,...o.data()}))}catch(a){return u.error("Erro ao buscar feedback do player:",a),[]}}randomChoice(e){return e[Math.floor(Math.random()*e.length)]}getCurrentTurn(){return parseInt(document.getElementById("turno-atual-admin")?.textContent?.replace("#",""))||1}async getCountryData(e){try{const t=await p.collection("paises").doc(e).get();return t.exists?t.data():null}catch(t){return u.error("Erro ao buscar dados do país:",t),null}}formatFeedbackForDisplay(e){let t=`
    ...
    `}setupModalEventListeners(){const e=document.getElementById("economic-simulator-modal");if(!e)return;e.querySelector("#close-economic-modal").addEventListener("click",()=>{e.remove()}),e.addEventListener("click",a=>{a.target===e&&e.remove()}),e.querySelector("#modal-country-select").addEventListener("change",a=>{this.selectedCountry=a.target.value,this.resetActions(),e.remove(),this.showModal()});const t=e.querySelector("#modal-industrial-policy");t&&t.addEventListener("change",a=>{const o=this.getCountryById(this.selectedCountry);if(o){o.PoliticaIndustrial=a.target.value;try{p.collection("paises").doc(this.selectedCountry).update({PoliticaIndustrial:a.target.value})}catch(r){console.warn("Erro ao salvar PoliticaIndustrial:",r)}}}),e.querySelectorAll(".economic-tab").forEach(a=>{a.addEventListener("click",o=>{this.switchTab(o.target.dataset.tab)})}),e.querySelector("#add-internal-action")?.addEventListener("click",()=>{this.addAction("internal")}),e.querySelector("#add-external-action")?.addEventListener("click",()=>{this.addAction("external")}),e.querySelector("#reset-economic-actions")?.addEventListener("click",()=>{this.resetActions()}),e.querySelector("#apply-economic-actions")?.addEventListener("click",()=>{this.applyEconomicActions()}),this.setupActionFieldListeners()}setupActionFieldListeners(){const e=document.getElementById("economic-simulator-modal");e&&(e.addEventListener("input",t=>{if(t.target.matches(".action-type, .target-country, .action-value, .action-dice, .action-buff, .resource-type")){const a=t.target.closest(".action-slot"),o=a.dataset.type,r=parseInt(a.dataset.index);this.updateActionFromSlot(o,r,a),this.updatePreview(a),this.updateBudgetBar(),this.updateSummary()}}),e.addEventListener("click",t=>{if(t.target.matches(".remove-action")){const a=t.target.closest(".action-slot"),o=a.dataset.type,r=parseInt(a.dataset.index);this.removeAction(o,r)}}),e.addEventListener("change",t=>{t.target.matches(".action-type")&&(this.toggleActionDescription(t.target),this.toggleResourceSelector(t.target))}),e.querySelectorAll(".economy-subtab").forEach(t=>{t.addEventListener("click",a=>{const o=a.target.dataset.subtab;this.switchEconomySubtab(o)})}),e.addEventListener("click",t=>{if(t.target.matches(".tech-increment")){const a=t.target.dataset.field;this.adjustTechnology(a,1)}else if(t.target.matches(".tech-decrement")){const a=t.target.dataset.field;this.adjustTechnology(a,-1)}}),e.addEventListener("input",t=>{if(t.target.matches(".tech-input")){const a=t.target.dataset.field,o=parseInt(t.target.value)||0;this.changes.technology[a]=o,this.updateTechPreview(a),this.updateSummary()}}),e.addEventListener("click",t=>{if(t.target.matches(".indicator-increment")){const a=t.target.dataset.field;this.adjustIndicator(a,1)}else if(t.target.matches(".indicator-decrement")){const a=t.target.dataset.field;this.adjustIndicator(a,-1)}}),e.addEventListener("input",t=>{if(t.target.matches(".indicator-input")){const a=t.target.dataset.field,o=parseFloat(t.target.value)||0;this.changes.indicators[a]=o,this.updateIndicatorPreview(a),this.updateSummary()}}))}calculateBudget(e){return fe(e)}formatCurrency(e){return ne(e)}getCountryById(e){return this.countries.find(t=>t.id===e)}switchTab(e){const t=document.getElementById("economic-simulator-modal");if(!t)return;t.querySelectorAll(".economic-tab").forEach(r=>{r.classList.remove("border-purple-500","text-purple-400","bg-slate-700/30"),r.classList.add("border-transparent","text-slate-400")}),t.querySelectorAll(".economic-tab-content").forEach(r=>{r.classList.add("hidden")});const a=t.querySelector(`[data-tab="${e}"]`),o=t.querySelector(`#economic-tab-${e}`);a&&o&&(a.classList.add("border-purple-500","text-purple-400","bg-slate-700/30"),a.classList.remove("border-transparent","text-slate-400"),o.classList.remove("hidden")),e==="summary"&&this.updateSummary()}addAction(e){const t=e==="internal"?A.maxInternalActions:A.maxExternalActions;if(this.actions[e].length>=t){g("warning",`Máximo de ${t} ações ${e==="internal"?"internas":"externas"} atingido`);return}this.actions[e].push({type:"",value:0,dice:0,buff:0,targetCountry:e==="external"?"":null}),this.recreateActionSlots(e),this.updateTabCounters()}removeAction(e,t){t>=0&&t<this.actions[e].length&&(this.actions[e].splice(t,1),this.recreateActionSlots(e),this.updateTabCounters(),this.updateBudgetBar(),this.updateSummary())}recreateActionSlots(e){const t=document.getElementById(`${e}-actions-container`);t&&(t.innerHTML=this.createActionSlots(e))}updateActionFromSlot(e,t,a){this.actions[e][t]||(this.actions[e][t]={});const o=this.actions[e][t];o.type=a.querySelector(".action-type")?.value||"",o.value=parseFloat(a.querySelector(".action-value")?.value)||0,o.dice=parseInt(a.querySelector(".action-dice")?.value)||0,o.buff=parseFloat(a.querySelector(".action-buff")?.value)||0,o.isExternal=e==="external",e==="external"&&(o.targetCountry=a.querySelector(".target-country")?.value||""),o.type==="exploration"&&(o.resourceType=a.querySelector(".resource-type")?.value||"");const r=a.querySelector(".buff-value");r&&(r.textContent=o.buff),o.type==="exploration"&&this.updateExtractionPreview(a,o)}updatePreview(e){const t=e.dataset.type,a=parseInt(e.dataset.index),o=this.actions[t][a];if(!o||!o.type||!o.value||!o.dice){e.querySelector(".growth-preview").textContent="+0.0%";return}const r=this.getCountryById(this.selectedCountry);if(r)try{let s;if(o.type==="exploration"&&o.resourceType){const l={Combustivel:"CombustivelProducao",Carvao:"CarvaoProducao",Metais:"MetaisProducao"}[o.resourceType],d=parseFloat(r[l])||1,y=A.actionTypes[o.type]?.multiplier||.8;s=(o.value/d*y*100).toFixed(2)}else s=(k.calculateBaseGrowth(o,r).preInflationGrowth*100).toFixed(2);const n=e.querySelector(".growth-preview");n.textContent=`+${s}%`,n.className="growth-preview text-xs text-center px-2 py-1 rounded";const i=parseFloat(s);i>1?n.classList.add("bg-emerald-700","text-emerald-200"):i>0?n.classList.add("bg-blue-700","text-blue-200"):i===0?n.classList.add("bg-yellow-700","text-yellow-200"):n.classList.add("bg-red-700","text-red-200")}catch(s){u.error("Erro no preview:",s),e.querySelector(".growth-preview").textContent="Erro"}}updateBudgetBar(){const e=[...this.actions.internal,...this.actions.external].reduce((n,i)=>n+(parseFloat(i.value)||0),0),t=this.formatCurrency(e*1e6),a=Math.min(e*1e6/this.currentBudget*100,100),o=document.getElementById("budget-used"),r=document.getElementById("budget-bar"),s=document.getElementById("apply-economic-actions");if(o&&(o.textContent=t),r&&(r.style.width=`${a}%`,r.className="h-2 rounded-full transition-all",a>90?r.classList.add("bg-gradient-to-r","from-red-500","to-red-600"):a>70?r.classList.add("bg-gradient-to-r","from-yellow-500","to-orange-500"):r.classList.add("bg-gradient-to-r","from-emerald-500","to-green-500")),s){const n=[...this.actions.internal,...this.actions.external].some(c=>c.type&&c.value>0&&c.dice>0),i=e*1e6>this.currentBudget;s.disabled=!n||i,i?s.textContent="❌ Orçamento Excedido":n?s.textContent="⚡ Aplicar Investimentos":s.textContent="⏳ Configure as Ações"}}updateTabCounters(){const e=document.querySelector('[data-tab="internal"]'),t=document.querySelector('[data-tab="external"]');e&&(e.innerHTML=`🏠 Ações Internas (${this.actions.internal.length}/${A.maxInternalActions})`),t&&(t.innerHTML=`🌍 Ações Externas (${this.actions.external.length}/${A.maxExternalActions})`)}updateSummary(){const e=document.getElementById("economic-summary-content");if(!e)return;const t=this.getCountryById(this.selectedCountry);if(!t)return;const a=[...this.actions.internal,...this.actions.external].filter(n=>n.type&&n.value>0),o=Object.values(this.changes.technology).some(n=>n!==0),r=Object.values(this.changes.indicators).some(n=>n!==0);if(!(a.length>0||o||r)){e.innerHTML=`
      ...
      `;return}try{let n=null;if(a.length>0){const i={};a.filter(c=>c.isExternal).forEach(c=>{c.targetCountry&&(i[c.targetCountry]=this.getCountryById(c.targetCountry))}),n=k.processAllActions(a,t,i)}e.innerHTML=this.createSummaryHTML(n,t)}catch(n){u.error("Erro ao atualizar resumo:",n),e.innerHTML=`
      ...
      `}}createSummaryHTML(e,t){let a='<div class="space-y-6">';if(e){parseFloat(t.PIB);const s=(e.finalGrowth*100).toFixed(2),n=(e.totalGrowth*100).toFixed(2),i=(e.totalInflation*100).toFixed(1);a+=`
        ...
        <div class="bg-gradient-to-r from-purple-900/30 to-blue-900/30 border border-purple-500/30 rounded-xl p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="text-center">
              <div class="text-lg font-bold text-slate-300">${Z(parseFloat(t.PIBPerCapita)||0)}</div>
              <div class="text-xs text-slate-400 mt-1">PIB per Capita</div>
            </div>
            
            <div class="text-center">
              <div class="text-lg font-bold text-emerald-400">${Z(e.newPIBPerCapita)}</div>
        ...
        <div class="border border-slate-600/50 rounded-xl p-6">
          ...
          <div class="space-y-3">
            ${e.actions.map(c=>{const l=(c.preInflationGrowth*100).toFixed(3),d=A.actionTypes[c.type],v=c.dice>5,y=c.dice===4||c.dice===5;return`
                <div class="flex items-center justify-between p-3 rounded-lg ${v?"bg-emerald-900/20 border border-emerald-500/30":c.dice<=3?"bg-red-900/20 border border-red-500/30":y?"bg-yellow-900/20 border border-yellow-500/30":"bg-slate-700/30 border border-slate-600/30"}">
                  <div class="flex-1">
                  ...
                  <div class="text-right">
                    <div class="font-semibold ${v?"text-emerald-400":c.dice<=3?"text-red-400":y?"text-yellow-400":"text-slate-400"}">
                      ${parseFloat(l)>=0?"+":""}${l}%
                    </div>
                    <div class="text-xs text-slate-500">
                      +${Z(c.preInflationGrowth*(parseFloat(t.PIBPerCapita)||0))}
                    </div>
                  </div>
                </div>
              `}).join("")}
      ...
      `}const o=Object.values(this.changes.technology).some(s=>s!==0);o&&(a+=`
        <div class="border border-slate-600/50 rounded-xl p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            ${Object.entries(this.changes.technology).filter(([s,n])=>n!==0).map(([s,n])=>{const i=parseFloat(t[s])||0,c=Math.max(0,i+n),l={Tecnologia:"Tecnologia Civil",Aeronautica:"Aeronáutica",Marinha:"Marinha",Veiculos:"Veículos",Exercito:"Exército"};return`
                  <div class="flex items-center justify-between p-3 rounded-lg ${n>0?"bg-emerald-900/20 border border-emerald-500/30":"bg-red-900/20 border border-red-500/30"}">
                    <div class="text-slate-200">${l[s]||s}</div>
                    <div class="text-right">
      ...
      `);const r=Object.values(this.changes.indicators).some(s=>s!==0);return r&&(a+=`
        <div class="border border-slate-600/50 rounded-xl p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            ${Object.entries(this.changes.indicators).filter(([s,n])=>n!==0).map(([s,n])=>{const i=parseFloat(t[s])||0,c=Math.min(100,Math.max(0,i+n)),l={Estabilidade:"Estabilidade",Burocracia:"Burocracia",Urbanizacao:"Urbanização",IndustrialEfficiency:"Eficiência Industrial"};return`
                  <div class="flex items-center justify-between p-3 rounded-lg ${n>0?"bg-emerald-900/20 border border-emerald-500/30":"bg-red-900/20 border border-red-500/30"}">
                    <div class="text-slate-200">${l[s]||s}</div>
                    <div class="text-right">
                      <div class="text-sm text-slate-400">${i}% → ${c}%</div>
    ...
    `,a}switchEconomySubtab(e){const t=document.getElementById("economic-simulator-modal");t&&(t.querySelectorAll(".economy-subtab").forEach(a=>{a.dataset.subtab===e?(a.classList.remove("bg-slate-700","text-slate-300"),a.classList.add("bg-purple-600","text-white")):(a.classList.remove("bg-purple-600","text-white"),a.classList.add("bg-slate-700","text-slate-300"))}),t.querySelectorAll(".economy-subtab-content").forEach(a=>{a.classList.add("hidden")}),t.querySelector(`#economy-subtab-${e}`)?.classList.remove("hidden"))}adjustTechnology(e,t){this.changes.technology[e]=(this.changes.technology[e]||0)+t;const a=document.querySelector(`.tech-input[data-field="${e}"]`);a&&(a.value=this.changes.technology[e]),this.updateTechPreview(e),this.updateSummary()}updateTechPreview(e){const t=this.getCountryById(this.selectedCountry);if(!t)return;const a=parseFloat(t[e])||0,o=this.changes.technology[e]||0,r=Math.max(0,a+o),s=document.querySelector(`.tech-preview[data-field="${e}"]`);s&&(s.textContent=r)}adjustIndicator(e,t){const a=this.getCountryById(this.selectedCountry);if(!a)return;const o=parseFloat(a[e])||0,s=(this.changes.indicators[e]||0)+t,n=o+s;if(n<0||n>100){g("warning",`${e} deve estar entre 0% e 100%`);return}this.changes.indicators[e]=s;const i=document.querySelector(`.indicator-input[data-field="${e}"]`);i&&(i.value=this.changes.indicators[e]),this.updateIndicatorPreview(e),this.updateSummary()}updateIndicatorPreview(e){const t=this.getCountryById(this.selectedCountry);if(!t)return;const a=parseFloat(t[e])||0,o=this.changes.indicators[e]||0,r=Math.min(100,Math.max(0,a+o)),s=document.querySelector(`.indicator-preview[data-field="${e}"]`);s&&(s.textContent=`${r}%`);const n=s?.closest(".border")?.querySelector(".bg-gradient-to-r");n&&(n.style.width=`${r}%`)}resetActions(){this.actions.internal=[],this.actions.external=[],this.changes={technology:{},indicators:{}},this.recreateActionSlots("internal"),this.recreateActionSlots("external"),this.updateTabCounters(),this.updateBudgetBar(),this.updateSummary()}toggleActionDescription(e){const a=e.closest(".action-slot").querySelector(".action-description"),o=a.querySelector("div");if(e.value){const r=A.actionTypes[e.value];r&&(o.textContent=`${r.description}. Exemplos: ${r.examples.join(", ")}.`,a.classList.remove("hidden"))}else a.classList.add("hidden")}toggleResourceSelector(e){const t=e.closest(".action-slot"),a=t.querySelector(".resource-type-selector"),o=t.querySelector(".extraction-preview");e.value==="exploration"?(a?.classList.remove("hidden"),o?.classList.remove("hidden")):(a?.classList.add("hidden"),o?.classList.add("hidden"))}updateExtractionPreview(e,t){const a=e.querySelector(".extraction-amount");if(!a)return;if(!t.resourceType||!t.value||!t.dice){a.textContent="Aguardando dados...";return}const o=this.getCountryById(this.selectedCountry);if(!o)return;const s={Combustivel:"PotencialCombustivel",Carvao:"PotencialCarvao",Metais:"PotencialMetais"}[t.resourceType],n=parseFloat(o[s])||0,i=t.value/10*(t.dice/12)*(n/10),c=Math.round(i*100)/100,l={Combustivel:"Combustível",Carvao:"Carvão",Metais:"Metais"};a.innerHTML=`<strong>${c.toFixed(2)}</strong> unidades de ${l[t.resourceType]} (Potencial: ${n}/10)`}async applyEconomicActions(){const e=document.getElementById("economic-simulator-modal"),t=document.getElementById("apply-economic-actions");if(!(!e||!t))try{t.disabled=!0,t.textContent="⏳ Aplicando...";const a=this.getCountryById(this.selectedCountry);if(!a)throw new Error("País não encontrado");const o=[...this.actions.internal,...this.actions.external].filter(d=>d.type&&d.value>0),r=Object.values(this.changes.technology).some(d=>d!==0),s=Object.values(this.changes.indicators).some(d=>d!==0);if(!(o.length>0||r||s))throw new Error("Nenhuma mudança configurada");const i={};for(const d of o.filter(v=>v.isExternal))d.targetCountry&&(i[d.targetCountry]=this.getCountryById(d.targetCountry));const c=o.length>0?k.processAllActions(o,a,i):null;await this.saveEconomicResults(c,o,i);try{await Ae.generatePlayerFeedback(this.selectedCountry,c,o),u.info("Feedback narrativo gerado para o player")}catch(d){u.warn("Erro ao gerar feedback narrativo:",d)}let l="Mudanças aplicadas com sucesso!";if(c)l=`Investimentos aplicados! PIB cresceu ${(c.finalGrowth*100).toFixed(2)}%`;else if(r||hasResourceChanges||s){const d=[];r&&d.push("tecnologia"),hasResourceChanges&&d.push("recursos"),s&&d.push("indicadores"),l=`Mudanças em ${d.join(", ")} aplicadas!`}g("success",l),e.remove(),window.carregarTudo&&await window.carregarTudo(),setTimeout(()=>{window.location.pathname.includes("narrador")&&window.location.reload()},1500)}catch(a){u.error("Erro ao aplicar ações econômicas:",a),g("error",`Erro: ${a.message}`)}finally{t&&(t.disabled=!1,t.textContent="⚡ Aplicar Investimentos")}}async saveEconomicResults(e,t,a){const o=p.batch(),r=parseInt(document.getElementById("turno-atual-admin")?.textContent?.replace("#",""))||1;try{const s=p.collection("paises").doc(this.selectedCountry),n=this.getCountryById(this.selectedCountry)||{},i={TurnoUltimaAtualizacao:r};e&&(i.PIB=e.newPIB,i.PIBPerCapita=e.newPIBPerCapita,i["geral.PIB"]=e.newPIB,i["geral.PIBPerCapita"]=e.newPIBPerCapita);for(const[b,w]of Object.entries(this.changes.technology))if(w!==0){const D=parseFloat(n[b])||0,S=Math.max(0,D+w);i[b]=S,i[`geral.${b}`]=S}for(const[b,w]of Object.entries(this.changes.indicators))if(w!==0){const D=parseFloat(n[b])||0,S=Math.min(100,Math.max(0,D+w));i[b]=S,i[`geral.${b}`]=S}const c=t.filter(b=>b.type==="exploration"&&b.resourceType&&b.value&&b.dice);for(const b of c){const D={Combustivel:"PotencialCombustivel",Carvao:"PotencialCarvao",Metais:"PotencialMetais"}[b.resourceType],S=parseFloat(n[D])||0,W=b.value/10*(b.dice/12)*(S/10),q=Math.round(W*100)/100,z={Combustivel:"CombustivelProducao",Carvao:"CarvaoProducao",Metais:"MetaisProducao"}[b.resourceType],he=parseFloat(n[z])||0;i[z]=he+q,u.log(`[Exploration] ${n.Pais}: Extraiu ${q} unidades de ${b.resourceType} (Potencial: ${S}/10, Dado: ${b.dice}/12)`)}let l=0,d=0,v=0,y=0;const x=n.PoliticaIndustrial||n.Politica||"combustivel";for(const b of t){if(b.type==="industry"){const w=k.computeIndustryResourceConsumption(b.value,n);x==="carvao"?d+=w:l+=w,v+=(parseFloat(b.value)||0)*.5}if(b.type==="exploration"){const w=parseFloat(b.value)||0,D=parseFloat(n.PotencialCarvao||n.Potencial||n.PotencialCarvao||0)||0,S=Math.min(D*.1,w*.1);y+=S}b.type==="research"&&(v+=(parseFloat(b.value)||0)*.2)}const h=parseFloat(n.IndustrialEfficiency)||50,C=t.filter(b=>b.type==="industry").length*.5,I=Math.min(100,h+C+(e.technologyChanges||0)*.2);i.IndustrialEfficiency=I;const j=parseFloat(n.EnergiaCapacidade)||parseFloat(n.EnergiaDisponivel)||0,R=k.computeEnergyPenalty(j,v);if(R<1){const b=(1-R)*100,w=e.newPIB*(1-R)*.1;e.newPIB=Math.max(e.newPIB-w,e.newPIB*.95),e.newPIBPerCapita=e.newPIB/(parseFloat(n.Populacao)||1),u.info(`Penalidade de energia aplicada: ${b.toFixed(1)}% déficit, -${w.toFixed(0)} PIB`)}i.EnergiaCapacidade=j;const H=parseFloat(n.Combustivel)||0,le=parseFloat(n.CarvaoSaldo||n.Carvao||0),ae=Math.max(0,H-l),de=Math.max(0,le-d+y);i.Combustivel=ae,i.CarvaoSaldo=de,y>0&&(Q.results.producedCarvao=y);const ue={Graos:n.Graos||0,Combustivel:ae,EnergiaDisponivel:j},G=k.computeConsumerGoodsIndex(n,ue);i.BensDeConsumo=G;const N=parseFloat(n.Estabilidade)||0;if(G>75?(i.Estabilidade=Math.min(100,N+3),i["geral.Estabilidade"]=Math.min(100,N+3)):G<25&&(i.Estabilidade=Math.max(0,N-3),i["geral.Estabilidade"]=Math.max(0,N-3)),e.technologyChanges>0){const b=parseFloat(this.getCountryById(this.selectedCountry).Tecnologia)||0,w=Math.min(100,b+e.technologyChanges);i.Tecnologia=w,i["geral.Tecnologia"]=w}if(e.stabilityChanges>0){const b=parseFloat(this.getCountryById(this.selectedCountry).Estabilidade)||0,w=Math.min(100,b+e.stabilityChanges);i.Estabilidade=w,i["geral.Estabilidade"]=w}o.update(s,i);for(const b of t.filter(w=>w.isExternal))if(b.targetCountry&&a[b.targetCountry]){const w=a[b.targetCountry],S=k.calculateBaseGrowth(b,w).preInflationGrowth*b.value/1e6*.5,W=p.collection("paises").doc(b.targetCountry),q=parseFloat(w.Populacao)||1,J=parseFloat(w.PIBPerCapita||0)+S/1e6,z=q*J;o.update(W,{PIB:z,PIBPerCapita:J,TurnoUltimaAtualizacao:r})}const me=p.collection("economic_history").doc(),Q={countryId:this.selectedCountry,turn:r,timestamp:new Date,totalInvestment:t.reduce((b,w)=>b+(parseFloat(w.value)||0),0),actions:t,results:{totalGrowth:e.totalGrowth,finalGrowth:e.finalGrowth,inflation:e.totalInflation,newPIB:e.newPIB,productiveChains:e.productiveChains},externalInvestments:{}};t.filter(b=>b.isExternal).forEach(b=>{b.targetCountry&&(Q.externalInvestments[b.targetCountry]=parseFloat(b.value)||0)}),o.set(me,Q);const pe=p.collection("change_history").doc();o.set(pe,{countryId:this.selectedCountry,section:"economia",field:"simulacao_economica",oldValue:{PIB:parseFloat(this.getCountryById(this.selectedCountry).PIB),PIBPerCapita:parseFloat(this.getCountryById(this.selectedCountry).PIBPerCapita)||0},newValue:{PIB:e.newPIB,PIBPerCapita:e.newPIBPerCapita},userName:T.currentUser?.displayName||"Narrador",reason:`Simulação econômica: ${t.length} ações aplicadas`,timestamp:new Date,turn:r}),await o.commit(),u.info("Simulação econômica aplicada com sucesso")}catch(s){throw u.error("Erro ao salvar resultados econômicos:",s),s}}async buildPowerPlant(e,t){try{const a=this.getCountryById(e);if(!a)return g("error","País não encontrado."),{success:!1,message:"País não encontrado."};const o=ye[t];if(!o)return g("error","Tipo de usina inválido."),{success:!1,message:"Tipo de usina inválido."};if(a.PIB<o.cost)return g("error",`PIB insuficiente para construir ${o.name}. Necessário: ${this.formatCurrency(o.cost)}`),{success:!1,message:"PIB insuficiente."};if(a.Tecnologia<o.tech_requirement)return g("error",`Tecnologia insuficiente para construir ${o.name}. Necessário: ${o.tech_requirement}%`),{success:!1,message:"Tecnologia insuficiente."};if(o.type==="hydro"){if(!a.PotencialHidreletrico||a.PotencialHidreletrico<=0)return g("error",`País não possui potencial hidrelétrico para construir ${o.name}.`),{success:!1,message:"Potencial hidrelétrico insuficiente."};a.PotencialHidreletrico--}if(o.resource_input==="Uranio"){if(!a.Uranio||a.Uranio<=0)return g("error",`País não possui Urânio suficiente para construir ${o.name}.`),{success:!1,message:"Urânio insuficiente."};a.Uranio--}return await p.runTransaction(async r=>{const s=p.collection("paises").doc(e),i=(await r.get(s)).data(),c=i.PIB-o.cost,l=[...i.power_plants||[],{id:t,built_turn:parseInt(document.getElementById("turno-atual-admin")?.textContent?.replace("#",""))||1}],d={PIB:c,power_plants:l,...o.type==="hydro"&&{PotencialHidreletrico:a.PotencialHidreletrico},...o.resource_input==="Uranio"&&{Uranio:a.Uranio}};r.update(s,d)}),a.PIB-=o.cost,a.power_plants.push({id:t,built_turn:parseInt(document.getElementById("turno-atual-admin")?.textContent?.replace("#",""))||1}),g("success",`${o.name} construída com sucesso!`),u.info(`${o.name} construída para ${a.Pais}`,{countryId:e,plantTypeId:t}),{success:!0,message:`${o.name} construída.`}}catch(a){return u.error(`Erro ao construir usina ${t} para ${e}:`,a),g("error",`Erro ao construir usina: ${a.message}`),{success:!1,message:`Erro ao construir usina: ${a.message}`}}}}let Y=null;async function Te(){try{return Y=new Be,await Y.initialize(),Y}catch(m){throw u.error("Erro ao inicializar simulador econômico:",m),m}}async function Le(){try{if(!await L("Confirmar Migração de Dados","Esta ação irá verificar TODOS os países e adicionar os novos campos de economia (Carvão, Energia, etc.) com valores padrão. Execute esta operação APENAS UMA VEZ. Deseja continuar?","Sim, migrar agora","Cancelar")){g("info","Migração cancelada pelo usuário.");return}g("info","Iniciando migração... Isso pode levar um momento.");const e=await p.collection("paises").get(),t=p.batch();let a=0;e.forEach(o=>{const r=o.data(),s=o.ref,n={};r.PotencialCarvao===void 0&&(n.PotencialCarvao=3),r.CarvaoSaldo===void 0&&(n.CarvaoSaldo=0),r.PoliticaIndustrial===void 0&&(n.PoliticaIndustrial="combustivel"),r.Energia===void 0&&(n.Energia={capacidade:100,demanda:100}),r.IndustrialEfficiency===void 0&&(n.IndustrialEfficiency=30),r.BensDeConsumo===void 0&&(n.BensDeConsumo=50),r.PotencialHidreletrico===void 0&&(n.PotencialHidreletrico=2),r.Uranio===void 0&&(n.Uranio=0),r.EnergiaCapacidade===void 0&&(n.EnergiaCapacidade=100),r.power_plants===void 0&&(n.power_plants=[]),Object.keys(n).length>0&&(a++,t.update(s,n))}),a>0?(await t.commit(),g("success",`${a} países foram migrados com sucesso!`)):g("info","Nenhum país precisava de migração. Tudo já está atualizado.")}catch(m){console.error("Erro durante a migração:",m),g("error",`Erro na migração: ${m.message}`)}}function De(){const m=document.querySelectorAll(".tab-button"),e=document.querySelectorAll(".tab-panel");if(!m.length||!e.length)return;function t(o){m.forEach(n=>{n.classList.remove("border-brand-500","text-brand-300"),n.classList.add("border-transparent","text-slate-400"),n.setAttribute("aria-selected","false")}),e.forEach(n=>{n.classList.add("hidden")});const r=document.querySelector(`[data-tab="${o}"]`),s=document.getElementById(`panel-${o}`);r&&s&&(r.classList.remove("border-transparent","text-slate-400"),r.classList.add("border-brand-500","text-brand-300"),r.setAttribute("aria-selected","true"),s.classList.remove("hidden")),o==="players"?setTimeout(()=>{window.playerManager&&(window.playerManager.loadPlayers(),window.playerManager.loadCountries())},100):o==="gameplay"&&setTimeout(()=>{window.updateQuickStats&&window.updateQuickStats()},100)}m.forEach(o=>{o.addEventListener("click",()=>{const r=o.getAttribute("data-tab");t(r)})});const a=m[0]?.getAttribute("data-tab");a&&t(a),window.updateTabBadges=function(o){const r=document.getElementById("gameplay-badge"),s=document.getElementById("players-badge");r&&o?.vehiclesPending>0?(r.textContent=o.vehiclesPending,r.classList.remove("hidden")):r&&r.classList.add("hidden"),s&&o?.playersOnline>0?(s.textContent=o.playersOnline,s.classList.remove("hidden")):s&&s.classList.add("hidden")},window.switchTab=t}const O={geral:{label:"Geral",campos:[{key:"PIBPerCapita",label:"PIB per Capita",tipo:"moeda",min:0},{key:"PIB",label:"PIB Total",tipo:"calculado",dependeDe:["PIBPerCapita","Populacao"]},{key:"Populacao",label:"População",tipo:"inteiro",min:0},{key:"Estabilidade",label:"Estabilidade",tipo:"percent",min:0,max:100},{key:"Burocracia",label:"Burocracia",tipo:"percent",min:0,max:100},{key:"Urbanizacao",label:"Urbanização",tipo:"percent",min:0,max:100},{key:"Tecnologia",label:"Tecnologia",tipo:"percent",min:0,max:100},{key:"ModeloPolitico",label:"Modelo Político",tipo:"texto"},{key:"Visibilidade",label:"Visibilidade",tipo:"opcoes",opcoes:["Público","Privado"]}]},exercito:{label:"Exército",campos:[{key:"Infantaria",label:"Infantaria",tipo:"inteiro",min:0},{key:"Artilharia",label:"Artilharia",tipo:"inteiro",min:0}]},aeronautica:{label:"Aeronáutica",campos:[{key:"Caca",label:"Caça",tipo:"inteiro",min:0},{key:"CAS",label:"CAS",tipo:"inteiro",min:0},{key:"Bomber",label:"Bombardeiro",tipo:"inteiro",min:0}]},marinha:{label:"Marinha",campos:[{key:"Fragata",label:"Fragata",tipo:"inteiro",min:0},{key:"Destroyer",label:"Destroyer",tipo:"inteiro",min:0},{key:"Submarino",label:"Submarino",tipo:"inteiro",min:0},{key:"Transporte",label:"Transporte",tipo:"inteiro",min:0}]},inventario:{label:"Inventário de Veículos",campos:[{key:"cavalos",label:"Cavalos",tipo:"inteiro",min:0},{key:"tanquesLeves",label:"Tanques Leves",tipo:"inteiro",min:0},{key:"mbt",label:"MBT",tipo:"inteiro",min:0},{key:"tanquesPesados",label:"Tanques Pesados",tipo:"inteiro",min:0},{key:"caminhoes",label:"Caminhões de Transporte",tipo:"inteiro",min:0},{key:"spg",label:"SPG",tipo:"inteiro",min:0},{key:"sph",label:"SPH",tipo:"inteiro",min:0},{key:"spaa",label:"SPAA",tipo:"inteiro",min:0},{key:"apc",label:"APC",tipo:"inteiro",min:0},{key:"cacaTanques",label:"Caça-Tanques",tipo:"inteiro",min:0},{key:"veiculosEng",label:"Veículos de Engenharia",tipo:"inteiro",min:0},{key:"ifv",label:"IFV",tipo:"inteiro",min:0}]},recursos:{label:"Recursos",campos:[{key:"Graos",label:"Graos (estoque)",tipo:"inteiro",min:0},{key:"Combustivel",label:"Combustível (unidades)",tipo:"inteiro",min:0},{key:"CombustivelSaldo",label:"Saldo de Combustível",tipo:"inteiro"},{key:"Metais",label:"Metais",tipo:"inteiro"},{key:"PotencialCarvao",label:"Potencial de Carvão (Jazidas)",tipo:"inteiro",min:0}]},ocupacao:{label:"Ocupação",campos:[{key:"PopOcupada",label:"População Ocupada",tipo:"inteiro",min:0},{key:"PIBOcupado",label:"PIB Ocupado",tipo:"moeda",min:0}]},arsenal:{label:"Arsenal Especial",campos:[{key:"Nuclear",label:"Bomba Nuclear",tipo:"inteiro",min:0}]}};let $=null,E={paises:[],paisSelecionado:null,secaoSelecionada:"geral",realTimeEnabled:!0,autoSave:!0,listeners:new Map,pendingChanges:new Set};const f={gate:document.getElementById("gate"),adminRoot:document.getElementById("admin-root"),turnoAtual:document.getElementById("turno-atual-admin"),menuSecoes:document.getElementById("menu-secoes"),selectPais:document.getElementById("select-pais"),selectSecao:document.getElementById("select-secao"),formSecao:document.getElementById("form-secao"),listaCampos:document.getElementById("lista-campos-secao"),btnSalvarSecao:document.getElementById("btn-salvar-secao"),btnRecarregar:document.getElementById("btn-recarregar"),btnSalvarTurno:document.getElementById("btn-salvar-turno"),turnoInput:document.getElementById("turno-input"),btnSalvarCatalogo:document.getElementById("btn-salvar-catalogo"),btnCarregarCatalogo:document.getElementById("btn-carregar-catalogo"),btnAdicionarCampo:document.getElementById("btn-adicionar-campo"),logout:document.getElementById("logout-link"),realTimeToggle:document.getElementById("realtime-toggle"),autoSaveToggle:document.getElementById("autosave-toggle"),historyList:document.getElementById("history-list"),historyRefresh:document.getElementById("history-refresh"),exportHistory:document.getElementById("export-history"),playersList:document.getElementById("players-list"),availableCountries:document.getElementById("available-countries"),playerCount:document.getElementById("player-count"),availableCount:document.getElementById("available-count"),refreshPlayers:document.getElementById("refresh-players"),assignRandom:document.getElementById("assign-random"),clearAllAssignments:document.getElementById("clear-all-assignments")};async function ke(){try{const m=await p.collection("configuracoes").doc("campos").get(),e=m.exists?m.data():{};$=Object.assign({},O,e),Object.keys(O).forEach(t=>{$[t]?(!$[t].campos||$[t].campos.length===0)&&($[t].campos=O[t].campos):$[t]=O[t]})}catch(m){console.warn("Falha ao carregar catálogo, usando local.",m),$=O}}function te(){!f.menuSecoes||!$||(f.menuSecoes.innerHTML="",Object.keys($).forEach(m=>{const e=$[m],t=document.createElement("button");t.type="button",t.className=`w-full text-left rounded-md px-2 py-1.5 text-sm ${E.secaoSelecionada===m?"bg-white/5 border border-slate-600/40":"border border-transparent hover:bg-white/5"}`,t.textContent=e.label||m,t.onclick=()=>{E.secaoSelecionada=m,te(),U()},f.menuSecoes.appendChild(t)}),f.selectSecao&&(f.selectSecao.innerHTML=Object.keys($).map(m=>`<option value="${m}" ${m===E.secaoSelecionada?"selected":""}>${$[m].label||m}</option>`).join("")))}function Me(){f.selectPais&&(f.selectPais.innerHTML="",E.paises.forEach(m=>{const e=document.createElement("option");e.value=m.id,e.textContent=m.Pais||m.id,f.selectPais.appendChild(e)}),!E.paisSelecionado&&E.paises.length&&(E.paisSelecionado=E.paises[0].id),E.paisSelecionado&&(f.selectPais.value=E.paisSelecionado))}function Fe(m,e,t,a=null){const o=document.createElement("div"),r=document.createElement("label");r.className="block text-xs text-slate-400 mb-1",r.textContent=e.label||m;let s;if(e.tipo==="calculado"){if(s=document.createElement("div"),s.className="mt-1 w-full rounded-lg bg-slate-700/50 border border-slate-600 p-2 text-sm text-slate-300 italic",m==="PIB"&&a){const n=parseFloat(a.Populacao)||0,i=parseFloat(a.PIBPerCapita)||0,c=ie(n,i);s.textContent=`${ne(c)} (calculado)`,s.dataset.calculatedValue=c}else s.textContent="Campo calculado";s.name=m}else e.tipo==="opcoes"&&Array.isArray(e.opcoes)?(s=document.createElement("select"),e.opcoes.forEach(n=>{const i=document.createElement("option");i.value=n,i.textContent=n,t===n&&(i.selected=!0),s.appendChild(i)}),s.name=m,s.className="mt-1 w-full rounded-lg bg-bg border border-bg-ring/70 p-2 text-sm focus:border-brand-500/50 focus:ring-1 focus:ring-brand-500/50 transition-colors"):(s=document.createElement("input"),e.tipo==="percent"||e.tipo==="inteiro"||e.tipo==="moeda"?s.type="number":s.type="text",s.value=t??"",e.min!=null&&(s.min=String(e.min)),e.max!=null&&(s.max=String(e.max)),e.tipo==="moeda"?s.step="0.01":e.tipo==="percent"?s.step="0.1":e.tipo==="inteiro"&&(s.step="1"),s.name=m,s.className="mt-1 w-full rounded-lg bg-bg border border-bg-ring/70 p-2 text-sm focus:border-brand-500/50 focus:ring-1 focus:ring-brand-500/50 transition-colors");return o.appendChild(r),o.appendChild(s),{wrap:o,input:s,get:()=>e.tipo==="calculado"?Number(s.dataset.calculatedValue||0):e.tipo==="inteiro"||e.tipo==="percent"||e.tipo==="moeda"?Number(s.value||0):s.value??""}}function U(){if(!$||!f.formSecao)return;const m=E.paises.find(o=>o.id===E.paisSelecionado),e=$[E.secaoSelecionada],t=m&&m[E.secaoSelecionada]||{};f.formSecao.innerHTML="",f.listaCampos&&(f.listaCampos.innerHTML="");const a={};(e.campos||[]).forEach(o=>{const r=Fe(o.key,o,t[o.key]??m?.[o.key],m);f.formSecao.appendChild(r.wrap),a[o.key]=r.get}),f.btnSalvarSecao&&(f.btnSalvarSecao.onclick=async()=>{if(m)try{f.btnSalvarSecao.disabled=!0,f.btnSalvarSecao.textContent="Salvando...";const o={};Object.keys(a).forEach(s=>o[s]=a[s]()),E.secaoSelecionada==="geral"&&o.PIBPerCapita&&o.Populacao&&(o.PIB=ie(o.Populacao,o.PIBPerCapita));const r={[`${E.secaoSelecionada}`]:o};E.secaoSelecionada==="geral"&&Object.assign(r,o),await p.collection("paises").doc(m.id).update(r),g("success","Seção salva com sucesso")}catch(o){u.error("Erro ao salvar seção:",o),g("error",`Erro ao salvar: ${o.message}`)}finally{f.btnSalvarSecao.disabled=!1,f.btnSalvarSecao.textContent="Salvar Seção"}})}async function Ve(m){if(!m){window.location.href="index.html";return}try{const e=await ve(m.uid);if(!e.isNarrator&&!e.isAdmin){f.gate&&f.gate.classList.remove("hidden"),f.adminRoot?.classList.add("hidden");return}f.gate&&f.gate.classList.add("hidden"),f.adminRoot?.classList.remove("hidden"),await ke(),await B()}catch(e){console.error("Erro no gatekeeper",e),f.gate&&f.gate.classList.remove("hidden"),f.adminRoot?.classList.add("hidden")}}async function B(){const m=await re();m&&m.turnoAtual&&f.turnoAtual&&(f.turnoAtual.textContent=`#${m.turnoAtual}`),m&&m.turnoAtual&&f.turnoInput&&(f.turnoInput.value=m.turnoAtual),E.paises=await se(),E.paises.sort((e,t)=>(e.Pais||"").localeCompare(t.Pais||"")),Me(),te(),U()}T.onAuthStateChanged(Ve);f.selectPais&&f.selectPais.addEventListener("change",async m=>{E.paisSelecionado=m.target.value,U();try{await activateEnergyForSelectedCountry()}catch(e){u.warn("Erro ao ativar EnergyManager após mudança de país:",e)}});f.selectSecao&&f.selectSecao.addEventListener("change",m=>{E.secaoSelecionada=m.target.value,te(),U()});f.btnRecarregar&&f.btnRecarregar.addEventListener("click",B);f.btnSalvarTurno&&f.btnSalvarTurno.addEventListener("click",async()=>{const m=Number(f.turnoInput?.value||"");if(Number.isNaN(m)||m<0){alert("Informe um número de turno válido.");return}await ge(m),alert(`Turno atualizado para #${m}`),await B()});f.logout&&f.logout.addEventListener("click",m=>{m.preventDefault(),T.signOut()});document.addEventListener("DOMContentLoaded",()=>{De();const m=document.getElementById("btn-open-rules-editor"),e=document.getElementById("rules-editor-panel");m&&e&&m.addEventListener("click",()=>{e.classList.toggle("hidden")});const t=document.getElementById("btn-run-migration");t&&t.addEventListener("click",()=>{Le()});const a=document.getElementById("btn-process-energy");a&&a.addEventListener("click",async()=>{try{a.disabled=!0,a.textContent="⏳ Processando...";const{processEnergySystemTurn:h}=await P(async()=>{const{processEnergySystemTurn:C}=await import("./energyPenaltyProcessor-DisKBzyJ.js");return{processEnergySystemTurn:C}},__vite__mapDeps([2,0,1,3]));await h(),alert("Turno de energia processado com sucesso!"),await B()}catch(h){console.error("Erro ao processar energia:",h),alert("Erro ao processar energia: "+h.message)}finally{a.disabled=!1,a.textContent="⚡ Processar Turno de Energia"}});const o=document.getElementById("btn-assign-resources");o&&o.addEventListener("click",async()=>{try{o.disabled=!0,o.textContent="⏳ Processando...";const{assignResourcePotentials:h}=await P(async()=>{const{assignResourcePotentials:C}=await import("./assign-resource-potentials-DzDnj0dJ.js");return{assignResourcePotentials:C}},__vite__mapDeps([4,0,1]));await h(),await B()}catch(h){console.error("Erro ao atribuir recursos:",h),alert("Erro ao atribuir recursos: "+h.message)}finally{o.disabled=!1,o.textContent="🌍 Atribuir Potenciais de Recursos"}});const r=document.getElementById("btn-resource-report");r&&r.addEventListener("click",async()=>{try{const{generateResourceReport:h}=await P(async()=>{const{generateResourceReport:C}=await import("./assign-resource-potentials-DzDnj0dJ.js");return{generateResourceReport:C}},__vite__mapDeps([4,0,1]));h(),alert("Relatório de recursos gerado no console (F12)")}catch(h){console.error("Erro ao gerar relatório:",h),alert("Erro ao gerar relatório: "+h.message)}});const s=document.getElementById("btn-apply-consumption");s&&s.addEventListener("click",async()=>{try{s.disabled=!0,s.textContent="⏳ Calculando...";const{applyResourceConsumption:h}=await P(async()=>{const{applyResourceConsumption:C}=await import("./apply-resource-consumption-CnDlLCQ4.js");return{applyResourceConsumption:C}},__vite__mapDeps([5,0,1,6]));await h(),await B()}catch(h){console.error("Erro ao calcular consumo:",h),alert("Erro ao calcular consumo: "+h.message)}finally{s.disabled=!1,s.textContent="🍽️ Calcular Consumo de Recursos"}});const n=document.getElementById("btn-apply-consumption-all");n&&n.addEventListener("click",async()=>{try{if(!await L("Aplicar Consumo a Todos os Países","Esta ação irá calcular e definir o consumo mensal de recursos para TODOS os países baseado em suas características (população, PIB, tecnologia, etc.). Esta operação pode ser executada múltiplas vezes. Continuar?","Sim, aplicar","Cancelar")){g("info","Operação cancelada pelo usuário.");return}n.disabled=!0,n.textContent="⏳ Aplicando a todos os países...";const{applyResourceConsumption:C}=await P(async()=>{const{applyResourceConsumption:I}=await import("./apply-resource-consumption-CnDlLCQ4.js");return{applyResourceConsumption:I}},__vite__mapDeps([5,0,1,6]));await C(),g("success","🎉 Consumo aplicado a todos os países! Recarregue o dashboard para ver os novos valores."),await B()}catch(h){console.error("Erro ao aplicar consumo:",h),g("error","Erro ao aplicar consumo: "+h.message)}finally{n.disabled=!1,n.textContent="🚀 APLICAR CONSUMO A TODOS OS PAÍSES"}});const i=document.getElementById("btn-apply-production-all");i&&i.addEventListener("click",async()=>{try{if(!await L("Aplicar Produção a Todos os Países","Esta ação irá calcular e definir a produção mensal de recursos para TODOS os países baseado em suas características (população, PIB, tecnologia, geografia, clima). Esta operação pode ser executada múltiplas vezes. Continuar?","Sim, aplicar","Cancelar")){g("info","Operação cancelada pelo usuário.");return}i.disabled=!0,i.textContent="⏳ Aplicando produção a todos os países...";const{applyResourceProduction:C}=await P(async()=>{const{applyResourceProduction:I}=await import("./apply-resource-production-B1GNdxpT.js");return{applyResourceProduction:I}},__vite__mapDeps([7,0,1,8]));await C(),g("success","🎉 Produção aplicada a todos os países! Recarregue o dashboard para ver os novos valores."),await B()}catch(h){console.error("Erro ao aplicar produção:",h),g("error","Erro ao aplicar produção: "+h.message)}finally{i.disabled=!1,i.textContent="🏭 APLICAR PRODUÇÃO A TODOS OS PAÍSES"}});const c=document.getElementById("btn-simulate-production");c&&c.addEventListener("click",async()=>{try{c.disabled=!0,c.textContent="⏳ Simulando...";const{simulateProductionTurns:h}=await P(async()=>{const{simulateProductionTurns:C}=await import("./apply-resource-production-B1GNdxpT.js");return{simulateProductionTurns:C}},__vite__mapDeps([7,0,1,8]));await h(6),await B()}catch(h){console.error("Erro ao simular produção:",h),g("error","Erro na simulação: "+h.message)}finally{c.disabled=!1,c.textContent="📊 Simular Produção 6 Turnos"}});const l=document.getElementById("btn-apply-consumer-goods");l&&l.addEventListener("click",async()=>{try{if(!await L("Aplicar Bens de Consumo e Estabilidade","Esta ação irá calcular os bens de consumo para TODOS os países e aplicar os efeitos de estabilidade (+3% até -5%). Esta operação deve ser executada a cada turno. Continuar?","Sim, aplicar","Cancelar")){g("info","Operação cancelada pelo usuário.");return}l.disabled=!0,l.textContent="⏳ Aplicando bens de consumo...";const{applyConsumerGoodsEffects:C}=await P(async()=>{const{applyConsumerGoodsEffects:I}=await import("./apply-consumer-goods-Cb9gDCqT.js");return{applyConsumerGoodsEffects:I}},__vite__mapDeps([9,0,1,10]));await C(),g("success","🎉 Bens de consumo e efeitos de estabilidade aplicados! Recarregue o dashboard."),await B()}catch(h){console.error("Erro ao aplicar bens de consumo:",h),g("error","Erro ao aplicar bens de consumo: "+h.message)}finally{l.disabled=!1,l.textContent="🛍️ APLICAR BENS DE CONSUMO E ESTABILIDADE"}});const d=document.getElementById("btn-simulate-consumer-goods");d&&d.addEventListener("click",async()=>{try{d.disabled=!0,d.textContent="⏳ Simulando...";const{simulateConsumerGoodsOverTime:h}=await P(async()=>{const{simulateConsumerGoodsOverTime:C}=await import("./apply-consumer-goods-Cb9gDCqT.js");return{simulateConsumerGoodsOverTime:C}},__vite__mapDeps([9,0,1,10]));await h(5),await B()}catch(h){console.error("Erro ao simular bens de consumo:",h),g("error","Erro na simulação: "+h.message)}finally{d.disabled=!1,d.textContent="📈 Simular Estabilidade 5 Turnos"}});const v=document.getElementById("btn-test-turn-processing");v&&v.addEventListener("click",async()=>{try{v.disabled=!0,v.textContent="⏳ Testando...";const{default:h}=await P(async()=>{const{default:I}=await import("./turnProcessor-Cr7QHs9b.js");return{default:I}},__vite__mapDeps([11,1,0,10,12,13])),C=await h.testTurnProcessing();g("success",`Teste concluído: ${C.length} países analisados. Veja o console (F12) para detalhes.`)}catch(h){console.error("Erro no teste:",h),g("error","Erro no teste: "+h.message)}finally{v.disabled=!1,v.textContent="🧪 Testar Processamento de Turno"}});const y=document.getElementById("btn-reprocess-turn");y&&y.addEventListener("click",async()=>{try{if(!await L("Reprocessar Turno Atual","Esta ação irá forçar o reprocessamento do turno atual, aplicando novamente todos os efeitos de bens de consumo e estabilidade. Use apenas se necessário. Continuar?","Sim, reprocessar","Cancelar")){g("info","Operação cancelada.");return}y.disabled=!0,y.textContent="⏳ Reprocessando...";const I=(await re()).turnoAtual||1,{default:j}=await P(async()=>{const{default:H}=await import("./turnProcessor-Cr7QHs9b.js");return{default:H}},__vite__mapDeps([11,1,0,10,12,13])),R=await j.reprocessTurn(I);g("success",`Turno ${I} reprocessado: ${R.processedCount} países atualizados!`),await B()}catch(h){console.error("Erro no reprocessamento:",h),g("error","Erro no reprocessamento: "+h.message)}finally{y.disabled=!1,y.textContent="🔄 Reprocessar Turno Atual"}});const x=document.getElementById("btn-simulate-consumption");x&&x.addEventListener("click",async()=>{try{x.disabled=!0,x.textContent="⏳ Simulando...";const{simulateConsumptionTurns:h}=await P(async()=>{const{simulateConsumptionTurns:C}=await import("./apply-resource-consumption-CnDlLCQ4.js");return{simulateConsumptionTurns:C}},__vite__mapDeps([5,0,1,6]));await h(3),alert("Simulação concluída! Veja os resultados no console (F12)")}catch(h){console.error("Erro na simulação:",h),alert("Erro na simulação: "+h.message)}finally{x.disabled=!1,x.textContent="🔮 Simular 3 Turnos"}})});let X=null,K=null,ee=null,ce=null;async function je(){try{X=new $e,await X.initialize(),u.info("Sistema de aprovação de veículos inicializado")}catch(m){u.error("Erro ao inicializar sistema de aprovação de veículos:",m)}}async function Re(){try{K=new be,await K.initialize(),u.info("Sistema de produção naval inicializado")}catch(m){u.error("Erro ao inicializar sistema de produção naval:",m)}}async function Oe(){try{ee=new xe,await ee.initialize(),u.info("Sistema de inventário inicializado")}catch(m){u.error("Erro ao inicializar sistema de inventário:",m)}}async function _e(){try{ce=await Te(),u.info("Sistema econômico inicializado")}catch(m){u.error("Erro ao inicializar sistema econômico:",m)}}async function Ne(){try{V&&typeof V.loadPlayers=="function"?(await V.loadPlayers(),await V.loadCountries(),V.setupRealTimeListeners?.(),u.info("Player management inicializado")):u.warn("playerManager não disponível para inicialização")}catch(m){u.error("Erro ao inicializar player management:",m)}}async function oe(){try{console.log("🔧 Inicializando sistemas do narrador..."),await Promise.all([Ne(),je(),Re(),Oe(),_e()]),window.playerManager=V,window.vehicleApprovalSystem=X,window.navalProductionSystem=K,window.inventorySystem=ee,window.economicSimulator=ce,window.narratorData={getCatalog:()=>$,getCountries:()=>E.paises},console.log("✅ Todos os sistemas do narrador inicializados")}catch(m){console.error("❌ Erro ao inicializar sistemas do narrador:",m)}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",oe):oe();
